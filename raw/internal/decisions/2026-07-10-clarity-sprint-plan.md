...
- **PARTIAL 2026-07-11 (`0a1ceebe` + fixups):** Old demo plan SUSPENDED with a correction banner (scene 1 markets the internal Machine asset; scenes 2-3 stand on the retired regime); decision template committed with options + the explicit zero-staging rule-amendment condition; two-stage Slack plan bannered superseded/deferred. The founder decision itself is NOT made — X1/demo-shape call still owed before Jul 18.
- **PARTIAL 2026-07-11 (`cc952273`):** Trap-map '(already queued)' corrected to 'named, not yet specced'. The Claude private-memory CI-split record reassignment remains owed.
- **DONE 2026-07-11 (`cc952273`):** Per-stage evidence qualifiers annotated in the canonical model; 'full five-stage loop ran live' walked back there and in the advisor-cycle record.
- **DONE 2026-07-11 (`4d7a842a` + fixups):** Lifecycle vocabulary landed in the shape the orientation doc locked — status stays delivery-state (shipped|planned); lifecycle is the separate axis (active|deferred|retired) + superseded_by; schema, manifest, index rendering/validation, and retired-page migration done together.
...
- **DONE on `maint/clarity-phase2` 2026-07-11 (through `10c578fd`, independently rerun, pending merge):** Gitleaks 8.30.1 ran with `--all` and full redaction over textual history; zero findings. A required second pass enumerated 7 binary paths / 8 unique blobs, with archive traversal and 228,139 bytes of extracted ASCII/UTF-16LE/UTF-16BE strings also clean. Exact version, binary/checksum pins, commands, ref accounting, negative controls, and limitations are recorded in `2026-07-11-phase2-exposure-baseline.md`.
- **DONE for declared detectors on `maint/clarity-phase2` 2026-07-11:** Sanitized semantic-content inventory covers the tracked tree; `tools/semantic-history-scan.mjs` reproducibly covers known redaction diffs/anchors plus note-ID, absolute-path, and email-shaped classes without emitting values. It is explicitly not NER and does not clear semantic content.
- **DONE 2026-07-11 (`001d7fe3` + `849c0b3c`):** Enumerated redactions applied (fixture behaviorally equivalent, suite green); residuals recorded in the committed inventory.
- **DECIDED 2026-07-12 (`47ae3c03`):** Founder chose `DEFER` for the filter-repo history rewrite, owned by the founder and triggered by the first G4 exclusive window after holdout-131 evidence/branch/worktree closure, or an external report of flagged content, whichever occurs first. Known history exposure is accepted until then; decision input SHA `afe26cdc`.
- **PARTIAL on `maint/clarity-phase2` 2026-07-12:** Pinned repo-owned text+binary Gitleaks wrapper, immutable checkout/setup-node actions, Node 22 job runtime, all-path push/PR CI job, and manual idempotent pre-push installer implemented with 13/13 focused tests. The first branch run passed at `10d01db4`; separately, GitHub warned that the pinned v4 action implementations target deprecated Node 20 runtimes and forced those implementations to Node 24. Read-only GitHub audit is VERIFIED RED: native secret scanning/push protection, branch/ruleset protection, vulnerability alerts/fixes, enforced action pinning, protected release environment, and immutable releases are not enabled. Current release bytes/checksum match. Remediation or explicit risk disposition and the first post-merge `main` run remain open.
- **IN PROGRESS 2026-07-12:** Founder authorized Phase 3 remediation. Native secret scanning/push protection, dependency controls, CodeQL, main/tag rulesets, protected production environment, immutable releases, and private vulnerability reporting are active. The readiness branch updates all action pins, workflow-token permissions, release routing, and vulnerable locks. Initial CodeQL analysis exposed 77 non-workflow source findings requiring terminal disposition; G1 remains open. See `2026-07-12-phase3-github-security-remediation.md`.
- **DONE 2026-07-11 (`64d4eb2c` + fixups):** docs/committed-content-policy.md committed (incl. known-residuals section; no false CI-gate claim) + trap-map class 13 (PII/recording consent).
- **DONE 2026-07-11 (`64d4eb2c` + fixups):** docs/lab-data-handling.md committed — storage locations incl. WAL/SHM/logs/backups in the deletion story, custody, the full conditional egress inventory (not 'two endpoints'), MCP :38478, residual exposure.
- **DONE 2026-07-11 (`e4e541e9`, updated on Phase 2):** Tracking artifact committed and Job B now closes on the real Gitleaks run, not the earlier pattern report. Job C/filter-repo is separately deferred with owner, trigger, and residual exposure recorded.
...
- **DONE 2026-07-11 (`b6fc242a`):** Delete the 081 inbox zombie citing complete/081 (9bf44cea) as authoritative.
- **DONE 2026-07-11 (`d7298a40`):** Document inbox/ (parked, non-kanban, manual promotion gate), archive/, reviews/, and task-state/ in backlog/README.md.
- **DONE 2026-07-11 (`401ccc38`):** Add an Inbox section to tools/backlog_index.py and regenerate docs/BACKLOG.md.
...
- **DONE 2026-07-11 (`ce4fdec7`):** Three retired install docs bannered (SEND-TO-TESTER retired; example JSON gains a fail-loud _RETIRED marker key + sibling README; echoctl-install scope-bannered as Contract A).
...
- **DONE 2026-07-11 (`0122fa41`):** Fix the 2026-07 codex journal shard marker; journal-cat.sh 2026-07 verified exit 0.
- **DONE 2026-07-11 (`10a3d95d`):** Extend sync-skills.sh --check to flag orphan adapters (with ORPHAN_EXEMPT list); canonical skills/office-hours.md created.
...
| .claude/commands/office-hours.md | **DONE 2026-07-11 (`10a3d95d`)** — canonical skills/office-hours.md created and re-synced; --check extended with orphan detection + ORPHAN_EXEMPT list | The orphan adapter had no canonical source, which violated the skills-are-the-protocol rule and let --check pass falsely until the bidirectional check landed. |
