...
- **DONE on `maint/clarity-phase2` 2026-07-11 (through `8e0e591f`, pending merge + independent rerun):** Gitleaks 8.30.1 ran with `--all` and full redaction over textual history; zero findings. A required second pass enumerated 7 binary paths / 8 unique blobs, with archive traversal and 228,124 printable bytes also clean. Exact version, binary/checksum pins, commands, ref accounting, negative controls, and limitations are recorded in `2026-07-11-phase2-exposure-baseline.md`.
