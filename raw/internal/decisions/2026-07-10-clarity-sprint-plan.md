...
- **PARTIAL on `maint/clarity-phase2` 2026-07-12:** Pinned repo-owned text+binary Gitleaks wrapper, immutable checkout/setup-node actions with Node 22, all-path push/PR CI job, and manual idempotent pre-push installer implemented with 13/13 focused tests. The installer preserves other hook content by default and replaces only with explicit `--force`. Read-only GitHub audit is VERIFIED RED: native secret scanning/push protection, branch/ruleset protection, vulnerability alerts/fixes, enforced action pinning, protected release environment, and immutable releases are not enabled. Current release bytes/checksum match. Remediation or explicit risk disposition and the first landed secret-scan run remain open.
