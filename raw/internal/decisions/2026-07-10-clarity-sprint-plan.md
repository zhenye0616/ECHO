...
- **DECIDED 2026-07-12:** Founder chose `DEFER` for the filter-repo history rewrite, owned by the founder and triggered by the first G4 exclusive window after holdout-131 evidence/branch/worktree closure, or an external report of flagged content, whichever occurs first. Known history exposure is accepted until then; decision input SHA `afe26cdc`.
- **PARTIAL on `maint/clarity-phase2` 2026-07-11 (through `10c578fd`):** Pinned repo-owned text+binary Gitleaks wrapper, immutable checkout/setup-node actions with Node 22, all-path push/PR CI job, and manual idempotent pre-push installer implemented with 13/13 focused tests. The installer preserves other hook content by default and replaces only with explicit `--force`. GitHub push protection, repo settings, and authenticated release-asset policy remain unverified because the local `gh` token is invalid; record or explicitly defer that evidence rather than guessing.
