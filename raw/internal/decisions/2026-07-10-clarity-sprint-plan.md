...
- **DONE on `maint/clarity-phase2` 2026-07-11 (`5bd7b0cd`, pending merge + independent rerun):** Gitleaks 8.30.1 ran with `--all` and full redaction over reachable history; zero findings. Exact version, binary/checksum pins, command, ref accounting, negative control, and limitations are recorded in `2026-07-11-phase2-exposure-baseline.md`.
- **DONE for declared detectors on `maint/clarity-phase2` 2026-07-11:** Sanitized semantic-content inventory covers the tracked tree; the separate reachable-history pass covers known redaction diffs/anchors plus note-ID, absolute-path, and email-shaped classes without emitting values. It is explicitly not NER and does not clear semantic content.
...
- **PARTIAL on `maint/clarity-phase2` 2026-07-11 (`5bd7b0cd`):** Pinned repo-owned Gitleaks wrapper, all-path push/PR CI job, and manual idempotent pre-push installer implemented with 9/9 focused tests. GitHub push protection, repo settings, and authenticated release-asset policy remain unverified because the local `gh` token is invalid; record or explicitly defer that evidence rather than guessing.
