...
**Date:** 2026-07-10 · **Updated:** 2026-07-11 · **Evidence baseline:** `67f5949e` + founder commercial-focus decision · **Status:** founder orientation view
...
| V1 | Signals first-run cutoff and newest-first semantics | Gap verified; semantics open | Decision note + post-lift queue rank | Strategist | pending | — |
| V2 | Unattended brain auth expiry | Untested | Timed probe with predeclared pass/fail rubric | Operator | pending | — |
| A2 | Cold-db extraction quality | Untested | Scratch-ECHO_HOME comparison against warm baseline | Operator + independent grader | pending | — |
| A3 | Brain economics, terms, and key custody | API-key direction decided; analysis open | Cost model + account/custody/terms decision | Founder + strategist | pending | — |
| A5 | Repeatable client topology | Client-local direction locked; customer-2/multi-user/support shape open | Deferred state naming owner and customer-2 trigger, or topology decision | Founder | pending | — |
| A6 | Public history and customer-data handling | Exposure remediated/scanned; Job C rewrite deferred; branch CI green; GitHub remediation in progress; CodeQL gate exposed | Scan report + data/retention/deletion/custody decision | Founder + security executor | pending | Job C deferred in `2026-07-11-filter-repo-decision-template.md`; pre-remediation audit `2026-07-12-github-security-readonly-audit.md`; current controls/findings `2026-07-12-phase3-github-security-remediation.md`; awaits landing evidence, post-land enforcement, and terminal CodeQL dispositions |
| B1 | Lab rollout calendar | Open | Milestones and dependency dates confirmed with lab | Founder + lab counterpart | pending | — |
| B5 | Pricing and customer-visible name | Product choice closed; commercial mechanics open | Pricing posture + rename decision or explicit timed deferral | Founder | pending | — |
| B6 | Phase-1 operational definition of done | Product choice closed; delivery outcome open | Client-machine install/use/repeat metric plus rollout, rollback, and support thresholds; never a product-demand kill gate | Founder | pending | — |
| T1 | Product source allowlist | Open | Source-by-source inclusion decision | Strategist | pending | — |
| T2 | Sidecar classification | Open | Instance-local vs transferable state table | Strategist | pending | — |
| T5 | Remote-write path | Open | Direction decision with failure/rollback boundary | Strategist + operator | pending | — |
| T7 | Cross-instance row identity | Open | Identity contract or explicit single-instance deferral | Strategist | pending | — |
| T8 | Embeddings on the client machine | Open | Include/exclude decision with privacy/cost rationale | Founder + strategist | pending | — |
| T9 | Backup rule | Open | Backup/restore decision plus drill or named pre-client blocker | Operator | pending | — |
| T10 | Migration order on populated db | Open | Upgrade/migration/rollback order plus rehearsal gate | Operator | pending | — |
| T11 | Granola quota and plan gating | Open | Measured quota/plan result and payer decision | Founder | pending | — |
| C1 | Lab transcript/workspace settings | Unknown | Access-discovery checklist completed with lab | Founder + lab counterpart | pending | — |
| C2 | Meeting language and length distribution | Unknown | Sample or interview result; accepted bounds documented | Founder + lab counterpart | pending | — |
| C3 | Internal vs external meeting mix | Likely internal-heavy; unverified | Lab confirmation and intake-gate consequence | Founder + lab counterpart | pending | — |
| C4 | Initial sales execution | Product choice closed; offer and cadence unwritten | Written offer, named buyer, target-account list, outreach cadence, demo/CTA, and first-contact date | Founder | pending | — |
| X1 | YC application and demo-vs-halt collision | Open | Submit/defer decision + honest post-pivot demo scope | Founder | pending | — |
| X2 | Entity, IP process, and initial client agreement | Open | Entity/client-agreement decision; IP questions handled through the appropriate external process | Founder | pending | — |
| X3 | Consent/IRB, offboarding, and incident ownership | Open | Lab-approved handling/consent path + deletion/offboarding/support protocol | Founder + lab counterpart | pending | — |
| X4 | Founder-unavailable operations | Open | Credential/restart inventory + pause/notify protocol a second operator can follow | Founder + operator | pending | — |
| X5 | Confirm-leg interaction shape and responder operation | Propose/confirm proven only while the hand-run responder is up; founder wants confirmation "natural, not a chore" | Interaction-shape decision (responder-as-service, MCP confirm path, or batch gesture) or explicit deferral naming the first-client workaround | Founder + strategist | pending | — |

`C4` begins immediately. G1 gates sharing client/repo/live-data material, not outreach; C4 does not wait for G2 and never functions as a demand-validation gate. Before G2, every `pending` value above must change to exactly `resolved`, `accepted-risk`, or `deferred-with-owner-and-trigger`, with the final column pointing to the evidence or trigger.
