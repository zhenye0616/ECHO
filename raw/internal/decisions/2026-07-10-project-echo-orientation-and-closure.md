...
`C4` begins at its recorded trigger and never functions as a demand-validation gate. The founder-approved terminal package is recorded in `2026-07-12-g2-terminal-dispositions-and-repository-topology.md`: 27 rows total, 11 resolved, 16 deferred with owner and trigger, zero accepted-risk, and zero pending. This completed register still requires independent mechanical verification at an exact landed SHA and a separate founder halt-lift.
