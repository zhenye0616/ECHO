...
*Snapshots as of survey time (before `0ab0af05` landed): where a summary mentions 132/133 as inbox-parked, read it through the withdrawal. The quadrant entries below are corrected; these summaries are preserved verbatim as the surveys' raw verdicts.*
...
- **backlog & work-in-flight** — Pipeline is fully drained: proposed/ready/claimed/pending_review are all empty; the only work-in-flight is 3 inbox-parked items — 132/133 (client carve, review-converged, gated on post-Jul-24 demo) and a zombie duplicate of shipped item 081. The live risk surface is the followup queue: ~20 undispositioned post-Jul-7 bullets, several of which gate the client loop directly (130 producer go-live opt-in, prod draft-store fixture pollution, RC3 current-run filtering residual, brief target-miss diagnostics, a test that decays vacuous ~Jul 30). Doc drift: docs/BACKLOG.md is 9 items stale and structurally blind to inbox/; wiki promotions for 124-131 (including the two client-facing surfaces, brief + changeset compiler) never happened; inbox convention itself is undocumented in backlog/README.md.
...
Register Part 4 + commit 0ab0af05: no new build specs, no carve, no box until the founder judges the unknowns map complete — this document is that map. Standing decisions that survive the 132/133 withdrawal: the product name and lab/product boundary concept; shared single-schema db with source-filtered instances; retrieval-less product brain mode (A1); dedicated founder-run Mac box (B2); founder accounts in client workspaces (B3); macOS-only phase 1 (B4); tarball deploys, no repo on the box (T4); org repo reserved for phase-2 clean split; client scope = meeting→brief only; promote-to-product graduation mechanism (skill retained, dormant). Part 4 also enumerates the map-filling agenda; the cross-check at the end of this document maps every agenda line to its entry.
...
### Deploy model locked (tarball → dedicated box); carve specs 132/133 review-converged, then WITHDRAWN
*deploy · client-relevance high*

Deploy = npm-pack tagged tarballs to a dedicated founder-controlled Mac box (prod = loop-of-record); repo separation explicitly rejected; data flows prod→dev only, code dev→prod only. Specs 132/133 reached r3 proceed/proceed with zero findings, then were WITHDRAWN and deleted at 0ab0af05 (numbers freed for reuse). The carve survives as a standing decision (register Part 4); spec text and review arcs are recoverable in git history (specs at 95a6b581; convergence at 18f72f89 / 71647084). src/product/ does not exist.

Evidence: raw/internal/decisions/2026-07-10-product-carve-unknowns-register.md Part 2 (T4, B2) + Part 4 standing-decisions list · commit 0ab0af05 (withdraw + clarity halt) · absence of src/product and of backlog/inbox 132/133 files (verified at HEAD)
...
### Four register unknowns resolved + API-key brain binding decided (2026-07-10)
*strategy · client-relevance high*

Founder resolved A1 = retrieval-less product mode, B2 = dedicated box, B3 = founder's accounts/bots invited into lab workspaces, B4 = macOS-only phase 1 (Windows tester off critical path). Same day: product brain binds via ANTHROPIC_API_KEY — minimal client deployment = node + tarball + GRANOLA_API_KEY + ANTHROPIC_API_KEY + launchd unit; founder's box may keep CLI auth short-term. All four survive the 132/133 withdrawal as standing decisions.
...
A 36-agent default-refute stress test confirmed 30/30 findings against code, consolidated to 12 failure classes (#1 argv-less brief silently briefs the PREVIOUS meeting; #4 owner mis-stamping verified in live prod rows), with every guard explicitly allocated to item 131 (now complete), a future calendar-trigger item, or the human pilot-onboarding checklist.
...
**Action:** Real production data exists on the founder box for demo and for prod→dev data copies; the legal-note flattening is the known quality edge.
...
**Action:** A drained pipeline is the ideal moment for repo reorg — nothing in flight can conflict; delete the 081 zombie as a first trivial cleanup.
...
**Action:** Bites install-day/first-run — the register calls it the top unmitigated trap. Halt-compatible resolution this sprint: a written DECISION on the cutoff semantics (mirror item 128's intake pattern) queued as the first post-halt build item; verify behavior on the box-day dry run.
...
**Action:** Bites install and every upgrade/config edit on the unattended box. Resolves: fail-loud + surfaced-health item (echoctl doctor line or heartbeat status) specced from the trap map; verify on the box-day dry run.
...
**Action:** Bites first-run of the card leg and pollutes the loop-of-record today. Resolves: disposition these six bullets in the next strategist sweep — purge the fixture record + fix the offending test immediately; the rest become one small 130-hardening item before the advisor meeting.

### Confirm leg: no MCP path, hand-run responder, confirm-after-edit gap, shape still an open design question
*product-loop · client-relevance medium*

Confirmation dies whenever the manually-started local Socket Mode responder is down (a founder-authorized confirm had no sanctioned execution path — journaled red); card 8f40ef6c and 5 EchoBrain Legal drafts remain pending; the founder wants confirmation 'natural, not a chore' (ride the meeting wrap-up, one batch gesture) and tier-2/3 of the 130 live test plus the pre-meeting checklist (confirm-after-edit fix, responder up, Linear creds, triage stamps) are deferred to the advisor meeting.
...
**Action:** Bites steady-state (every ratification). Resolves: founder design call on confirm-leg shape + responder-as-daemon or MCP confirm path; near-term, execute the pre-advisor-meeting checklist.
...
**Action:** Bites first-run for the lab (their meetings are mostly internal). Resolves: short-term default/document the bypass in the onboarding doc; medium-term spec content-class routing.
...
**Action:** Bites steady-state, silently. Resolves: pre-install check of the client's Granola transcript setting (onboarding doc line) + surface the drop-warning in brief target-miss diagnostics.
...
**Action:** Bites install (proxy, path default) and steady-state (language, long meetings, heartbeat). Resolves: triage each into fix-now / onboarding-doc caveat / accepted-risk during the followup disposition sweep — the open question is which bucket, and today none has even a bucket.
...
**Action:** Bites steady-state on the unattended box. Resolves: box checklist covers TZ+local-disk for the pilot; the checkpoint lockfile and a network-path runtime guard should become small items before the box runs both daemon and manual brief invocations concurrently.
...
**Action:** Bites pilot contracting (lab/IRB will ask) and compounds with every customer-adjacent commit. Resolves: write the one-page data-handling/retention story this sprint; run the history scan + decide on filter-repo before sharing the repo link with the lab.

### Burned-buyer and WTP screens owed before any adapter is built — no record either was run
*strategy · client-relevance high*

The recap-pilot decision explicitly requires two screens before Zoom/Mattermost work: the lab's named burned-buyer incident and academia WTP (identify the actual payment instrument); no document records either screen passing.

Evidence: raw/internal/decisions/2026-07-07-office-hours-org-recap-pilot.md Addendum 5 'Screens owed' · memory project_recap_pilot_decision

**Action:** Bites strategy — sequencing gate on all adapter build work. Resolves: run both screens at/before the lab advisor meeting and record the answers in a decision doc; if they fail, the adapter roadmap changes.

### B1/B5/B6: rollout calendar, pricing + rename, and phase-1 definition of done are all undecided
*strategy · client-relevance high*

No rollout calendar exists against lab semester/grant deadlines (B1); $25/mo per-seat was priced for a different persona and the ECHO rename deadline now covers a customer-visible module name (B5); phase-1 has no definition of done — the register itself warns that left unspecified 'the carve becomes the goal instead of the customer' (B6).

Evidence: raw/internal/decisions/2026-07-10-product-carve-unknowns-register.md B1/B5/B6 + 'still open' line 50

**Action:** Bites strategy now (B6 shapes what the sprint optimizes for) and contracting later. Resolves: founder decisions; B6 first — define the WTP-equivalent of the V1 '3/5 ask to pay' before the sprint's client-prep work is scoped.
...
Checking candidates against the ledger + execution state is the named missing stage and the uniquely-ECHO claim; the intake classifier currently proposes with zero retrievals, triage was strategist-concierge in the one live cycle (n=1), and spec-writing is deliberately gated on 2-3 more meetings.

Evidence: raw/internal/decisions/2026-07-09-decision-loop-canonical-model.md stage 2 + 'Immediate application'

**Action:** Bites steady-state/scale (concierge doesn't scale past the founder). Resolves by design: run the next 2-3 advisor meetings, log triage patterns, then spec — the sprint's job is only to keep the concierge notes disciplined.
...
**Action:** Bites sprint start (the sweep IS clarity work) and Jul 30 (test decay). Resolves: run the disposition sweep first thing; fix the fixture clock (injectable-clock audit) before Jul 30.
...
**Action:** Bites future install (Windows tester) and any all-green CI aspiration. Resolves: next Windows-chain item (EBUSY) + CI-only repro investigation; the epic itself stays parked unless the Windows tester re-activates.
...
**Action:** Bites every test run this sprint (flaky gates corrupt the cleanup sprint's own verification signal). Resolves: one-shot bootout now + small teardown item.
...
### A5: multi-tenant concierge topology undefined before customer #2
*strategy · client-relevance low*

Second box vs second ECHO_HOME vs shared db is explicitly undecided; the pid lock only serializes one ECHO_HOME, so nothing today prevents cross-tenant confusion the day a second customer exists.

Evidence: raw/internal/decisions/2026-07-10-product-carve-unknowns-register.md A5

**Action:** Bites at customer #2, not this sprint. Resolves: one decision doc when customer #2 is real; the register correctly sequences it after the pilot.
...
**Action:** Bites at halt-lift. Resolves: the clarity sprint's reorg keeps a committed move log written for the re-spec to consume, and never touches src/.

### T-series residue: eight tracked unknowns with no owner (T1, T2, T5, T7–T11)
*deploy · client-relevance high*

Product source allowlist (T1), sidecar classification (T2), remote-write path (T5), cross-instance row identity (T7), embeddings-on-prod (T8), backup rule (T9), migration ordering against a populated db (T10), Granola quota check (T11) — all named in register Part 4's agenda, none owned by any artifact. T9 (backup/restore of the loop-of-record db) and T11 (Granola quota) have direct client-box impact; earlier drafts of this map misfiled the backup and migration fragments under UU — they are tracked KUs.

Evidence: register Part 4 map-filling agenda (T-series line)

**Action:** Bites at box setup and first upgrade. Resolves: one analysis/decision note per T-item during the clarity sprint; T9 and T11 first.

### Legal/entity readiness: no entity, no pilot agreement, and the YC application asks the hard questions
*strategy · client-relevance high*

No entity/incorporation or pilot-agreement artifact exists anywhere in the repo (grep-verified) despite an imminent university pilot needing a counterparty, and the YC application (due Jul 24) asks IP/prior-employer questions directly while an employment transition is in flight (office-hours 07-07 line 152 is the only written mention). The public repo timestamps the full development history, which raises the priority of the filter-repo/redaction work.
...
**Action:** Bites at the YC application (Jul 24) and pilot contracting. Resolves: founder settles the IP/employment question within the in-flight process BEFORE the application; decides entity form; drafts a one-page free-pilot letter this sprint.
...
Personal-scope API keys are plan-gated with the vendor's own docs inconsistent (Business vs Enterprise); the T11 quota check has never run; nobody has priced Granola seats for the lab or decided who pays — and the pilot doc demotes Granola to 'reference competitor, not the substrate' for a lab whose actual tools are Zoom + Mattermost. The interim pilot substrate is an unpriced, unverified vendor dependency.
...
**Action:** Bites pilot contracting. Resolves: verify plan gating against the founder's actual plan, run T11, and fold 'who pays for Granola seats — or does the pilot gate on the Zoom adapter instead' into the B5 economics decision.

## 3. Unknown knowns — implicit knowledge and drift

*The UK quadrant is dominated by one structural fact: the product pivoted twice (org-alignment 07-01, lab-pilot meeting->brief pin 07-09/10) but every orientation surface an agent or human reads first — CLAUDE.md, v1-spec, NORTH_STAR, AGENTS.md, the entire wiki architecture/capture layer, and all install docs — still describes the April dev-context-layer product. Meanwhile the shipped client loop (Granola poller, signals, decision store, brief) exists only in raw/ decisions and completed backlog items, and its operational knowledge (concierge folklore, brain CLI/security facts, content-freeze semantics, plist wipe) lives in founder memory and one trap-map inventory. A second cluster is self-referential tooling whose own maintenance contracts are silently broken (journal-cat, sync-skills check, architecture-map, BACKLOG.md generator). These are the sprint's cheapest wins: almost every entry resolves with a doc edit, a banner, or the one owed wiki-promotion pass.*

### CLAUDE.md 'V1 Scope Reminder' instructs agents toward the dead April product
*strategy · client-relevance high*

The project operates on the meeting->brief lab-pilot scope, but the highest-authority operating file every session loads still pins indie AI builders / Cursor+CC+GitHub+Slack bundle / $25/mo / browser extension+hotkey overlay and explicitly lists 'meeting transcripts, Zoom' as Cut from V1 — the exact current product. CLAUDE.md is an operating-model file that by its own rules must update immediately.

Evidence: CLAUDE.md:83 '- **Cut from V1:** Email, Linear, Notion, meeting transcripts, Zoom, calendar' (verified this run) · CLAUDE.md 'V1 Scope Reminder (Tape Above Desk)' full block vs 2026-07-10 client scope pin (commit c2ea91cd) · Flagged independently by 5 of 8 domain surveys — the single most-duplicated finding

**Action:** Rewrite the 'V1 Scope Reminder' block in /Users/zhenye/Desktop/Project_echo/CLAUDE.md to the pinned client scope (university lab, meeting->brief loop, Zoom+Mattermost owed adapters, macOS tarball box); operating-model files have no shipping gate, so this needs no validation milestone.

### wiki/product/v1-spec.md is a mandatory builder read that specs the superseded customer
*strategy · client-relevance high*

docs/AGENT_INSTRUCTIONS.md makes v1-spec a mandatory per-run read as 'Locked V1 spec', and the page opens 'Locked 2026-04-30. Treat as source of truth' with indie-AI-builders, $25/mo, and non-goals 'Meeting transcripts (Granola/Otter) — V2' and 'Zoom direct — never' — structurally orienting every fresh agent on the opposite of the current product, with no supersession banner despite the 07-01 reframe formally superseding the wedge.

Evidence: wiki/product/v1-spec.md:12 'Locked 2026-04-30. Treat as source of truth' (verified this run) · docs/AGENT_INSTRUCTIONS.md:20 mandatory-read table · raw/internal/decisions/2026-07-01-org-alignment-reframe.md 'What this supersedes'

**Action:** Add a dated supersession banner at the top of wiki/product/v1-spec.md pointing to the org-alignment reframe + client scope pin (banner is a correction, not an aspirational-spec violation), and fix the AGENT_INSTRUCTIONS mandatory-read table to point at the current scope anchor.
...
**Action:** Update the Current Sprint Focus block in docs/NORTH_STAR.md to the cleanup/clarity sprint + lab-pilot readiness; it is the natural home for the sprint's 'no new features' rule.

### AGENTS.md hands every fresh Codex session a dead strategy gate
*coordination · client-relevance high*

Codex's entry doc (last commit 2026-06-08) presents 'friction-first prioritization / zero new architecture until e1+e2' as the CURRENT operating gate, a month stale versus the YC-demo sprint and the client-facing pivot.

Evidence: AGENTS.md 'Current operating gate' section; git log -1 = 2026-06-08

**Action:** Update AGENTS.md's operating-gate section in the same pass as CLAUDE.md/NORTH_STAR — the three orientation docs should name the same sprint.

### The shipped client product has zero wiki presence: 124-131 promotion pass never ran
*product-loop · client-relevance high*

Per the lagging-doc rule, shipped capabilities get wiki pages after landing in complete/ — but the last promotion commit is dd4e3cb8 (2026-07-06, items 116-123). The Granola poller, intake bridge, decision store, responder, post-meeting brief (131), and changeset compiler (130) — the entire client deliverable — exist only in raw/ decisions and completed backlog items; wiki/capture/ documents only dev-tool extractors.
...
The project brands itself 'we make every AI smarter about you' via cross-tool context, but the 123 live trace shows the intake classifier makes ZERO ECHO MCP retrievals (capture_status=zero_retrievals on all fresh cards) — the shipped client loop uses meeting signals only, and no spec or wiki page states this. Decision A1 (retrieval-less product mode) was resolved 07-10 but only in raw/ decisions.
...
**Action:** State 'client loop v0 is meeting-signals-only; cross-tool retrieval is the moat layer, not yet wired into intake' in the client scope anchor + the new post-meeting-brief wiki page, so pitch and product claims stop diverging.
...
**Action:** Document the freeze semantic in the client onboarding doc ('edit the note before ECHO polls it, never after') and in the new Granola-capture wiki page; give the re-ingest follow-up an owning followups bullet or inbox item.
...
Every must-fix from the trap map, register, and stress test (signals first-run cutoff, API-key brain, --wait, item 134 deploy spec, onboarding doc, brief target-miss copy) has zero specced backlog items — proposed/ready/claimed/pending_review are all empty (verified). The trap map even asserts the API-key brain binding is 'already queued', which is false, and several fixes are code changes colliding with the sprint's 'no new features' framing.
...
**Action:** Document the brain prerequisite + permission posture in the client install doc and the post-meeting-brief wiki page; the API-key-brain register item is the eventual code fix, but the dependency must be written down for the pilot box now.
...
The only written prices anywhere are $25/mo (dead indie persona) and the recurring-with-opt-out / $3-6k-duct-tape-anchor / WTP-upfront playbook inside the CANCELLED Justinian pilot doc. The actual customer is a university lab whose default is free-forever academia; register B5 names the gap but no document works it.
...
**Action:** One decisions doc: lab-pilot pricing posture (free-pilot terms, what converts to paid, WTP screen timing per the burned-buyer memory) — even 'deliberately free until X' written down beats folklore.
...
**Action:** Create the tracking artifact now (followups bullet or inbox item) distinguishing three separate jobs: git-history secret scan, filter-repo content rewrite, and the already-done db token scan — so 'we scanned' stops meaning the wrong thing.
...
**Action:** Write a one-page committed-content policy (docs/ or CLAUDE.md section): name classes allowed/forbidden in the public tree, a redaction rule for journal Returned fields, and pilot-participant naming rules — then apply it in the cleanup sweep.

### Third-party PII/consent is not a trap class anywhere
*exposure · client-relevance high*

The trap map's 'Secrets & config' class covers operational absence-behavior only; no doc treats third-party-human PII or recording consent as a risk class, even though the pilot ingests an advisor's and PhD students' meeting content, recording-consent culture is named 'the hard part' in the pilot doc, and real first names from live captures already sit in a committed test fixture.
...
**Action:** Bump to 0.1.1 (or 0.2.0) and backfill a CHANGELOG entry before cutting the pilot tarball; fold into item 134's deploy spec.
...
### Self-maintaining tooling contracts are silently broken
*hygiene · client-relevance low*

Several tools/docs that declare their own maintenance guarantees currently violate them: journal-cat.sh — the documented canonical read for validation evidence — hard-fails on 2026-07 (codex shard missing its '## Interactions' marker; reproduced this run); tools/sync-skills.sh --check passes despite .claude/commands/office-hours.md having no canonical skills/ counterpart (check is one-directional); docs/architecture-map self-declares 'edit in the same change wave' but omits the 07-10 brief command; the deprecated get_recent_work_context MCP tool remains registered ~2 months past its own inline removal date; and src/index.ts is an empty export{} stub with src/reasoning/causal.ts having no production importer.
...
**Action:** Detection: an access-discovery checklist executed WITH the lab (Mattermost version/admin/bot rights, Zoom plan + transcript export path, OAuth review timeline) plus a one-day post-to-Mattermost spike against their self-hosted instance — before any adapter spec is written.

### Unwritten client-journey stages: consent/IRB, offboarding/deletion, pilot-kill
*strategy · client-relevance high*

The journey stages before and after 'loop runs' have no written thinking: third parties in transcripts never consented to ECHO processing at a UNIVERSITY where IRB may apply; the only deletion story is dev 'rm -rf ~/.echo' against an append-only store with no in-place modify; and pilot-failure/kill-criteria/customer-#2 were never re-derived after the Justinian→lab switch — surprises hide here because these stages have simply never been reached, so nothing forces the questions.
...
**Action:** Detection: three cheap probes before the first real lab meeting — (1) ask the lab the IRB/data-policy question and add a consent line to the pilot agreement, (2) run a deletion drill on a scratch box and write the offboarding runbook from what breaks, (3) re-run the offer/kill-criterion design against the lab and name candidate customer #2 in writing.
...
**Action:** Detection: one-shot gitleaks/trufflehog over `git log --all` history + a content-class grep sweep of backlog/reviews/ and agent-runs/ + a `gh api` audit of repo security settings and release assets — all before any pilot-lab data enters the pipeline.
...
**Action:** Detection/prevention: decide a redaction rule or private-shard convention for client-derived journal/decision entries BEFORE the next pilot meeting, and install a pre-push hook + CI check failing on secret filename/content patterns.

### Deploy/upgrade/rollback path decided but never executed once
*deploy · client-relevance high*

The entire prod-box path chosen on 07-10 — tarball install, com.echo.product.daemon launchd unit, migration against a populated db, rollback-to-previous-tarball, prod→dev data copy — has zero execution history (no box exists), and the 'artifacts sent off-repo' variant of the same class is already live: the root runbook has silently diverged from docs/, and a stale Jun-2 echoctl-0.1.0.tgz sits at root while package version is 0.1.0-beta.1; the non-macOS branch of this path (win32/linux daemon no-ops, ~120-failure Windows EPIC) is a deferred sub-zone that silently constrains customers #2/#3.

Evidence: diff -q echoctl-agent-onboard-runbook.md docs/echoctl-agent-onboard-runbook.md → differ; root tgz mtime Jun 2 (re-verified this run) · product-carve-unknowns-register.md Part 2 T4 + B2 resolution (decided 2026-07-10, no execution record) · 2026-07-10-client-machine-trap-map.md (12 install/first-run trap classes, all static analysis) · src/cli/commands/daemon.ts:702,1120 off-darwin no-op paths; backlog/_followups.md R5 Windows EPIC
...
### No non-founder human has ever graded or acted on ECHO output
*product-loop · client-relevance high*

All quality and demand verdicts are founder-self-graded — the advisor received the 07-09 brief by hand-paste but no reception signal was captured, no WTP question has been asked, and n for 'someone else installed/used/valued it' is still zero — so the class of surprises about how briefs actually land (tone, length, wrongness a recipient spots but the author can't) is completely dark.

Evidence: raw/internal/decisions/2026-07-09-first-advisor-loop-cycle.md (verdict 'implicit', no recipient feedback captured) · memory project_n1_concierge_install_gate (untested floor is n=0) · recap-pilot decision mandates burned-buyer + WTP screens before build — not yet run against the lab
...
Concierge stages 2 and 5, the hand-started Socket Mode responder, the founder-personal Granola key and CLI auth, the founder-controlled prod box, founder accounts inside client workspaces, and n=0 non-founder installs all run through one human — who is simultaneously in an employment transition, a YC application crunch, and sole pilot operation. The map covers unattended-BOX telemetry but nothing asks what happens to a live loop-of-record pilot during a founder-unavailable week: where credentials live, who can restart anything, what the customer is told.

Evidence: first-advisor-loop-cycle.md (concierge stages) · register A3/B2/B3 · memory project_n1_concierge_install_gate (n=0)

**Action:** Detection: a one-page ops-continuity note inside the owed box runbook (credential inventory + storage location, restart steps a non-founder could follow, pilot pause/notify protocol) plus a support-expectation line in the pilot agreement.
