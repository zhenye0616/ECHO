...
| **B** | **Git-history secret scan** | Credential *strings* across all reachable textual patches plus separately enumerated binary/archive blob history | semantic content (names/quotes/IDs); existing clones; steganography/encrypted unsupported archives | **DONE on `maint/clarity-phase2` through `8e0e591f`, pending merge + independent rerun.** Gitleaks 8.30.1 text scan: 4,497 commits / ~28.56 MB, zero findings. Binary pass: 7 paths / 8 unique blobs / 2,765,149 raw bytes; archive and 228,124-byte printable-string scans clean. Exact evidence and limitations: `2026-07-11-phase2-exposure-baseline.md`. |
