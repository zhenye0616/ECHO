...
- **104 — Skill genericization.** `process-backlog`, `review-queue-*`, `merge-and-cleanup` embed Project_echo paths and the `~/Desktop/Project_echo--<slug>` worktree convention; make them repo-relative / config-driven so onboarded repos' agents use them unmodified.
