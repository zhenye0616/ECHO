...
| FOUNDER LIVE | Versioned candidate package built from a pinned SHA on the founder's Mac; isolated live namespace; real meetings and real failure recovery | Predeclared live-test plan, run log, output verdicts, known failure modes, recovery result | Founder-live candidate; not QUALIFIED or client-facing |
| QUALIFIED | Qualification change under the product boundary; build-once versioned package; client runtime configuration; clean qualification environments | Every release-qualification cell green; independent review; artifact manifest and checksum; exact-artifact founder staging smoke | Qualified release candidate; not client-enabled |
...
The next maturity event is therefore a pinned, versioned candidate-package run in an isolated environment on the founder's Mac. Passing that run advances the candidate to FOUNDER LIVE. It does not reopen the founder's decision that pain and demand are proven.
...
- **Next:** package the current candidate and run it on the founder's Mac with its own state, port, service, credentials, logs, and rollback. Nothing in the founder's existing ECHO environment should be at risk.
...
1. **Separate state.** Dev, founder-live, qualification, and client environments use distinct `ECHO_HOME`, database, port, launchd label, logs, credentials, and checkpoints. No stage reads another stage's mutable state by default.
2. **Separate composition roots.** Dev may boot the full lab. Founder-live boots the candidate plus explicit observability. Qualification and client stages boot only the Team-product composition root and its allowlisted dependencies.
3. **Separate runtime configurations.** Dev, founder-live, qualification, and client-live are runtime lanes, not install-profile values. The current `dogfood | customer` profile only filters coordination assets; reserve `profile` for that legacy audience split. Productization must introduce explicit runtime/composition configuration and must not overload the retired Machine-context meaning of `customer`.
...
5. **No repo on live or client machines.** FOUNDER LIVE, QUALIFIED, and CLIENT LIVE use versioned packages. No live stage requires a checkout.
...
### DEV -> FOUNDER LIVE: permit a founder live test

The candidate must be a versioned package built from a pinned SHA, with an isolated live-test configuration, a rollback command, a predeclared test plan, and no known risk of corrupting the founder's existing live state. Running a checkout or uncommitted working-tree code against live data does not count.

### FOUNDER LIVE -> QUALIFIED: open a graduation change
...
|---|---|---|
| Product source boundary | `src/product/` imports only approved kernel/product dependencies; no lab daemon, dev extractors, MCP/Fleet orchestration, or founder paths | Missing: `src/product/` and its fence do not exist |
| Product tests | Hermetic wedge tests live under `tests/product/`; no network, live credentials, founder database, or wall clock | Missing: `tests/product/` does not exist; current `test:product` is the broad non-orchestration suite |
...
| Artifact provenance and authority | Reviewed main-ancestor SHA, FOUNDER LIVE evidence, one stable aggregate required check, zero unexpected selftest skips, one build, SHA-256, dependency-lock hash, CI run, matrix result, monotonic version/tag, matching changelog, and founder authorization are bound together | Reusable build/checksum base exists; tags can currently publish without main ancestry, source-quality checks, or FOUNDER LIVE evidence |
...
This decision defines the pipeline but does not implement it or lift G2. Before G2, work may refine the qualification contract and prepare the isolated founder-live plan. After G2, the first productization proposal must establish the product composition root, runtime configurations, fence, `tests/product/`, build-once artifact flow, and machine-readable evidence records before individual capabilities can graduate.
