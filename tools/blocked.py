...
Input shape (parsed from each item's YAML frontmatter):
    ...
    status:      proposed | ready | claimed | pending_review | complete
...
Selection rule:
    ...
    proposed/, pending_review/, or claimed/ do NOT satisfy a blocked_by
    dependency — only complete/ does. This is the safety gate: founder review
    must promote an item to complete/ before its dependents become claimable.
...
def normalized_content_sha(text: str) -> str:
    ...
    excluded = CONTENT_MARKER_FIELDS | AGENT_MANAGED_FIELDS
    normalized = {
        "frontmatter": {
            key: fm[key]
...
def load_items(repo_root: Path) -> dict[str, dict[str, Any]]:
    """Walk all backlog stages and collect every item by id."""
...
def candidates(items: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    in complete/), and 'ready_block' (ready-content reason or None).
    """
    ...
    for iid, it in items.items():
        ...
        ready_ok, ready_block = ready_content_satisfied(it)
        out.append(
            {
                **it,
                "unblocked": len(unsatisfied) == 0 and ready_ok,
...
def format_block_reasons(candidate: dict[str, Any]) -> str:
    ...
    if candidate.get("ready_block"):
        reasons.append(candidate["ready_block"])
    return "; ".join(reasons)
