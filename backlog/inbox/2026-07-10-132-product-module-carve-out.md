...
files_to_modify:
  ...
  # MOVE AUTHORIZATION: every move listed in AC1 authorizes BOTH sides — deletion of the old path and
  # creation of the new path. The old-side paths are enumerated here so the allowlist matches the diff:
  - src/capture/surfaces/granola-poller.ts           # MOVE SOURCE (deleted by git mv per AC1)
  - src/enrich/granola-signals.ts                    # MOVE SOURCE
  - src/enrich/post-meeting-brief.ts                 # MOVE SOURCE
  - src/enrich/granola-intake-candidates.ts          # MOVE SOURCE
  - src/enrich/granola-intake-seed-store.ts          # MOVE SOURCE
  - src/surfaces/ceo-slack-responder/**              # MOVE SOURCE (whole dir)
  - src/cli/commands/brief.ts                        # MOVE SOURCE
  - tests/capture/granola-poller.test.ts             # MOVE SOURCE (test mirror)
  - tests/enrich/granola-signals.test.ts             # MOVE SOURCE
  - tests/enrich/post-meeting-brief.test.ts          # MOVE SOURCE
  - tests/enrich/granola-intake-candidates.test.ts   # MOVE SOURCE
  - tests/enrich/granola-intake-card-atom.test.ts    # MOVE SOURCE
  - tests/enrich/granola-intake-cutoff-clock.test.ts # MOVE SOURCE
  - tests/enrich/granola-intake-seed-store.test.ts   # MOVE SOURCE
  - tests/cli/brief-command.test.ts                  # MOVE SOURCE
  - tests/surfaces/**                                # MOVE SOURCE (ceo-slack-* + decision-changeset + responder subtree; builder enumerates exact set at claim)
  - tests/daemon/granola-intake-schedule.test.ts     # MOVE SOURCE
...
**RESOLVED (founder decision, 2026-07-10 — was the A1 OPEN block):** the product loop has a *runtime* (non-import) dependency on the full daemon's MCP server — brain children receive `ECHO_MCP_URL` (default `http://127.0.0.1:38478/mcp`, `src/brain/brain.ts`) for retrieval + retrieval-correlation, which `echoctl product daemon` does not serve. Founder chose **option (b): explicit retrieval-less product mode** (codex-ops r3 pre-approved this as AC2/AC3-compatible: "option (b) remains compatible if AC2 is amended to name retrieval-less mode explicitly"). AC2 is amended accordingly (retrieval-less clause). Option (a) — bundling a minimal MCP — is deferred to the self-install phase and would require its own spec + review. The A2 cold-db quality test (unknowns register) remains the empirical check on what this mode costs.
...
- **AC2 (module composition root):** NEW `src/product/daemon.ts` starts ONLY: storage open + granola poller + signals worker + intake bridge + decision responder. No fs/git watchers, no dev-tool extractors, no MCP server, no coord. Exposed as `echoctl product daemon`. It acquires the **same pid lock** (`resolveDataDir()` conventions) as the full daemon, so the two cannot run concurrently against one db — attempting to start it while `com.echo.daemon` runs fails loud with a message naming the conflict. `echoctl product brief` is added; top-level `echoctl brief` remains and produces byte-identical output (demo path unbroken). The full `echoctl daemon` behavior is unchanged — same workers, now imported from new paths. Smoke test: product daemon starts and stops cleanly against a scratch ECHO_HOME, and the registered worker set is exactly these FOUR workers — granola poller, signals worker, intake bridge, decision responder — with storage open/close asserted as lifecycle setup/teardown, not counted as a worker. **Pid-lock conflict test (pinned):** a second unattended test holds the daemon pid lock under the scratch ECHO_HOME (simulating a running `com.echo.daemon`), invokes `echoctl product daemon`, and asserts non-zero exit plus a conflict message naming `com.echo.daemon` — the no-concurrent-daemons invariant must be regression-tested, not just implemented. **Sanitized test environment (pinned):** the smoke test scrubs/overrides all external credentials and endpoints (Granola key, Slack tokens, Linear key, brain invocation, `ECHO_MCP_URL`) so no live integration can be reached from an unattended host; external workers run mocked or disabled; zero network side effects; shutdown is bounded by a kill timeout so CI cannot hang. **Retrieval-less product mode (pinned — A1 resolution, founder decision 2026-07-10):** brain children launched under the product daemon run WITHOUT MCP retrieval — the product composition root explicitly disables/omits retrieval wiring rather than inheriting a dead default URL; the run record's `capture_status` semantics stay honest (a deliberately retrieval-less run must be distinguishable from a broken capture — it must NOT be recorded as an ordinary `zero_retrievals` produced by a failed connection). No MCP endpoint is bundled; that remains a possible self-install-phase item.
