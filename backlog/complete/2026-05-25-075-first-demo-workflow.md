...
- ANY `workflowsResult.results[i].action === 'source-missing'` where `results[i].workflow` is named in the effective `defaultWorkflows` list (a default workflow that the spec promises to ship is missing from `workflowsSourceDir`). User-edited workflows (`action: 'user-modified'`) do NOT fail the rollup — the user's edit is preserved per AC2.2 step 5. **Design choice acknowledged (r4 codex-ops F1 MED disposition):** the spec deliberately does NOT validate (parse via `loadWorkflow`) the preserved user-modified target file before declaring `overallOk: true`. Two reasons: (a) validation would couple `syncAll`'s sync-time contract to 074's loader, which is a layering violation (`adapter-sync` does not import from `src/cli/workflow/`); (b) user-edits are the user's responsibility — corrupt/stale/symlinked preserved files are surfaced at `echoctl run` time (loadWorkflow throws) and via `echoctl doctor` (074's read-only health check, extended for workflows in a 076 follow-up per After Completion). The risk envelope is small: the only way to reach this state is a user explicitly editing `~/.echo/workflows/change-review.toml` AND breaking its TOML schema; the recovery (`rm ~/.echo/workflows/change-review.toml && echoctl init`) is one command.
...
- `Workflow.steps[0].prompt` ALSO contains a reference to ECHO MCP via `prompt.includes('mcp__echo__')` — this pins that ECHO MCP tools are named in the prompt at all. The MCP **best-effort semantic** (AC1.3 step 6: continue with repo-local context if MCP unavailable / errors / times out) is intentionally NOT pinned by a regex test (r3 MCP regex REMOVED per r4 codex-ops F2 + 058 disposition discipline). Reason: the regex /best.effort/i added in r3 was too weak (r4 codex-ops correctly observed it doesn't pin the failure-fallback chain); strengthening it to match `/fail|unavailable|timeout|down|error/i` near `/continue|local|repo/i` would be deeper prose-pinning that brittle-locks the prompt against natural iteration. The AC1.3 step 6 PROSE is the spec contract (the load-bearing requirement); a builder who deletes the best-effort instruction would fail r-N review on a future spec revision, not a unit test. This is the same removal-over-patching discipline that 058 names and that this spec applied at r2 (AC9 packed-install correctness claim) and r3 (echo-owned-skill marker in 074). Prompt prose iterates; AC body is the contract.
...
- **`echoctl doctor` for workflow-loadability surface (r4 codex-ops F1 disposition — 076 follow-up).** AC3.5 deliberately does NOT validate user-modified `~/.echo/workflows/<name>.toml` files at sync time (layering: `adapter-sync` does not import 074's `loadWorkflow`). The right surface for "this preserved file is corrupt / unloadable" is `echoctl doctor`'s read-only health check: extend it to call `loadWorkflow` on each `~/.echo/workflows/*.toml`, report which are unloadable + the parse error, and recommend `rm <path> && echoctl init` as the recovery. That's a 074-extension follow-up, NOT 075 scope. File entry in `backlog/_followups.md`.
