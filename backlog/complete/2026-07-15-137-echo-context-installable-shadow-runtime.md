...
Every serve path requires absolute `--config` and `--log-dir` arguments. Before parsing configuration, loading native modules, reading secrets, or touching the writer lock/SQLite, the runtime validates the current-user 0700 log directory and opens its 0600 non-link bounded startup sink. Every later pre-listener fatal path writes one typed credential-redacted record there. If that first open fails for any reason, the process exits 0 to suppress launchd retry; `status`/`doctor` report `startup_logging_unavailable` only while independent path checks still prove the failure and otherwise report honest `startup_outcome_unknown`, never an invented historical cause. After the sink opens, deterministic config/secret/permission failures write a typed record and exit 0; lock/contention and other enumerated transient failures write a typed record and exit 75; unexpected crashes remain nonzero. The launchd-equivalent test matrix proves persistent and cleared/transient logger-open failures, every later fatal mapping, and no silent 10-second loop with stdout/stderr at `/dev/null`.
...
Final assembly begins with empty npm and Node caches. It may fetch only (a) official `https://nodejs.org/dist/v22.22.1/node-v22.22.1-darwin-x64.tar.gz`, required to match SHA-256 `07b13722d558790fca20bb1ecf61bde24b7a4863111f7be77fc57251a407359a`, and (b) the exact HTTPS `resolved` URLs in the reviewed `package-lock.json`, each verified by its lockfile `integrity`; redirects or Git/GitHub/unlocked URLs fail. Fetches use `/usr/bin/curl --proto '=https' --tlsv1.2 --location --fail --show-error` or npm's integrity-verifying fetch under a deny-by-default URL allowlist. The candidate evidence commits the exact `xcode-select` path plus `xcrun clang`, `make`, Python, Node ABI, npm, and environment allowlist tuple; final build requires that reviewed tuple, runs `npm ci --ignore-scripts`, and explicitly rebuilds only the manifest-enumerated native packages from source with `npm_config_nodedir` pointing at the verified Node distribution. No prebuilt-addon, cache, host-Node, or unreviewed compiler fallback is permitted.

The tgz bundles that Node distribution's license/notices, the package-lock-pinned `better-sqlite3` and writer-lock closure, generic capture adapters, the manifest-bound synthetic fixture, schemas, CLI, and runtime provenance. The runtime manifest records the dependency URL+integrity closure, toolchain tuple, Node ABI, Mach-O architecture, and every native `.node` member's path/mode/SHA-256 plus `otool -L` and rpath closure; the bundled Node must load each addon in a build-time probe. The bundle excludes source/tests/dev tools/cache, credentials/state/config/logs, lifecycle rebuild scripts, absolute repository paths, Project_echo/siblings, and workflow/coordination code. A bounded architecture preflight rejects an incompatible Intel/Apple-Silicon/Rosetta state before real-path mutation; the staged bundled Node is probed before config, secret, plist, or service mutation.

No completed bundle is tagged, uploaded, downloaded as a release asset, published, or named a release; the closed Node+lockfile dependency acquisition above is build input, not a release surface. There is no release FSM, approval-file challenge, remote asset reconciliation, or private prerelease. If the final build is interrupted or ambiguous, the item stops for coordinator reconciliation; no automatic rebuild, adoption, alternate output, or hidden second artifact is permitted.

### AC4 — Install with one layout resolver, one lifecycle lock, and launchd owning the runtime directly

Add CLI commands `install`, `start`, `stop`, `restart`, `status --json`, `doctor --json`, `uninstall`, `internal serve --config --log-dir --ready-fd`, and `internal seed-fixture --config --fixture-id`, plus one closed layout resolver for support, release, state, log, secret, plist, label, port, and GUI-domain values. There is deliberately no `disable` command and no invocation of `launchctl enable|disable`; preflight refuses a pre-existing disabled override for the label rather than taking ownership of persistent GUI-domain state. Refuse root/sudo, LaunchDaemons, symlinked/traversing paths, foreign ownership, and collisions. Candidate mode requires `--candidate-root <absolute-root>`, a run-unique `com.echo.context.candidate.<id>` label, port `0`, and `--no-launchd`; every filesystem path stays below that root. `internal serve` writes exactly one schema-valid `{pid,start_time,port,generation,artifact_sha256}` record plus LF to the inherited ready FD only after loopback bind, and never writes it to stdout/stderr or a shared path. The harness spawns it as a detached process group, owns that pipe, and uses the returned kernel-selected port. The GUI domain is not claimed to be below a filesystem root.

Every mutating lifecycle command and offline fixture seed takes `flock(2) LOCK_EX` on one current-user 0600 regular-file `lifecycle.lock` before recovery/state reads and holds it through filesystem, SQLite, and launchd convergence. `status` and `doctor` take `LOCK_SH`; when no owned support root/lock exists they return not-installed without creating either. Under the shared lock they capture launchd PID/start time plus authenticated readiness `{pid,start_time,generation,artifact_sha256}` before probes, repeat both after probes, and accept a verdict only when the identities are equal and match the listener/writer-lock evidence. A change discards all observations and retries up to three times within five seconds; contention or continued instability emits one schema-valid `busy:true` timeout result and exits 4 rather than mixing incarnations. This lock is distinct from the runtime writer lock. The one permitted pre-intent bootstrap is creating or validating the exact 0700 support root plus empty lock file; a crash there is recoverable only when the root is empty and owner/mode/type match. Before any other real-path mutation, install durably writes an intent enumerating every release/config/secret/log/plist/state path, stages verified bytes, atomically commits the immutable release and ownership receipt—including the exact runtime-manifest hash—and only then loads the job. Rerun verifies and adopts, rolls back only enumerated owned paths, or refuses. Uninstall bootouts first, removes only receipt-owned install bytes, and preserves secret and shadow state.
...
For `U=$(/usr/bin/id -u)`, domain `gui/$U`, and service `gui/$U/com.echo.context`, the only mutating vectors are `/bin/launchctl bootstrap gui/$U <absolute-plist>` and `/bin/launchctl bootout gui/$U/com.echo.context`; `/bin/launchctl print` and `print-disabled` are read-only probes. Source-owned exit mapping treats print-not-found and bootout-not-found as already absent and every other nonzero as typed failure. `start` is a healthy no-op only for a loaded job whose exact plist/PID/listener/artifact identities match; any loaded stopped/unhealthy job is booted out to job/PID/listener absence before bootstrap. `stop` bootouts and waits for that same absence. `restart` bootouts if present, waits for absence, then bootstraps and waits for matching readiness. Uninstall bootouts to absence before byte removal. No command changes persistent disabled overrides. Each transition has a fixed 30-second deadline.

In `--no-launchd` mode the harness is the sole process owner: start spawns one detached process group with private ready pipe; stop sends SIGTERM to the group, waits for bounded drain, then SIGKILLs survivors and proves PID/listener absence; restart completes that stop before a fresh group/pipe; uninstall performs stop before receipt-owned removal. No launchctl command runs. No stable user shim, purge, `prepare-final`, supervisor child, last-exit file, or competing restart mechanism exists.
...
The schemas report `busy`; observation attempts; beginning/ending process-generation identities; installed state; predecessor and successor source/runtime identities; version and SBOM/lock hashes; label/port/home; direct runtime PID/start time/executable realpath; launchd state; authenticated readiness; exact roster; capture/authority state; config/plist/secret ownership and modes; listener addresses; writer-lock state; startup-logging state (`healthy|unavailable|outcome_unknown`); effective log inventory/caps; repository dependency; and overall verdict. Doctor independently recomputes installed-byte, plist, process, listener, auth, roster, SQLite, capture, authority, permission, architecture, startup logging, bounded-log, and repo-independence truth under the generation sandwich. It never repairs, starts, stops, writes fixtures, or prints credentials. No supervisor, owner-sidecar, last-refusal, or last-exit fields exist.

### AC6 — Prove a clean candidate, authorize exact operations, and leave one healthy real shadow

`tools/shadow-install-smoke.mjs` performs the repo-free candidate lifecycle from the four protected bundle assets under a disposable root with inherited `ECHO_*`, Node, npm, and repository paths absent. It verifies the bundle, installs with `--no-launchd`, and runs `internal seed-fixture --config <absolute> --fixture-id synthetic-v1` while the service is stopped and under both lifecycle and writer locks. The seed command first verifies the ownership receipt's runtime-manifest hash, then resolves the fixture's path and SHA-256 from that immutable installed manifest; the CLI supplies no path/digest trust. It opens the manifest member with `O_NOFOLLOW`, verifies it before SQLite, and records the ID+digest transaction. An identical replay is a successful no-op; same ID/different digest or any unmanifested fixture is a typed refusal. There is no online write path. The smoke then starts the installed runtime in one owned foreground process group on port 0, authenticates, lists exactly eight tools, exercises that fixture, proves `POST /v1/capture` is 403 without body consumption, restarts, stops, uninstalls, proves state/secret preservation, terminates survivors in `finally`, proves the listener gone, and deletes only receipt-owned candidate paths. Before/after sentinels prove real Application Support, secrets, logs, LaunchAgents, labels, and ports 39478/38478/38479 unchanged.

The target implementation and Project coordination head are reviewed by an agent other than the builder. Before each target-main write and the real-shadow install, the persistent coordinator creates the separate immutable delegated-operation record required by the sequential-program decision, commits/pushes/reads it back from Project `origin/main`, and binds the exact reviewed spec/seal, landed repositories, typed artifacts, preflight/plan, before-image backup, uninstall/restore proof, authority boundary, identities, and nonce. Merge approval never substitutes for installation authorization.

Installation runs only through independently reviewed `tools/execute-authorized-install.mjs`, whose source hash and canonical target SHA are bound by that record. The runner opens every source asset `O_RDONLY|O_NOFOLLOW`, rejects non-regular/non-owned/link files, copies each once into a `mkstemp` file that is immediately unlinked, fsyncs and rewinds it, and hashes that unlinked snapshot against the read-back authorization. It keeps all snapshot descriptors open and invokes `/bin/sh /dev/fd/<bootstrap-fd>` with literal inherited `--authorized-manifest-sha256 <digest> --manifest-fd <n> --runtime-fd <n> --sbom-fd <n>` arguments. Bootstrap and installer consume only those descriptor streams—never the source or snapshot path—and verify the manifest plus remaining assets before real-path mutation. A source rename/in-place rewrite during copy yields an unauthorized snapshot hash and aborts; after unlink there is no pathname to swap or restore. Race tests cover rename, in-place rewrite, and swap-restore for every asset.

After final-bundle candidate smoke and installation authorization, install those same descriptor-protected bytes at label `com.echo.context`, port 39478, and `/Users/zhenye/.echo-context-shadow`; run the same stopped-service `--fixture-id synthetic-v1` seed command; prove stop/start/restart and healthy doctor; and leave the shadow installed with capture and authority false. Independent PID/lsof/hash/listener observations prove no open Project_echo, `~/.echo`, future `~/.echo-context`, or legacy Application Support path; Project_echo remains healthy at 38478; 38479 remains unused; clients/configs are unchanged; and no live state or capture worker is touched. The Project migration record binds both canonical landed SHAs, every artifact hash, authorization records, redacted evidence hash, and exact authority boundary.
...
- `tests/install/runtime-artifact.test.ts`, `bundled-node.test.ts`, and `bootstrap.test.ts` verify both source tuples, all four asset identities, Node and exact lockfile URL/integrity allowlists, reviewed toolchain tuple, empty-cache `npm ci --ignore-scripts` plus explicit native rebuild, substituted host/cache/toolchain rejection, ABI/native-member hashes, `otool`/rpath closure and load probes, descriptor-only protected execution, coherent substitution and rename/in-place/swap-restore races against the external authorization, architecture ordering, and tamper/unsafe-path rejection.
...
- `tests/install/launchd.test.ts` and `tests/cli/lifecycle.test.ts` assert the direct-runtime plist, the complete registered-job/PID/listener FSM using only bootstrap/bootout/print, idempotence, disabled-override refusal, restart/uninstall convergence, the separate no-launchd group FSM, `/dev/null`, mandatory log-dir, exact fatal exit/evidence mapping including persistent and cleared logger-open failure, bounded logs, exact ready-FD cleanup, and absence of enable/disable/kickstart/kill/supervisor/shim/purge/prepare-final surfaces.
- `tests/cli/doctor.test.ts` validates both schemas, absent-lock not-installed behavior, shared-lock plus beginning/end generation consistency, SIGKILL+KeepAlive race retries, canonical stdout/LF, combined-failure exit precedence, bounded probes, busy timeout, redacted stderr, honest startup-logging states, every required health mismatch, and read-only behavior.
...
- `tests/integration/shadow-install.test.ts` performs the candidate lifecycle with receipt+manifest-resolved stopped-service seed, identical replay no-op and mismatch/substitution negatives, ready-FD restart and failure cleanup, and real-path sentinels; the authorized operator smoke repeats lifecycle and independent observations on the founder shadow.
