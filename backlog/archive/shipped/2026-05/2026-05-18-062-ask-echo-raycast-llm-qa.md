...
- **One new daemon-side unit test for the ring buffer.** `tests/mcp/request-log.test.ts` covers: (a) append + read-back ordering; (b) `pending → ok` transition updates in place (same entry, not a duplicate); (c) `pending → error` likewise (BOTH via thrown exception AND via resolved `isError: true` envelope — round-2 arch reviewer); (d) cap at 1000 entries (oldest evicted); (e) `since`/`until`/`status` filters return correct subsets; (f) **pending-evicted-then-completes** case (round-2 ops reviewer): fill the ring past 1000 with a pending entry as the oldest, force eviction, then fire the completion callback — assert no throw, no entry resurrected, no other entry corrupted.
- **One new daemon-side integration test for the wrapper end-to-end (round-2 arch reviewer 2026-05-18, scope-corrected by both round-3 reviewers).** `tests/mcp/recent-calls-endpoint.test.ts` starts `startMcpServer`, then fires **one POST `/mcp` invocation per currently-registered tool** (`echo_ping`, `search_memories`, `find_clusters`, `get_atoms`, `get_atom`, and any other tool registered via `register*` at the time the test runs) with minimal safe inputs (empty/zero-result is fine — the test is about wrapper activation, not retrieval correctness). After each call, asserts (1) the ring buffer contains exactly one new entry for that tool name with the correct projection shape and `status: "ok"` (or `status: "error"` if the tool's empty-input path errors deterministically) and a finite `duration_ms`, (2) the POST `/mcp` response envelope is unchanged from pre-wrapper behavior. **The test enumerates registered tools at runtime** (e.g., reads the same registration list the production code uses, or asserts against a maintained whitelist that fails loudly when a new tool is registered) so a future tool added to the daemon without going through the wrapper is caught — this is the load-bearing coverage that the original "MUST prove every registered tool is logged" claim required (round-3 both reviewers flagged the prior single-tool test as an overclaim).
...
- **AC2** — Form view accepts a multi-line question; on submit, a Detail view is pushed and a subprocess is spawned with the active profile's resolved `(binary, args, stdin)`. **Subprocess lifecycle contract** (per codex-ops reviewer 2026-05-18):
  ...
  - Best-effort cleanup: an `unmount`/dismount handler in the Detail view kills the subprocess tree. **If Raycast itself crashes, the child is reparented (likely to launchd/init) and will continue running until it exits on its own or is manually killed** — the 5-minute ceiling and unmount cleanup do NOT run in that case. (Round-2 codex-ops reviewer 2026-05-18 corrected an earlier "OS will reap eventually" wording that was inaccurate.) README MUST document the manual cleanup command (`pkill -f '<binary>'` or `ps -ef | grep <binary>` → `kill <pid>`) as the founder's recourse.
  - **ANSI / terminal-control-sequence handling (round-3 codex-ops reviewer 2026-05-18).** The spawn `env` MUST include `NO_COLOR=1` AND `TERM=dumb` to discourage agents from emitting terminal formatting in the first place. Even with those, both `codex` and `claude` CLIs may still emit ANSI/control sequences in some paths; the runner MUST strip ANSI escape sequences (matching `/\x1b\[[0-9;]*[A-Za-z]/g` plus other common CSI/OSC patterns) from streamed stdout AND from the 4KB stderr tail BEFORE feeding either into `setMarkdown` or the non-zero footer. `agent-runner.test.ts` (extend existing test file) MUST cover at least: (a) stdout chunk with bold escape `\x1b[1mhello\x1b[0m` → renders as `hello`; (b) stderr tail with cursor-move sequences → cleaned tail in the footer; (c) chunk split mid-escape-sequence across two stdout reads is handled (don't render the partial half-escape).

- **AC3** — agent stdout streams to the Detail markdown state during the run via a **throttled flush** (codex-ops reviewer 2026-05-18): chunks are appended to a `useRef` buffer (after ANSI-stripping per AC2); `setMarkdown` is called at most once every 80 ms OR immediately on subprocess exit, whichever comes first. The buffer growth is unbounded for v0 (a 5-min ceiling × any plausible token rate keeps it well under 1 MB; no truncation needed). On subprocess exit, the `Detail.Metadata` sidebar populates from `GET /mcp/recent-calls?since=<launch_ts - 2000>&until=<exit_ts + 2000>` — the ±2 s padding (round-3 codex-ops reviewer 2026-05-18) absorbs wall-clock drift between Raycast's `Date.now()` and the daemon's `Date.now()` so a valid call near the run boundaries isn't filtered out. R4 already accepted cross-process contamination from concurrent ECHO consumers within the run window; the padding's contamination is bounded the same way. Sidebar rows render as `<tool name> · <duration_ms>ms · <status>`. **Sidebar data MUST come from `audit.ts` typed records only** — stdout-parsing for tool calls is forbidden (closes the model-authored-sidebar hole; codex arch reviewer 2026-05-18 promoted from OoS to AC).

- **AC4** — error paths handled (preflight + runtime):
  - **Preflight (BEFORE spawn):** (a) loopback probe `HEAD http://127.0.0.1:38478/mcp` with 1s timeout → **any HTTP response counts as reachable**, including 405 Method Not Allowed (which is the current daemon's intentional response to non-POST `/mcp`; checking `response.ok` would falsely report a live daemon as unreachable — both round-2 reviewers flagged this). Only a network error or timeout counts as unreachable → toast `"ECHO daemon unreachable at 38478"` + no spawn. (b) `which <binary>` (or equivalent existence check) → if missing, toast `"Agent '<binary>' not found"` with "Open Raycast preferences" action + no spawn.
...
- **AC5** — daemon exposes `GET /mcp/recent-calls?since=<unix_ms>[&until=<unix_ms>][&status=pending|ok|error]` on `http://127.0.0.1:38478` (NOT 38479 — that's the trace viewer), loopback-only (inherited from existing bind). Response:
  ...
  **Pending-eviction semantics (round-2 codex-ops reviewer 2026-05-18):** the cap-1000 ring-buffer evicts oldest entries when full. If a still-pending entry is evicted before its completion callback fires, the completion handler MUST no-op silently (NOT throw, NOT update the wrong index after shifting). The wrapper holds an opaque entry handle (e.g., a monotonic call-id), and the completion path looks up by id and short-circuits if absent. Tests MUST cover the "pending evicted then completes" case explicitly.

  **Status determination (round-2 codex arch reviewer 2026-05-18):** the wrapper MUST classify a call as `status: "error"` when EITHER the handler throws OR the handler resolves with an MCP envelope where `isError === true`. Catching only thrown exceptions would mislabel error-envelope returns as `ok` — many MCP tool handlers return error envelopes rather than throw. The wrapper returns the original envelope unchanged to the JSON-RPC caller; only the ring-buffer status is affected. Per redaction allowlist, `result_shape` for error cases is `{ is_error: true, content_text_length: <number> }` — not the raw error text.

  **Wrapper insertion point (round-2 codex-ops reviewer 2026-05-18):** the spec ALLOWS EITHER (a) refactoring every existing `server.registerTool(...)` call site to go through a `registerTool(server, ...)` helper, OR (b) instrumenting the per-request `McpServer` instance via `instrumentMcpServer(mcp, projections)` BEFORE the existing `register*` calls run (monkeypatching `mcp.registerTool` on that instance only). Approach (b) reduces blast radius for v0 by leaving existing tool modules untouched. The builder picks; whichever path is chosen, AC6's daemon integration test MUST prove that every registered tool is logged (catches missed registrations) and projected deterministically (catches projection regressions).

- **AC6** — `agent-profiles.test.ts` + `system-prompt.test.ts` + `audit.test.ts` (new per Tests section) pass via `npx vitest run` from `tools/raycast-echo`; daemon-side `tests/mcp/request-log.test.ts` + `tests/mcp/recent-calls-endpoint.test.ts` (the wrapper end-to-end integration test, round-2 arch reviewer addition) pass via the existing root test runner.
