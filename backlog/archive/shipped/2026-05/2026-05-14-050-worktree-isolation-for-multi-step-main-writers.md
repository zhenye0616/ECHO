...
- **Per-tick lifecycle (in this exact order):**
  ...
  4. **Route the child reviewer process into `$WT` explicitly** — set ALL FOUR handoffs: `cd "$WT"` in the wrapper, `export ECHO_REVIEW_QUEUE_REPO_ROOT="$WT"` so the prompt body's mandatory Step 1 (`cd "${ECHO_REVIEW_QUEUE_REPO_ROOT:-$HOME/Desktop/Project_echo}"`) lands in the worktree, set `PROMPT="$WT/.claude/commands/${SLASH_COMMAND}.md"` so the reviewer prompt bytes are read from the worktree copy (so R3-disposition prompt fixes like the same-reviewer-overlap guard and journal-before-cleanup ordering actually reach Codex's stdin), AND invoke the child Codex with `codex exec -C "$WT" --sandbox danger-full-access - < "$PROMPT"`. Without ALL FOUR — CWD + env + prompt-path + `-C` — the child escapes back to the live checkout OR reads stale prompt bytes from the founder checkout (R4 codex F1). Convergent finding lineage: R1 codex F1 + codex-ops F3 surfaced the first three; R4 codex F1 surfaced the prompt-path source-of-truth as the missing fourth.
...
- **Possible followup specs (file under `backlog/_followups.md`, not auto-spec'd):**
  ...
  - **050-followup-G — Race-tight same-reviewer launchd-overlap guard** (R4 codex-ops F1). The R3 disposition added a pre-commit re-fetch+no-op-exit check in AC1 step 5, but codex-ops R4 surfaced that two launchd ticks can both pass that check (re-fetch happens before either has pushed) and then collide in `push-with-retry.sh`'s rebase loop — which is path-unaware and just retries, producing a noisy failed tick on the loser. The full fix requires either (a) making `push-with-retry.sh` path-aware (env var `ECHO_PUSH_RETRY_ABORT_IF_PRESENT=<path>` makes it `git ls-tree origin/main <path>` after each rebase and exit 0 if non-empty), OR (b) adding a post-rebase, pre-commit-retry re-check inside the reviewer prompt loop. 050 ships with the R3 pre-commit re-check only — it narrows the race window but doesn't close it; the loser produces a noisy failed tick which the next launchd cadence re-fires cleanly. Filed as followup-G with an explicit AC for same-reviewer-overlap in AC6.
