...
verification waived; rationale: Founder-resolved r8 divergence (both reviewers agreed on fix; severity-only divergence). Fix is text-only narrowing of AC6.4 step 2 and AC8.12(d) — not load-bearing for runtime correctness. AC6.7 per-id Promise chain remains the actual concurrency primitive. No verification needed.
