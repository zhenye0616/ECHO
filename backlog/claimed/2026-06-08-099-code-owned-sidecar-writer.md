...
ready_content_sha: 3b3c6ec526796ea0eb79b1e2f11f8e2e0bb14c47f5f4c3bfa13656a3b51574ab
...
files_to_modify:
  ...
  - tools/review-queue/test-validate-sidecar.sh    # MODIFY (builder-found, was incorrectly in spec_refs) — its producer loop currently asserts ALL THREE producer values validate (line ~57: `for PRODUCER in claude-code-subagent codex-child review-pending-orchestrator`). AC4 collapses the enum, so this MUST be updated: assert only `review-pending-orchestrator` validates, and that the two retired values (`claude-code-subagent`, `codex-child`) now FAIL validation — proving AC4's collapse end-to-end through both the validator CLI and the shared helper. All other assertions in this test (malformed reject, heading checks, merge-and-cleanup round-trip) stay and must still pass.
spec_refs:
...
- **AC3 — single validation implementation.** `tools/review-queue/_sidecar_validate.py` is import-safe (underscore-named) and exposes a pure validation entry point; both `validate-sidecar.py` and `emit-sidecar.py` import it. `validate-sidecar.py`'s existing CLI contract (argv shape, exit codes, stderr messages) is unchanged. Its existing tests still pass, with ONE intended change: `test-validate-sidecar.sh`'s producer-enumeration assertion is updated to match AC4's enum collapse (see files_to_modify) — only `review-pending-orchestrator` validates; the two retired values now fail. All other assertions in that test are unchanged.
...
- **AC7 — tests.** `tools/review-queue/test-emit-sidecar.sh` is a standalone executable shell test run as `bash tools/review-queue/test-emit-sidecar.sh` (exit 0 = pass), mirroring how `test-validate-sidecar.sh` is run — there is no aggregate runner.
  - **Isolation (codex-ops r2 #3 + codex r3 #3):** because the writer derives its target from `git rev-parse --show-toplevel`, **every** case that creates a canonical sidecar or commits a fixture runs inside a **disposable temp git repo/worktree** (init temp repo, run with cwd set there so toplevel resolves to it) with a **cleanup trap** — no case may `git add`, commit, or leave dirty `pending_review/` artifacts in the caller's checkout, index, or working tree, even on failure.
  - **Writer cases:** valid descriptor → written + passes `validate-sidecar.py`; input with a non-constant producer (or a `target_path`/other forbidden key) → reject (no write); missing required field → reject (no write); existing target without `--replace` → reject (existing file untouched); with `--replace` → overwrite.
  - **Concurrent-create coverage (codex r3 #2):** the existing-target-reject case IS the race coverage — `os.link` no-clobber is atomic (the existence check and the write are one syscall; a target appearing at any time before finalize makes `os.link` raise `FileExistsError`), so there is no separate check-then-act window to test. No flaky two-process race and no monkeypatch seam are required; do NOT add a separate "appears mid-write" case.
  - **AC5 gate cases:** `check-coupled-invariants.sh` passes with an empty `pending_review/`, and fails — printing the offending path — when an invalid committed `*.review.md` is present (run in the disposable repo per Isolation above).
