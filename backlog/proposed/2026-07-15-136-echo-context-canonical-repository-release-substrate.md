...
Publication is staged and never adopts or clobbers state. Before creating the draft, the job constructs a canonical ownership marker containing repository ID, workflow run ID and run attempt, source SHA/tree, version, and approved tuple hash; the draft body carries that marker from its creation request. All three deterministic asset names belong to that marked draft. The annotated-tag message carries the same marker and tuple hash, and its tag-object bytes/SHA are constructed deterministically before the tag ref is pushed. After a lost or ambiguous response at draft creation, each asset upload, annotated-tag ref push, or release-publish update, the job makes no repeat write until API listing/readback finds exactly one same-run marker and exact expected name, source, bytes, and digest. Zero matches, multiple matches, or any metadata/byte/hash mismatch stops for founder disposition; the run log is evidence, never attribution authority. On a failure after any external write, the job best-effort deletes only its exactly marked same-run draft/assets/tag where the API permits; surviving or uncertain state remains blocking under the empty-namespace rule. Before declaring success, the job reads back the annotated tag object/message/source and all three published asset bytes/hashes against the approved tuple. The marker is server metadata, not a fourth release asset, and run attempt is execution provenance rather than a new founder-approved content field. Publication is executed by the guarded workflow under founder approval, never by a builder identity directly.
...
- tests/governance/workflow-policy.test.ts parses all workflows and rejects mutable actions, excess permissions, unpinned Node/npm, Project_echo checkout, founder-local paths, live-state access, and rebuild-on-publish; it also asserts the build/publish job separation (unprivileged build-artifact; approval-gated publish-release behind the source-release protected environment), the `refs/heads/main` ref/SHA dispatch guards, the workflow-level `cancel-in-progress:false` policy with pre-start pending-run replacement versus post-start partial-state handling, the inner/outer tuple-record layering (pre-upload seven-field inner record inside the workflow artifact; post-upload artifact-ID and workflow-artifact-digest job outputs) with artifact-ID-bound download and digest verification, and bootstrap/committed secret-scan command equivalence. Namespace fixtures prove a fully paginated empty namespace is required and that any tag/draft/published release/asset, attribution claim, authentication/listing/pagination failure, or incomplete result blocks. Lost-response fixtures after draft creation, each of the three asset uploads, annotated-tag ref push, and publish update resume only on one exact same-run server marker plus byte/hash/source match; zero, ambiguous, or mismatched readback blocks without repeating the write. Dispatch-input fixtures prove a wrong expected lock hash or wrong expected manifest hash fails build-artifact, and concurrency fixtures prove a newer dispatch may replace only a pending run that never started a job or external write while the running job remains protected.
