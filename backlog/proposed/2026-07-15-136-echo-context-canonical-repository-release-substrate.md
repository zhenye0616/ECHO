...
Commit the exact standalone tsconfig.json and eslint.config.mjs configurations used in the accepted item-135 private-clone review. Add explicit vitest.ci.config.ts and vitest.operator.config.ts. package.json defines the exact scripts the gate, CI, and the fresh-clone run all share: `typecheck`, `lint`, `test:ci` (vitest run --config vitest.ci.config.ts), `test:operator` (vitest run --config vitest.operator.config.ts), `verify:inventory`, `verify:authority`, `verify:artifact`, and `scan:secrets` (tools/secret-scan.sh). The scripted fresh-clone acceptance run is tools/fresh-clone-acceptance.sh, whose command surface is limited to these named scripts plus git fsck and which has an explicit argument contract with two modes and no optional skips: `--mode=source --source-sha <sha>` (source-only acceptance — builds the artifact locally from the checked-out committed objects via tools/build-source-artifact.mjs so `verify:artifact` always has a defined input) and `--mode=release --source-sha <sha> --version <version> --archive <path> --checksum <path> --manifest <path>` (post-release acceptance — verifies the three downloaded AC5 assets instead of building). A missing or extra argument for the selected mode fails with usage text rather than skipping a check; `test:operator` is never invoked by either mode. Move only the raw Project_echo Git-object recomputation portions of tests/migration/parity-matrix.test.ts and tests/migration/context-tool-evidence.test.ts into tests/operator/source-parity.test.ts; the operator suite requires an explicit ECHO_SOURCE_GIT_DIR and pinned source SHA and never supplies a founder-path default.
...
source-release.yml is dispatched manually (workflow_dispatch) with explicit inputs — source SHA, version, and expected lock/manifest hashes — and must execute the reviewed workflow definition: the run fails unless `github.ref` is `refs/heads/main`, `github.sha` equals the source-SHA input, and both equal the canonical main HEAD re-read through the hosting API at run start. The source-release protected environment's deployment-branch policy is restricted to main; that policy is read back through the API and recorded in the migration record. A concurrency group serializes release runs. The workflow's unprivileged build-artifact job (contents: read) checks out the landed SHA as a fresh detached clone, verifies extraction-baseline ancestry, builds the artifact exactly once, fails on any mismatch between the computed lock/manifest hashes and the dispatch-input expected values, and persists the three AC5 files as an immutable workflow artifact identified by run ID, artifact ID, and SHA-256 digest. Because a protected-environment approval does not itself encode what was approved, build-artifact writes the complete tuple (source SHA, tree, version, artifact SHA-256, lock hash, manifest hash, run ID, artifact ID) to the run summary and to a machine-readable tuple record persisted inside the same workflow artifact (a workflow-only record, not a fourth release asset), and exports every tuple field as job outputs. Founder release approval happens after this build, over exactly that presented tuple, and is granted only through the protected environment source-release, whose sole authorized reviewer is the founder identity zhenye0616.

The publish-release job runs only after that approval with the minimum write scope, downloads the persisted artifact by the exact artifact ID exported from build-artifact, re-hashes it and revalidates every tuple field against the approved record, and publishes those exact bytes without rebuilding. Immediately before its first external write it re-reads the repository ID, owner, private visibility, and canonical main HEAD through the API and fails unless the approved source SHA still equals main HEAD. Publication is staged: the job rejects any pre-existing tag, release, or asset of the same name and never clobbers; it uploads all three assets to an unpublished draft release and verifies each uploaded asset's hash against the approved tuple before the annotated tag (bound to the landed canonical source SHA) or the published release becomes visible. On a failure after any external write, the job deletes its own same-run draft and partial uploads where the API permits; any partial tag/draft/asset state that survives is recorded in the run log and stops subsequent release runs for founder disposition — a retry never silently dead-ends behind the no-clobber rule and never adopts partial state. Before declaring success the job reads back the annotated tag object and published asset hashes against the approved tuple. Publication is executed by the guarded workflow under founder approval, never by a builder identity directly.
