...
files_to_modify:
  ...
  - src/daemon/index.ts                             # AC1/AC2/AC6 — schedule the bridge worker (debounced, after signal extraction); ECHO_GRANOLA_INTAKE_* config parsing (enabled flag, lookback days, internal domains, owner map, default owner, ECHO_GRANOLA_INTAKE_CHANNEL_ID) + ECHO_SLACK_BOT_TOKEN reuse; enabled-but-misconfigured fails closed before claiming seed records
