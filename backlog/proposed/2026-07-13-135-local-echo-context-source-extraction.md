...
One assigned builder lane owns `/Users/zhenye/Desktop/echo-context`; sibling lanes never touch it. After the orchestrator verifies every parent component is a real non-symlink directory and the target is absent, the builder's first mutation is plain non-recursive `mkdir /Users/zhenye/Desktop/echo-context`, aborting on `EEXIST`; only that invocation may write inside. It initializes branch `migration/2026-07-13-135` with local identity `ECHO Migration Agent <migration@echo.local>`, signing/hooks/templates/global and system Git config disabled, and no remote. Every source/target Git command uses a minimal environment clearing repository/index/object/alternate/config/replace/graft/askpass/SSH/proxy injection variables and sets `GIT_NO_REPLACE_OBJECTS=1`; final checks prove all Git storage is target-local, no alternate/promisor/replace state exists, and `git fsck --full` passes. Source bytes come only from pinned commit objects.
...
Before target code is finalized, the builder exports the pinned source to scratch, installs from the pinned source lock in a source-only cache/root, launches the source MCP over stdio with all network denied, calls `tools/list`, and runs `/Users/zhenye/Desktop/echo-context/tests/fixtures/context-tool-parity.v1.json`. Sidecar `context-tool-parity.v1.sha256` hashes the manifest bytes, avoiding self-reference. Both files use UTF-8/LF; manifest objects use recursively sorted keys, arrays retain order, and aggregate framing is `case-id UTF8 + NUL + 64-lowercase-hex + LF` sorted by case ID. The manifest pins exact JSON-RPC request bytes, fresh per-case seed state, fixed clock/random/ID inputs/timeouts, and named volatile JSON pointers only.

The manifest has an explicit per-tool case matrix rather than universal caps: ping message/missing; MRU match/miss; clusters default/compact; atom found/miss; atoms duplicate/missing; recent bounded/truncated; search filter/pagination; wait new/timeout, plus schema-invalid where supported. Source descriptors are projected by exact eight IDs with missing/duplicate failure; target roster has exactly those IDs and no extras. Each case starts a fresh MCP child/state, consumes identical request bytes, and produces canonical per-case/aggregate hashes. A target semantic mutation with unchanged registry must fail.
...
After target HEAD is committed, every verifier treats the shared target read-only and creates a unique private `git clone --no-local --no-hardlinks` of the recorded HEAD. Source and target installs use separate roots/caches/manifests and `npm ci --ignore-scripts` under scratch HOME/XDG/npm config with auth/proxy variables removed, an active filesystem-denial sandbox, and registry network allowed only for lock-authorized fetch; path/link/workspace/Git resolutions are rejected. Required lifecycle/build commands are enumerated in runtime inventory and run later inside the same sandbox with network denied. Caches and node_modules are never shared.

Committed target profiles `tests/sandbox/stdio.sb.in` and `tests/sandbox/service.sb.in` plus `npm run verify:stdio-parity` and `npm run verify:service-parity` fail if enforcement is unavailable. Stdio parity denies all network and probes Project_echo/sibling/live-state/credential/external-write/nonloopback/loopback denials. Service parity permits only the run-owned scratch root and its already-listening server, which reports exact `127.0.0.1` or `::1` port through a dedicated readiness channel; unrelated loopback/nonloopback sentinel access must fail. A hostile HOME/npm/Git/PATH/source/sibling/live-daemon sentinel must not affect output.
...
Every failed stop first appends phase/command/exit, target HEAD/tree when available, and retained/archive/scratch paths to the Project_echo agent-run log/agent notes; it cleans only run-owned scratch and never adopts/deletes the target. After all checks pass, the builder writes `raw/internal/migrations/2026-07-13-135-echo-context.md` in its isolated Project_echo feature worktree. The record contains source SHA, operator-audit and build commands/exits, target path/branch/HEAD/tree, package/lock/provenance/evidence/parity/tool/runtime/dependency hashes, test/service results, no-remotes/clean status, differences, and `authority:false`, `live_state_migrated:false`. The builder commits that record with backlog handoff and does not mutate target history afterward.
