...
Source and target dependency acquisition use separate roots/caches/manifests. Every root has mode 0700 beneath the attempt; `HOME`, `XDG_CONFIG_HOME`, `TMPDIR`, `TMP`, and `TEMP` point there, empty 0600 npm user/global configs are explicit, and default temp paths must remain unchanged. `PATH` contains only `<attempt-root>/tool-bin`; that directory contains verified links for approved non-JS native tools, while every JavaScript CLI is invoked as `/usr/local/bin/node <absolute-js-entry>`. Each fetch invokes pinned `/usr/local/bin/node <absolute-npm-cli> ci --ignore-scripts --no-audit --no-fund` with explicit registry/cache/config flags under `env -i` and an active filesystem-denial sandbox. The network claim is deliberately endpoint-scoped: direct egress is allowed only to exact HTTPS registry hosts from lockfile resolved URLs, not package-authorized before TLS bytes arrive. Non-registry egress must be blocked before bytes; fetched content is admitted only after URL and integrity match the lock. Path/link/workspace/Git resolutions are rejected. A tampered or unlisted tarball from an allowed endpoint may be fetched but must fail admission and never enter the sealed cache.

The verifier writes `provenance/lifecycle-plan.v1.json` from locked package manifests. Each package is `no-script`, `verified-packaged-artifact`, or names one exact offline rebuild command, working directory, inputs, outputs, and tools. The current `better-sqlite3` row must either verify a lock/package-owned ABI-compatible `.node` artifact or run `/usr/local/bin/node /usr/local/Cellar/node@22/22.22.1_1/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js rebuild --release --directory <private-install>/node_modules/better-sqlite3 --nodedir <verified-node-headers>` under deny-all network. Only pinned `/bin/sh`, `/usr/bin/make`, `/usr/bin/clang`, `/usr/bin/clang++`, `/usr/bin/python3`, Node, npm's absolute node-gyp entry, and target-local JS entries are allowed, all by recorded version/hash. The sandbox permits writes only to the private install/build/temp roots. Source and target fetches verify lock integrities, seal distinct read-only cache seeds, delete node_modules, copy seeds to writable offline caches, and run `npm ci --offline --ignore-scripts --no-audit --no-fund`; lifecycle plans then run exactly. A clean-root rebuild repeats offline install plus lifecycle plan and tests the native load. Source and target caches, manifests, install roots, and `node_modules` are never shared.
...
A top-level finalizer covers every nonzero exit and catchable signal after attempt-root creation: export, dependency, lifecycle, audit, sandbox, parity, service, record generation, process stop, and handoff publication. It preserves the original status, terminates/reaps the owned process group, and publishes create-new `failures/<sequence>.json`. Command timeout is 900 seconds; TERM and KILL waits are 5 seconds each. Captured stdout and stderr are each capped at 1,048,576 bytes, sandbox diagnostics at 262,144 bytes, and the complete capsule at 2,621,440 bytes, with truncation flags and full-stream SHA-256.

Capsule publication opens a no-follow/O_EXCL mode-0600 same-directory temporary, writes and fsyncs bytes, atomically renames, fsyncs the failures directory, then reopens no-follow and verifies the hash before any cleanup. If publication fails, the original scratch and feature worktree remain; the finalizer emits both original and publication failures to stderr and the Project_echo agent-run log. Fault injections cover interruption before write, before fsync, before rename, after rename, and representative failures in export/install/audit/parity/service/record phases. Cleanup is limited to a unique orchestrator-created scratch root after canonical-path containment/no-symlink validation; prefix-collision, symlink-swap, and non-owned-path tests prove it cannot escape. It never adopts, repairs, or deletes the target.

After all checks pass, the builder writes `raw/internal/migrations/2026-07-13-135-echo-context.md` in its isolated Project_echo feature worktree. The record contains source SHA, operator-audit and build commands/exits, target path/branch/HEAD/tree, package/lock/provenance/evidence/parity/tool/runtime/dependency hashes, test/service results, no-remotes/clean status, differences, and `authority:false`, `live_state_migrated:false`. The builder commits that record with backlog handoff and pushes its feature branch. Push policy is initial attempt plus at most two retries after 5 and 20 seconds, only for transport timeout/reset or HTTP 5xx. Non-fast-forward, auth, dirty tree, or upstream divergence are terminal; automatic autostash, rebase, merge, force-push, and target-history mutation are forbidden. On failure the capsule records local commit/ref and observed remote OID, and the local worktree/commit remain. The target history is never mutated after the record commit.
