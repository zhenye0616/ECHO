...
files_to_modify:
  ...
  - src/echo-home/state-paths.ts                      # NEW: minimal local root/state resolver without profiles, projects, or review config
  - src/echo-home/paths.ts                            # preserve lab path behavior by composing the new minimal state-path primitive
  ...
  - product/npm-shrinkwrap.json                       # NEW: exact transitive runtime dependency tree included in the product tarball
  ...
  - tests/echo-home/paths.test.ts                     # prove the lab and minimal state-root resolvers stay behaviorally identical
...
The maximum initial internal allowlist is explicit: `src/product/**`; `src/capture/{granola-source-policy,pipeline-core}.ts`; `src/capture/surfaces/granola-poller.ts`; `src/enrich/{granola-signals,post-meeting-brief,worker-heartbeat}.ts`; `src/storage/{interface,sqlite,migrate,source-match}.ts`; `src/storage/migrations/**`; `src/echo-home/state-paths.ts`; `src/echo-home/adapters/atomic-write.ts`; `src/logging/index.ts`; `src/guards.ts`; and `src/util/{json,subject,timestamp}.ts`. The builder must reduce this list when the real closure is smaller and may not add another internal path without a spec-review amendment. A listed file is not automatically safe: the fence also rejects an import edge into any forbidden root.

Closure inventory comes first: before writing new product code, the builder runs the fence against this proposed allowlist and records the actual transitive closure in the agent-run log. If the real graph requires an internal edge outside the allowlist that no authorized seam in `files_to_modify` can absorb, the builder STOPS and moves the item to `pending_review/` with the missing edge named in `agent_notes`; silently widening the allowlist, or adding a dependency-seam refactor to a file not in `files_to_modify`, is a drift event. The allowlist may shrink without re-review; it may never grow without one.
...
`src/echo-home/state-paths.ts:1` owns only local root/state resolution needed by checkpoints and heartbeats. It contains no `customer|dogfood` install profile, project registration, skill/review paths, or reviewer defaults. The poller, signal worker, and heartbeat writer use this narrow primitive for their lab defaults while the product runtime injects explicit paths. `src/echo-home/paths.ts:1` composes the same primitive to preserve generic ECHO behavior, but it is not in the product allowlist or transitive closure. `tests/echo-home/paths.test.ts:1` proves both resolutions remain identical for the current lab environment.
...
`src/product/config.ts:1` loads and validates `schemas/product/runtime-config.v1.schema.json:1`. The schema requires `schema_version: 1`, an explicit `lane: "team-product"`, an absolute installation-local state directory with no `..` traversal, Granola workspace/input configuration, a secret *reference* for each credential, an explicit brain-adapter identifier, and `approval_mode: "manual"`. It rejects unknown fields, inline credential values, the retired `profile: dogfood|customer` vocabulary, relative/traversing state paths, and missing wedge dependencies. `validate-config` and `selftest` perform a read-only filesystem-type probe through an injectable `classifyStateFilesystem(path)` adapter returning `{kind: "local" | "network" | "unknown", raw}`. The production adapter realpath-resolves the closest existing ancestor of the configured state directory, matches it to the longest mount-point prefix in the output of `/sbin/mount` invoked by absolute path with `LC_ALL=C` and a bounded timeout, and normalizes the reported type through an explicit table: `nfs`, `smbfs`, `afpfs`, and WebDAV variants → `network` (fail closed); `apfs`/`hfs` → `local`; any other value, an unmatched mount point, or a probe/command failure → `unknown`, reported explicitly with the raw value and treated as fail-closed for `run`. Tests exercise the adapter with injected fixtures for every blocked, local, unknown, and command-failure result; no test shells out to the real `mount`. Logs, health records, checkpoints, manifests, drafts, and the database resolve underneath the configured product state directory; credentials never enter those records.

`src/product/runtime.ts:1` exports an injectable `startProductRuntime(config, dependencies)` and a deterministic shutdown handle. Startup is transactional: components start in declared order, and a failure after partial startup rolls back the already-started components in reverse order before the typed failure returns. Shutdown is idempotent and bounded by a deadline, after which any remaining handle is reported explicitly rather than leaked silently. The `run` command wires SIGINT/SIGTERM to that shutdown path and propagates a nonzero exit code on failure. It starts only the Granola meeting input, signal extraction dependency, local brief generation/manual approval surface, product-local state, and health. It injects explicit poll, signal-checkpoint, brief-output, database, and heartbeat paths under the configured state directory; `src/enrich/worker-heartbeat.ts:48-68` accepts the heartbeat destination instead of forcing the lab default. It does not import or start the generic daemon, MCP, Machine capture, developer extractors, decision-drift/intake workers, Slack/Linear, Fleet coordination, or remote delivery. Missing or invalid config and a missing production brain adapter return a typed nonzero `run` failure before polling, state mutation, or network access. `src/product/cli.ts:1` exposes product-only `validate-config`, `selftest`, and `run` commands over this root; it never delegates to generic `echoctl daemon`. Offline `selftest` succeeds without the rank-3 brain adapter while reporting that adapter/maturity cell as pending/unavailable; it must not pretend the real wedge ran.
...
`src/enrich/granola-signals-cli-adapter.ts:1` owns the existing lab-only brain parsing, preflight, prompt execution, timeout behavior, and a shared `createLabGranolaSignalOptions(env)` composition helper. `src/enrich/dispatch.ts:1-45` explicitly injects that adapter for the generic lab daemon. `src/cli/commands/brief.ts:139-147` uses the same helper when `BriefCommandOptions.extractFn` is absent, while retaining explicit fixture injection for tests. `tests/enrich/granola-signals.test.ts:1` proves the lab dispatch still selects the configured CLI brain, preserves timeout/error behavior, and does not silently switch adapters. A dispatch-level composition test additionally spies on `createLabGranolaSignalOptions(env)` at the `src/enrich/dispatch.ts` boundary and proves the existing environment→brain mapping, preflight wiring, prompt/parser options, timeout value, and absence of any fallback adapter are unchanged from current behavior. `tests/cli/brief-command.test.ts:1` adds a non-fixture-extractor path that proves real `echoctl brief` selects the configured lab adapter rather than failing closed. The product import-closure test proves `src/cli/commands/brief.ts`, `src/brain/brain.ts`, and `@anthropic-ai/claude-agent-sdk` do not enter the rank-1 product artifact accidentally.
...
All `tests/product/**` run with network disabled and with temporary state, injected clocks, injected extractor fixtures, and synthetic meeting content. They may not read the founder database, HOME credentials, live Granola, a live MCP server, or wall-clock time. `vitest.product.config.ts:1` registers a shared `setupFiles` guard for every product test file; it snapshots/clears relevant environment variables (including credential variables and `ECHO_*` overrides) and intercepts, in the Vitest worker process, `globalThis.fetch`, `net.Socket.prototype.connect`, `tls.connect`, and `http`/`https` request creation so an unexpected outbound attempt fails the offending file. Wall-clock discipline is enforced by injected-clock APIs in the product runtime plus a guard assertion that product modules under test never read the real clock directly. The in-worker guard cannot constrain spawned child processes; the child-process contract is separate and explicit: every child spawned by a product test receives a sanitized environment (credential variables absent, poisoned proxy, empty `NO_PROXY`, npm offline configuration), and a sentinel child test proves both that a child npm cache miss fails rather than fetching and that credential variables are absent inside the child. A guard test proves the shared setup is active in every file. `.github/workflows/ci.yml:1` runs `test:repo` under the existing quality matrix and additionally runs `test:product` unconditionally — no path filter — in the same quality job, so product coverage can never silently drop out of CI through an incomplete trigger-path list; neither job is called product qualification. The default `npm test` may continue to run the aggregate suite locally, including product tests; it is not a CI qualification command.

### AC5 — Product-only artifact is built once and is installable without the repo

`product/package.template.json:1` defines a private, unpublished `echo-brain` package with its own bin, Node engine, exact runtime dependency versions, and product README. `product/npm-shrinkwrap.json:1` pins the complete transitive runtime tree and is included in the tarball so target installs do not re-resolve ranges. The product package is not derived by renaming the generic `echoctl` tarball, and it contains no generic daemon, MCP, Machine capture, Fleet/review assets, wiki/backlog/raw content, test fixture, credential, or absolute repository path.

`tools/product/build-artifact.mjs:1` accepts an explicit valid prerelease version, source SHA, and a required `--out-dir`; verifies that `git rev-parse HEAD` equals the normalized supplied source SHA and that `git status --porcelain` — including untracked files — is empty, failing otherwise; builds inside a fresh temporary sibling directory and atomically renames it onto a previously absent final lineage directory, so a concurrent or retried build can never overwrite an existing lineage or mislabel one commit's bytes with another commit's SHA; runs the AC1 fence, stages only the compiled transitive product closure plus required package metadata/schema/migrations, verifies the shrinkwrap matches the template and root lock, and invokes `npm pack` exactly once per requested artifact lineage. It emits: one `.tgz`, one SHA-256 file, and one sorted artifact manifest containing package paths, sizes, per-file SHA-256 values, product-boundary version, source SHA, version, shrinkwrap/dependency-lock hash, and build command. The exactly-once rule binds the CI artifact uploaded for qualification: no downstream job may pack or rebuild it. A local `tests/product/packaged-product.test.ts` invocation may create its own isolated scratch lineage through this same tool when `ECHO_PRODUCT_ARTIFACT_DIR` is absent; in CI it must consume the already-built directory and must not pack.

The build job materializes a minimal content-addressed npm cache for the exact shrinkwrapped runtime tree after the normal root `npm ci`, records hashes for that cache in the qualification-support manifest, and performs no later dependency fetch. `tests/product/packaged-product.test.ts:1` installs the tarball into an empty temporary prefix, outside the repository, using only that cache with npm offline mode (`--offline --no-audit --no-fund`). Native modules use exactly one pinned strategy: source builds with `npm_config_build_from_source=true` plus bundled exact Node 22 headers referenced via `npm_config_nodedir` from the qualification-support directory (hashed in the support manifest), so neither a prebuild download nor a node-gyp header fetch is ever attempted; a toolchain preflight verifies the required compiler exists before installation begins. The child process receives a poisoned proxy, empty `NO_PROXY`, and npm offline settings; any cache miss, prebuild download attempt, or header fetch attempt fails the test rather than falling back online. The test verifies the artifact checksum before install, confirms the package file set matches the manifest, runs `echo-brain validate-config` and the offline `selftest`, and proves neither command resolves a repository-relative file. A local scratch run requires a cache prepared from the exact lock before `test:product`; it may not silently fetch during the test. The artifact is DEV evidence only; this item neither publishes it nor claims a monotonic public release version.
...
`.github/workflows/product-qualification.yml:1` triggers only on relevant `pull_request` paths, relevant pushes to `main`, and `workflow_dispatch`; it has no tag or release trigger and this item does not add it to branch protection. Path filters are a cost optimization only, not the coverage guarantee — AC4's unconditional `test:product` in `ci.yml` owns coverage. Its single build job checks out the exact commit it qualifies (`github.event.pull_request.head.sha` on pull requests, `github.sha` on push/dispatch — never the synthetic merge ref), passes that SHA to the builder, runs typecheck/lint, invokes AC5's artifact builder once (the builder's HEAD-equality check makes a checkout/SHA mismatch fail loudly), exports the output as `ECHO_PRODUCT_ARTIFACT_DIR`, then runs `test:product` and the boundary/manifest checks against that already-built lineage. The verified SHA propagates unchanged through manifests, reports, and artifact names. It uploads the tarball, checksum, artifact manifest, draft qualification report, and a qualification-support directory as one immutable CI artifact. The support directory is outside the tarball/manifest and contains only synthetic seed data, the hashed exact-lock npm cache, and checksum/evidence/aggregation scripts needed on a no-checkout runner. Every downstream job downloads those exact files, verifies the checksum and manifest before use, and is forbidden from checking out, packing, or rebuilding product bytes.

The initial target cell is macOS with Node 22, matching the phase-1 client contract; no Windows support claim is made. The macOS no-checkout job installs into a clean temporary prefix, runs the packaged offline selftest, exercises fresh and populated synthetic local state, and defines restart/state isolation as repeated packaged-CLI invocation over one persisted state root plus proof that a second prefix/state root cannot read it. It uploads a separate immutable evidence artifact. Every job and its child processes run under bounded timeouts; evidence-collection and upload steps run with `if: always()` so a failed install or selftest still yields a machine-readable evidence artifact. A final no-checkout aggregation job downloads the original bundle plus target evidence, composes a completed DEV draft report with the bundled support script, re-runs `validate-qualification.mjs`, and uploads a new immutable final-report artifact; it never mutates the original artifact. The aggregation job runs even when a dependency job failed (`if: always()` with explicit dependency-result inspection), recording failed, timed-out, or missing-evidence dependencies as explicit red cells while the workflow's overall conclusion remains failing. The workflow fails if any implemented rank-1 machine cell is red, if the tarball identity changes, or if an unexpected test is skipped. Pending later-rank and human-authority cells keep the report incomplete but do not make this foundation workflow dishonest or falsely QUALIFIED.
...
- The macOS Node 22 PID-lock/selftest race and Ubuntu Node 22 packaging-cleanup `ENOTEMPTY` race: owned by QA; if either class reproduces in the product-only qualification workflow, it is a blocking red cell rather than a retry-based waiver.
...
- `tests/product/runtime-isolation.test.ts`: only the wedge dependencies start; deterministic reverse shutdown; injected extractor works; absent production adapter fails before poll/state/network; forbidden services never start; startup failure injected after each component start rolls back the already-started components in reverse order without leaking timers or handles; SIGTERM during `run` shuts down and exits nonzero.
- `tests/product/hermeticity.test.ts`: in-worker guard blocks fetch/socket/http(s)/tls attempts and credential env access; sentinel child test proves sanitized child environment and offline npm failure on cache miss; filesystem-probe fixtures cover blocked, local, unknown, and command-failure classifications; synthetic fixture content only.
- `tests/product/packaged-product.test.ts`: one checksum-verified tarball installs outside the repo; manifest and file hashes match; offline config/selftest run; no prebuild download or node-gyp header fetch is attempted; forbidden paths and test fixtures absent.
...
- `tests/product/build-once-contract.test.ts`: workflow has one pack producer; downstream jobs consume the uploaded artifact and verify SHA-256 before install; no downstream checkout/build step; builder refuses a HEAD/source-SHA mismatch and a dirty or untracked-file tree; lineage publication is atomic-rename into a previously absent directory.
...
- `tests/echo-home/paths.test.ts`: the narrow state-path primitive matches current lab root/state behavior while the product closure excludes profiles/project/review configuration.
- `tests/enrich/granola-signals.test.ts`: lab CLI adapter injection preserves brain selection, timeout, error, and shutdown behavior while the shared core has no founder-CLI fallback; a dispatch-level composition spy on `createLabGranolaSignalOptions(env)` proves the environment→brain mapping, preflight, prompt/parser options, and timeout are unchanged.
