...
Use the item-137 prepare-final command to journal/rebind the unchanged context runtime from shadow to com.echo.context, final home, and 38478; never hand-edit installer-owned config/plist/state. Install the exact residual/rollback-full package as com.echo.project-residual on 38479 and replace/neutralize the old global echoctl package/start paths. Retire com.echo.daemon through the controller's journaled service-control step: bootout plus persistent disable, with its plist atomically relocated out of /Users/zhenye/Library/LaunchAgents into the protected snapshot area so that no login, KeepAlive respawn, or fresh launchd bootstrap can reload the retired job; a bootstrap probe must prove launchd can no longer load it before the generation commits. Make only the retained source-side context DB/WAL/SHM and old capture sidecars non-writable.
...
Prove historical retrieval, one uniquely tagged context append searchable by MCP, one real configured local-source capture, product-derived append, coord outbox/mirror visibility through wait/search, residual deadline status, clean restart, exact artifact/config/generation, no repository-open handles, and one context DB writer. Every enabled adapter—fs, git, Granola, Claude Code, Codex, and Cursor—must produce one live continuity observation during the drill/window or carry an explicit founder-approved disabled/no-activity adjudication whose rationale and healthy-checkpoint evidence stay in the protected local evidence area. Each observation or adjudication is recorded in the migration/evidence record as a fixed secret-free row whose complete closed field set is: adapter from the fixed six-adapter enum, plan-bound source slot index, UTC timestamp, generation, event count and atom count, and a health verdict drawn from exactly {observed, disabled-approved, no-activity-approved}. Committed rows carry no reason field: reason codes, adjudication rationale, and per-source checkpoint detail stay in the protected local evidence area and are never committed. Event and atom counts are non-negative integers counting only what that adapter captured within the row's own period (a per-row delta, never a cumulative total) and are zero on disabled-approved and no-activity-approved rows. A committed row containing any field outside this closed set, or any value outside these vocabularies and types, is invalid evidence and fails its gate. Committed rows never contain message bodies, file/document contents, titles, prompts, transcripts, token values, raw source names, paths, URLs, account/repository/document identifiers, checkpoint or cursor values, content-derived hashes, raw errors, or other free-form text. The same row schema and forbidden content classes govern AC10's seven-day evidence records. At this point Project_echo context is retained and non-authoritative pending acceptance, not frozen.
