...
Publish/cache the exact bytes under owned protected paths and independently download/reread them to reproduce hashes. No patch, npm install/rebuild, feature SHA, dirty byte, source-checkout runtime dependency, or alternate artifact is allowed. Dependency installation, compilation, and the Tests-section reruns of the item-137/138 suites all happen only inside the disposable detached build clones, against the exact candidate artifact bytes, before the artifact hashes are approved; founder approval is the last step and binds the hashes the suites passed against. After approval nothing rebuilds, reinstalls dependencies, reruns source suites, or mutates the artifacts — post-approval verification is artifact-level only: independently download/reread the published exact bytes and reproduce the approved hashes. The sole permitted post-approval installation is AC6's one-time live deployment of the approved artifacts through their prescribed artifact deployment entrypoints — the item-137 prepare-final command and the residual/rollback-full package's artifact install path — which lay down the already-built approved bytes verbatim: no lifecycle or build scripts, no dependency resolution, no byte mutation, followed by installed-byte hash readback reproducing the founder-approved hashes. A build/verification gap stops and escalates to the strategist, who creates the new proposed source item through the normal proposal flow; this item's workflow never writes repository artifacts outside its files_to_modify. Live hotfix is forbidden.

Before mutation, the controller emits a secret-free plan binding every artifact, source/final/shadow/residual/quarantine/backup path, actual source DB opened by the daemon, labels/ports, rosters, clients/installed adapters, capture workers/secret refs, source/coord cut lines, disk requirement, phases, and rollback/recutover policy. The plan includes a hash-bound source-slot inventory: for each adapter in the fixed six-adapter enum it materializes stable slots with explicit non-negative integer slot indices — including an explicit slot for every disabled adapter — and plan validation rejects a missing adapter, duplicate index, or unknown index. This inventory is the sole source from which the AC8 drill/window and AC10 daily expected evidence keys are mechanically derived. Founder approval binds the three exact artifact hashes and plan hash.
...
- Evidence-row gate checks reject extra or missing keys, unknown adapter/verdict values, plan-unknown or duplicate slot indices, malformed or non-UTC timestamps, author-supplied dates, string/fractional/negative counts, zero-count observed rows, rows bound to a non-active generation, and duplicate or plan-unknown daily matrix keys.
