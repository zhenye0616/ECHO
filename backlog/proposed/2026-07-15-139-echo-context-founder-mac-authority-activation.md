...
Publish/cache the exact bytes under owned protected paths and independently download/reread them to reproduce hashes. No patch, npm install/rebuild, feature SHA, dirty byte, source-checkout runtime dependency, or alternate artifact is allowed. Dependency installation and compilation happen only inside the disposable detached build clones before the artifact hashes are approved; after approval, every consumer — including the Tests-section reruns of the item-137/138 suites against the final artifact hashes — runs in disposable detached verification clones that consume the published exact bytes read-only and never rebuild, reinstall over, or mutate them. A build/verification gap stops and creates a new proposed source item; live hotfix is forbidden.
