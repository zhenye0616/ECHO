...
- **AC3 — doctor integration.** `echoctl doctor` resolves `tools/install-echo-codex-skills.sh` by an **absolute path from a stable anchor** (the repo root / module location — never cwd or a `PATH` lookup) and invokes it `--check` **`execFile`-style** (no shell-string, no `PATH` resolution), so an unattended/packaged doctor run from an arbitrary cwd never falsely degrades on script-not-found. It adds a structured `codexAdapter` field to `DoctorReport` with shape `{ status: 'ok' | 'drifted' | 'no-managed-install' | 'check-error'; staleSkills: string[]; remediationCommand?: string; detail?: string }`. Mapping (reusing doctor's existing non-fatal vocabulary, exactly like `syncLock.present`): `drifted` → `overall: 'degraded'` with `staleSkills` naming the skill(s) and `remediationCommand` = `tools/install-echo-codex-skills.sh`; `check-error` (installer genuinely missing/unrunnable, shell error) → also `degraded` — **graceful, never `broken`, never throws**; `ok` and `no-managed-install` → no contribution to `overall` (a human ok/skip line). `echoctl doctor --json` includes the `codexAdapter` field; `computeOverall` treats `codexAdapter.status` like the existing non-fatal degrade checks.
...
- **AC5 — tests.** `--check` is exercised in a **disposable temp `$HOME`/`~/.codex`** with a cleanup trap, never touching the operator's real `~/.codex`: clean install → exit 0; a hand-mutated installed `SKILL.md` → exit non-zero naming the skill; **a non-default `--namespace`/`--underscore-names` install with no drift → exit 0** (proves the sentinel-`skill_name` re-render reproduces byte-identically — no false drift on non-default installs); absent dir → exit 0; **the temp stage is removed on every exit path and nothing outside the temp stage is written (`~/.codex` is read-only)**. A doctor test asserts `degraded` + the stale-skill line when the check fails, ok/skip when it passes or there is no managed install (the shell-out may be stubbed), **and runs doctor from a non-repo cwd with a minimal `PATH` to prove the absolute-path/`execFile` resolution does not false-degrade**.
