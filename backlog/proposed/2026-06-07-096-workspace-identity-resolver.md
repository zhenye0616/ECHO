...
1. **`resolveCanonicalRoot(path)` exists, is fully specified, and is bounded best-effort.** New function in `src/capture/workspace-root.ts`. Resolution order:
   - **(i) git toplevel** when `path` is inside a git work tree (`git rev-parse --show-toplevel`, repo-root-scoped).
   ...
   - **Ambient-root guard (exact, r1 codex #2):** the set `{ $HOME, /, /tmp, /private/tmp, filesystem root }` is NEVER returned as a *discovered* (walk-up) root, and the walk never ascends above `$HOME`. Such a directory may only be the result when it is the literal `path` passed in (branch iii).
