...
Every source and target Git command runs as pinned `/usr/local/bin/git` 2.37.3 from an explicit minimal environment that clears `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, `GIT_COMMON_DIR`, `GIT_CONFIG_COUNT/KEY/VALUE`, replace/graft/ceiling variables, askpass/SSH/proxy settings, and sets `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null`, and `GIT_NO_REPLACE_OBJECTS=1`. Source reads never use archive attributes: the source audit receives explicit `--source-git-dir`, `--source-sha`, and `--boundary-path` arguments and runs `git --literal-pathspecs --git-dir=<source-git-dir> ls-tree -rz --full-tree <sha>` plus `git --git-dir=<source-git-dir> cat-file --batch` raw-blob reads under a read-only source-audit sandbox. Both source and target checks reject replace refs, graft files, alternates, promisor/partial-clone configuration, and external object stores. Final target checks require `git fsck --full` plus byte-sorted comparison of `git status --porcelain=v2 -z --untracked-files=all --ignored=matching` and a no-follow filesystem walk against HEAD: outside `.git/**`, every regular file must be tracked with the committed mode/hash and no untracked or ignored entry may exist. The shared-target check runs before and after all private-clone verification and an untracked nested-file fixture must fail.
...
There is no mutable receipt, acceptance state, retry reconciler, or evidence lock. Bootstrap is itself ledgered without trusting the later publisher: retained `<attempt-root>/operator/bootstrap-publisher.mjs`, invoked by pinned Node core only, uses descriptor-relative `openat` semantics with `O_CREAT|O_EXCL|O_NOFOLLOW|O_CLOEXEC`, full write, file fsync, close, and directory fsync to create `bootstrap/{intent,stdout,stderr,result}.v1`; failure preserves those files and stops. It compiles retained `evidence-publish.c` with `/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/clang` version `Apple clang 21.0.0 (clang-2100.1.1.101)`, SHA-256 `7def90dd8829726686213a747fc5bff1583df933dae5edc55d755479e0bfe00a`, exact argv `clang -std=c17 -O2 -Wall -Wextra -Werror -isysroot /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk -o <attempt-root>/operator/evidence-publish <attempt-root>/operator/evidence-publish.c`, and `env -i` containing only attempt HOME/TMPDIR, `PATH=/usr/bin:/bin`, `SDKROOT`, `LC_ALL=C`, and `TZ=UTC`. `toolchain.v1.json` pins compiler/binary hashes, SDK 26.5 path plus SDKSettings hash, dependency-file-listed headers, link inputs, flags, environment, and output mode 0500.

The publisher's crash-safe state machine is normative: create one single-component temp with `openat(dirfd,...,O_CREAT|O_EXCL|O_NOFOLLOW|O_CLOEXEC,0600)`, write completely, fsync, `fchmod(0400)`, close every writable descriptor, create the final name by descriptor-relative `linkat` without replacement, fsync the directory, remove only that publisher-owned temp by `unlinkat`, then fsync the directory a second time; this temp removal is the sole deletion exception. A collision or any leftover temp fails and preserves prior bytes. `<attempt-root>/attempt.v1.json` is create-once and binds item, attempt, source SHA, target path, builder identity, publisher hash, and parent-chain hash. Before command N it publishes create-new `commands/<NNN>.intent.json` with argv, cwd, sorted environment, runner/profile hashes, timeout, and raw stream paths; afterward it seals create-new streams and `<NNN>.result.json` with start/end, PID/PGID/session/start token, exit/signal, stream hashes, quiescence evidence, and produced-path hashes. Missing result means interrupted/unaccepted; no actor edits or completes it.

Every command, including successful exit 0, runs through retained hash-pinned `process-watch` in a fresh session/process group. The watcher records a pre-exec-barrier `started.v1.json`, polls macOS `sysctl(KERN_PROC_ALL)` identities keyed by PID plus start time, tracks every observed descendant across reparenting, snapshots `/usr/sbin/lsof -nP -F0` for attempt-root writers and TCP/UDP listeners, and after exit always sends TERM then KILL to any survivor, waits 5 seconds at each boundary, and requires an empty tracked-survivor/listener/open-writer set before publishing result. Timeout/signal uses the same path; a detached/double-forked child that closes streams but retains a writer or listener must fail the hostile fixture. All watcher paths, binaries, argv, polls, raw snapshots, and blind-spot/readiness self-tests are pinned; unavailable primitives fail closed and stop later launches.

Namespaces are exact. `fetch` alone performs online dependency fetch/install. `baseline` runs dependency, boundary, provenance, test-parity, typecheck, lint, product tests, synthetic end-to-end, artifact build/install, smoke/config/selftest, source-independence, fsck, and recursive diff-tree rows and supplies the canonical artifact. `hostile` repeats that entire target-check roster with a different clone/install/output plus poisoned HOME/npm/Git/proxy/PATH sentinels, forbidden Project_echo/sibling/live-state canaries, and fixed locale/timezone/clock inputs. `audit` alone runs the read-only pinned-source audit. `rebuild` alone performs the second clean artifact build/install and artifact comparison. Fetch, baseline, hostile, and rebuild each retain distinct cache and install manifests; plan validation fails missing, duplicate, repeated-in-wrong-namespace, or cross-namespace outputs. Attempt metadata, logs, probes, caches, installs, clones, artifacts, and manifests remain under the attempt root except the single publisher-temp removal described above.
...
Committed `provenance/schemas/verification-plan.v1.schema.json` requires each ordered row's index, stable ID, kind (`target-check`, `operator-audit`, or `artifact-rebuild`), literal argv template, cwd template, sorted environment templates, sandbox-profile template/hash, timeout_ms, stdout/stderr templates, output templates, namespace, repetition ID, and normalizer ID. The complete token vocabulary is `{TARGET}`, `{CLONE}`, `{EVIDENCE}`, `{NAMESPACE_ROOT}`, `{HOME}`, `{XDG}`, `{TMP}`, `{CACHE}`, `{INSTALL}`, `{OUTPUT}`, `{SOURCE_GIT_DIR}`, `{SOURCE_SHA}`, `{BOUNDARY_PATH}`, `{TARGET_HEAD}`, and `{TARGET_TREE}`. The runner performs one non-recursive left-to-right expansion after parsing; tokens may occupy a whole value or a slash-delimited absolute-path component, expansion values are captured once, unknown/nested/residual tokens fail, and every rendered path must remain descriptor-contained in its declared target, source-read-only, or namespace root. Profiles have separate template SHA-256 and rendered-byte SHA-256; intents bind both. The exact namespace/repetition roster is the AC5 roster, and validation rejects any extra or omitted row.

Before use, the builder publishes `<attempt-root>/operator/{verification-runner.mjs,operator-audit.mjs,semantic-normalizer.v1.json}` create-only with the helper, fsyncs, hashes, chmods source/config files 0400 and executable runners 0500, and records hashes/modes in attempt metadata; `evidence-publish.c` is likewise sealed 0400 and its binary 0500 immediately after bootstrap. Bootstrap is `/usr/local/bin/node <verification-runner> --plan <private-clone>/provenance/verification-plan.v1.json --target <private-clone> --source-git-dir <project-git-dir> --source-sha 2971310441b69735cbe759293abd8c4d044bf347 --boundary-path product/source-boundary.v1.json --evidence-root <attempt-root> --namespace <fetch|baseline|hostile|audit|rebuild> --ledger <namespace-ledger>`. `semantic-normalizer.v1.json` permits replacements only at `/timing/start_monotonic_ns`, `/timing/end_monotonic_ns`, `/process/pid`, `/process/pgid`, `/process/session`, and declared path-display fields, using only the plan token vocabulary. It must never normalize exit/signal/timeout state, raw byte/content hashes, artifact/member/install/cache manifests, Git OIDs/trees/identities, containment results, or probe verdicts; any undeclared pointer/token or security-relevant variance fails. Fixtures cover unknown/cross-root tokens, malformed expansion, and forbidden normalization.
...
Failures are durable in the immutable command ledger and raw logs only; after a handoff commit exists, the builder never edits Project_echo agent-run or backlog files. The builder records expected Project_echo parent SHA and a clean index/worktree baseline, stages only the exact item, task-state builder pointer, one run log, one migration record, and generated `docs/BACKLOG.md`, compares the NUL-delimited staged set with that allowlist, runs `git diff --cached --check`, proves empty unstaged/untracked state, commits once, and proves the committed tree differs from the expected parent only at those paths. Before the sole push it publishes create-new `<attempt-root>/handoff.intent.json` containing the single resolved-and-hashed Project_echo endpoint, fully qualified absent destination feature ref, exact 40-hex handoff commit/tree, and expected parent. The endpoint must be the normalized expected `https://github.com:443/<owner>/<repo>.git`; scp/file/ext/git/ssh forms, redirects, pushurl, rewrites, proxies, hooks, helpers, push options, and implicit refs are rejected. Pinned Git runs under `env -i`, `GIT_CONFIG_NOSYSTEM=1`, empty global config, noninteractive transport, protocol allowlist `https`, and a hash-pinned ephemeral-FD credential helper as its sole auth path. It pushes literal `<handoff-sha>:<full-ref>` with `--force-with-lease=<full-ref>:`. Porcelain output must describe only that exact SHA/ref update; any mismatch, existing ref, ambiguity, or nonzero result stops without retry. The target repository is never pushed.
...
The final builder write is create-new `attempt-seal.v1.json`: canonical UTF-8/LF byte-sorted rows cover every descendant path except the seal itself with type, relative path, mode, size, and content SHA-256; symlinks and sockets fail. Before publishing it, all writable descriptors are closed, every other file is 0400, every directory below the root is 0500, all leftover temps are rejected, and the manifest digest is recorded in the migration record. The seal is published/fchmoded 0400, root becomes 0500, and reviewers recompute the self-excluding digest without writing the builder root.

Each independent reviewer revalidates the complete AC1 parent chain, descriptor-relatively creates a separate 0700 `/Users/zhenye/Desktop/.echo-migration-evidence/133/<review-uuid>`, and binds `kind:review`, item, round, request correlation, artifact SHA, reviewer identity, accepted target HEAD/tree, builder attempt path, and canonical attempt-seal digest in create-once metadata. It copies the sealed runner/audit/normalizer/profile templates and read-only cache seed into reviewer-owned roots, verifies each against the seal before and after use, renders only reviewer namespaces, and publishes evidence with its own hash-verified publisher. Tests make the builder root read-only and prove every attempted reviewer write there fails. Review uses reviewer-owned clones/outputs, recomputes the source audit, and requires byte-identical artifact SHA-256 plus identical member manifest. Passing proves only a local DEV source split.
...
- `<attempt-root>/operator/tests/evidence-publication.test.mjs` — bootstrap records, no-replace state machine, double directory fsync, seal manifest, modes, and collision/temp failures.
- `<attempt-root>/operator/tests/verification-plan.test.mjs` — exact namespace roster, token expansion/containment, hostile canaries, and prohibited normalization.
- `<attempt-root>/operator/tests/process-quiescence.test.mjs` — success/timeout/signal plus detached child, listener, and open-writer survivor rejection.
- `<attempt-root>/operator/tests/reviewer-bootstrap.test.mjs` — request-bound reviewer roots, sealed-input hashes, and builder-root write denial.
