...
The extractor first acquires same-filesystem lock directory `/Users/zhenye/Desktop/.echo-brain-extraction-133.lock` with atomic `mkdir`, writes `owner.json` containing item ID, source SHA `2971310441b69735cbe759293abd8c4d044bf347`, run ID, PID, and start time, and materializes committed source objects into `/Users/zhenye/Desktop/.echo-brain-staging-<run-id>`. The staging repository owns `.echo-extraction.json` with durable phases `materializing`, `building`, `verifying`, `committed`, `published`, or `failed`. A retry may resume only an explicitly named run whose item ID and source SHA both match; it refuses unknown/mismatched locks, staging directories, or an existing `/Users/zhenye/Desktop/echo-brain`, and never deletes or adopts them. Failures atomically record phase, command, exit code, and recovery command in the marker and preserve staging for inspection. Tests cover interruption after lock acquisition, Git initialization, initial commit, and verification.

Only after all AC7 gates pass does the extractor atomically rename the staging directory to `/Users/zhenye/Desktop/echo-brain` and mark the lock `published`; staging and final paths are on the same filesystem. `/Users/zhenye/Desktop/echo-brain/.git:1` then exists on branch `migration/2026-07-13-133-local-echo-brain-source-extraction`, contains at least one signed-off-by-content commit whose message names the source SHA, and reports no configured remotes from `git remote -v`. No GitHub API, `gh repo create`, remote URL, tag, release, package publication, or client installation is allowed.

### AC2 — Make echo-brain the complete and accurately named client product

`/Users/zhenye/Desktop/echo-brain/package.json:1` names the private package and binary `echo-brain`, sets `engines.node` to exactly `22.22.1`, sets `packageManager` to `npm@10.9.4`, owns its exact runtime and development dependencies, and contains no workspace, `file:`, Git, tarball-path, or absolute-path dependency. A committed `/Users/zhenye/Desktop/echo-brain/package-lock.json:1` is the sole clean-install lock. Before any package write or install, the extractor resolves `node` and `npm`, records their absolute executable paths and versions, and hard-fails unless they are exactly `v22.22.1` and `10.9.4`; `engines` is not treated as enforcement. `/Users/zhenye/Desktop/echo-brain/src/:1` owns the Team meeting-to-decision-card/brief product composition: canonical meeting input, deterministic product runtime, decision reasoning adapter seam, human approval, local state/health, brief generation, and qualification evidence. Internal symbols may use accurate capability names such as `DecisionReasoner` and `DecisionRuntime`; the whole product remains `echo-brain`.
...
`/Users/zhenye/Desktop/echo-brain/schemas/runtime-config.v1.schema.json:1` preserves the client-local Team-product configuration contract with secret references only. `/Users/zhenye/Desktop/echo-brain/src/runtime/paths.ts:1` resolves all mutable state under an explicit installation-local root and cannot read `Project_echo` state implicitly. `/Users/zhenye/Desktop/echo-brain/tools/build-artifact.mjs:1` acquires the extraction lock, rejects a dirty tree including untracked files except its declared ignored output root, captures one candidate HEAD, exports that commit with `git archive` into a scratch verification directory, and builds one private DEV artifact only from that archive. It emits a manifest containing the captured HEAD plus artifact SHA-256, refuses overwrite, and never shells into or reads the migration repo. Trap/finally cleanup removes successful scratch outputs and preserves failed outputs with their path in `.echo-extraction.json`.

### AC6 — Preserve product behavior at the pinned source boundary

`/Users/zhenye/Desktop/echo-brain/provenance/test-parity.v1.json:1` inventories exactly the eight files returned at the pinned SHA by `git ls-tree -r --name-only 2971310441b69735cbe759293abd8c4d044bf347 -- tests/product`: `build-once-contract.test.ts`, `hermeticity.test.ts`, `import-fence.test.ts`, `packaged-product.test.ts`, `qualification-report.test.ts`, `runtime-config.test.ts`, `runtime-isolation.test.ts`, and `setup.ts`. Every row records source blob, destination path, and `ported` or `excluded`; exclusion requires a boundary-specific rationale and zero silent omissions. `/Users/zhenye/Desktop/echo-brain/tools/check-test-parity.mjs:1` asserts the source count is eight, the manifest covers all eight exactly once, destination test counts match, and rewritten tests differ only by manifest-enumerated import specifier or fixture-path literal substitutions after normalization; assertion, fixture-data, and expected-runtime changes fail the check.
...
`/Users/zhenye/Desktop/echo-brain/tests/migration/source-independence.test.ts:1` fails on symlinks, submodules, absolute `Project_echo` paths outside the allowed provenance value, imports escaping the repository, undeclared child-process access, and forbidden dependency schemes. `/Users/zhenye/Desktop/echo-brain/tools/verify-source-independence.sh:1` verifies `/usr/bin/sandbox-exec` can deny a sentinel read, then runs the entire verification child under a generated profile containing `(deny file-read* (subpath "/Users/zhenye/Desktop/Project_echo"))`; renaming, chmodding, unmounting, or otherwise mutating the source checkout is forbidden. The child uses `env -i` with only resolved `PATH`, scratch `HOME`, `TMPDIR`, `ECHO_BRAIN_HOME`, and fixed locale/timezone values, first asserts the source sentinel is unreadable, then runs `npm ci --ignore-scripts=false --no-audit --no-fund`, `npm run typecheck`, `npm run lint`, `npm run check:boundary`, `npm run check:test-parity`, `npm test`, `npm run build:artifact`, `npm run smoke:installed`, installed-artifact `validate-config`, and installed-artifact `selftest`, all with bounded timeouts. Scoped isolation unavailable or any source read is a hard failure with durable marker evidence.
...
The migration record explicitly says `candidate_authority:false`, `remote_created:false`, and `maturity:DEV`, plus the final target path, clean local HEAD, provenance/test-manifest hashes, exact verification commands, and `git status --porcelain=v1 --untracked-files=all` output. The candidate must remain clean and preserved at that HEAD until independent review disposition; review blocks if the path is unavailable, HEAD/hash differs, the tree is dirty, or any remote exists. No existing `Project_echo` product path is deleted, frozen, redirected, or declared stale. A later founder checkpoint may accept the local repository, make it authoritative, and authorize a private remote; this item only produces the reviewable local candidate and parity evidence.
...
- `/Users/zhenye/Desktop/echo-brain/tests/migration/extraction-lifecycle.test.ts` — lock races, staged phase markers, interruption/resume, unknown-target refusal, and atomic publication.
