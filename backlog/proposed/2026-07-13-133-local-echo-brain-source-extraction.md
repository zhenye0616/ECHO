...
`schemas/runtime-config.v1.schema.json:1` preserves the client-local config contract using secret references only; `src/runtime/paths.ts:1` owns install-local state distinct from Project_echo and siblings. Before target creation, the orchestrator validates a lowercase UUID attempt ID and atomically creates 0700 `/Users/zhenye/Desktop/.echo-migration-evidence/133/<attempt-id>` after no-follow ancestor checks; EEXIST refuses. Every receipt/artifact/record binds that immutable attempt root. The atomically written receipt transitions only `NOT_ACCEPTED -> checks_passed/handoff_pending -> handoff_published`; failures persist phase/command/exit/stdout-stderr hashes and remain NOT_ACCEPTED. `npm run build:artifact -- --expected-head <target-head> --run-output <attempt-root>/artifact` refuses pre-existing output. Artifact/manifest stay retained through review; `handoff_published` is written only after the Project_echo handoff commit is pushed, and cleanup happens only after recorded disposition.

### AC6 — Preserve product behavior at the pinned boundary

`provenance/test-parity.v1.json:1` inventories exactly the eight pinned `tests/product` files, with source blob/hash, destination, disposition, and ordered non-overlapping substitutions (`start_byte`, `end_byte`, exact source bytes/hash, replacement bytes/hash) applied highest-offset-first. Standalone `npm run check:test-parity` validates schema/count/order and destination transformed hashes only; AC7's pinned-source operator audit exclusively verifies source hashes and replays substitutions. A new test outside that count proves synthetic `meeting input -> adapter -> manual review gate -> brief artifacts` with fixed time, no credentials, no real data, and no external service.
...
After the target HEAD is committed, each verifier treats the shared target read-only, captures its exact 40-hex HEAD/tree, clones to a unique absent root with `--no-local --no-hardlinks`, detaches that OID, verifies commit/tree, removes origin, proves clean/no-remotes/no-alternates/no-promisor, and rechecks shared HEAD unchanged. Every Node/npm/sandbox phase uses `env -i` with allowlisted scratch HOME/XDG/TMPDIR/cache/config and prevalidated absolute Git `2.37.3`, Node `22.22.1`, npm `10.9.4`, and `/usr/bin/sandbox-exec`; `PATH`, NODE_OPTIONS/NODE_PATH, npm/Corepack/proxy/auth variables are otherwise absent.

Phase 1 runs script-disabled/no-audit/no-fund registry fetch inside a retained fetch profile that denies source/sibling/live-state/credential reads and external writes while allowing registry network; it records lock hash, cache manifest, exact profile/runner hashes, probes, and exits. After removing node_modules and sealing a cache copy, phase 2 runs `npm ci --offline --ignore-scripts --no-audit --no-fund` with deny-all network. Enumerated lifecycle/build commands and all tests run in phase-specific deny-all-network profiles. Mandatory positive/negative probes prove allowed roots/fetch and forbidden read/write/network classes; enforcement absence fails closed. Profiles and probe evidence remain under the attempt root.

Verification runs exact dependency, boundary, provenance, test-parity, typecheck, lint, product tests, synthetic end-to-end, artifact build/install, smoke/config/selftest, source-independence, `git fsck --full`, and `git diff-tree --check --root HEAD` commands. A separate read-only operator audit derives the manifest universe from the item-132 boundary and pinned Git tree, recomputes every source blob/content hash at `2971310441b69735cbe759293abd8c4d044bf347`, replays ordered substitutions against committed target HEAD, and records exact rerunnable commands/exits. Hostile source/HOME/npm/Git/PATH/Node/sibling sentinels must not affect output.
