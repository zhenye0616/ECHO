...
Add src/install/secrets.ts. Install generates 32 cryptographically random bytes and atomically creates secrets/mcp-bearer-token under the install's configuration-derived support root as a 0600 regular file inside a 0700 directory owned by the current user. The secret/support root is never hard-coded: AC7's repo-free clean-home smoke resolves it under the temporary HOME/support prefix (writing no real user state), and the real per-user shadow install resolves it to /Users/zhenye/Library/Application Support/echo-context/secrets/mcp-bearer-token, exercising the same resolution code path in both. The config schema also accepts separately named 0600/owner-checked secret_ref values for capture adapters such as Granola, without reading them while the worker is disabled. Existing symlinks, hardlinks, wrong owner/mode, unexpected content length, or a path outside the owned support root fail. Same-artifact restart/reinstall reuses the MCP credential; no automatic rotation is needed in this item.
...
The artifact is built once in the reviewed release job. All release-phase clean-home and real-user installs consume those exact bytes; no npm install, npm rebuild, source checkout, compiler, or alternative artifact is allowed after the release artifact hash is recorded.

Ownership is two-phase and explicit. Phase one (builder, pre-review): implement everything, keep all tests green, and run the AC7 clean-home lifecycle against a locally built candidate runtime artifact whose manifest marks a non-release candidate version identity; candidate artifacts are never published, never installed to real per-user paths, and never recorded in the migration record. Phase two (founder execute, post-review): after independent target review and founder merge to echo-context/main, the reviewed release job seals the successor source SHA, builds the single release runtime artifact from the fresh detached clone of the landed SHA, re-runs the clean-home smoke against those exact release bytes, publishes the private prerelease, installs the real founder shadow, and records the evidence. Only phase-two bytes are the release artifact; build-exactly-once binds to that release artifact, and no source change may reopen after phase two begins without returning to review. Tamper, wrong platform/Node/source, native ABI mismatch, missing/extra/unsafe member, checksum/SBOM/lock drift, or repository-path leakage fails before extraction.

### AC5 — Install safely as a per-user service with an ownership manifest

Add src/cli/index.ts; CLI commands install, start, stop, restart, disable, status --json, doctor --json, uninstall, internal serve --config, and offline prepare-final --authority-record; and src/install/layout.ts, artifact.ts, launchd.ts, install-state.ts. Refuse root/sudo and /Library/LaunchDaemons. Verify the runtime artifact before extracting through path-traversal/symlink/hardlink/type/mode limits into the immutable owned directory /Users/zhenye/Library/Application Support/echo-context/releases/<version>-<artifact-prefix>/. Install runs an architecture preflight for the darwin/x64 artifact: before extraction it verifies the host can execute x64 code — on Apple Silicon it explicitly detects whether Rosetta 2 translation is installed and available (bounded probe) and fails with a clear remediation message when it is not; after extraction and before any plist creation or service start, it proves the bundled Node executes via a bounded version/architecture probe of the bundled absolute node path. The same preflight governs the founder-Mac shadow install in AC7.

The stable shim /Users/zhenye/.local/bin/echo-context may point only to the owned immutable release. The per-user plist /Users/zhenye/Library/LaunchAgents/com.echo.context.plist invokes that release's executable with the owned config path, contains no token or repository path, sets no secret environment variable, and writes only to /Users/zhenye/Library/Logs/echo-context/stdout.log and stderr.log. The installer itself creates that log directory with current-user ownership and owner-only-write modes, and service log output is bounded: install/start enforce a documented maximum retained size for stdout.log and stderr.log via rotation or truncation of installer-owned log files, so an unattended crash/output loop cannot grow logs without limit. launchctl actions verify GUI user domain, label, PID/start time, executable realpath, and artifact identity rather than trusting a stale PID file.
...
status --json and doctor --json emit a versioned schema with installed, runtime_authority, source SHA/tree, source-artifact and runtime-artifact hashes, runtime version, lock/SBOM hashes, label, port, home, PID/start time, executable realpath, launchd state, authenticated readiness, exact tool roster, capture acceptance/worker list, config/plist/secret ownership and modes, process architecture and Rosetta/translation status, startup-lease holder identity and state, log-path ownership/modes and bounded-retention status, listener addresses, repo_dependency, and overall verdict.

Doctor is read-only over installed state and must fail honestly for a stale/mismatched executable, modified release/config/plist, bad modes/ownership/type, port or label collision, non-loopback listener, missing/wrong auth, unexpected tool, DB integrity/schema error, capture enabled, authority true, open repository dependency, misowned or unbounded log paths, an incompatible process architecture or missing required translation, a startup blocked by a live or foreign lease owner, or service/launchd disagreement. It never repairs, starts, stops, writes a fixture, or prints a credential. A separate scratch selftest owns write/retrieval proof.
...
Add tools/shadow-install-smoke.mjs. The clean-home lifecycle runs in both AC4 phases: phase one against the builder's candidate artifact, phase two re-run against the exact release bytes before the founder shadow install. In each run, under a repo-free temporary HOME/support/log/state prefix with ECHO_* variables cleared, verify the checksum, install, start, authenticate, list exactly eight tools, run the service fixture, restart, disable and prove the listener is gone, exercise prepare-final crash/recovery against disposable prepared records without activation, re-enable, uninstall, and prove state/secret preservation. Timeouts terminate the process group and report only bounded redacted stderr.
...
- tests/runtime/composition.test.ts injects every startup/shutdown failure boundary and proves reverse unwind, bounded drain, SQLite close, lease release, capture disabled, and exact eight-tool registration, plus stale-lease reclaim after crash/SIGKILL and distinct refusal when a live matching or foreign process owns the lease.
...
- tests/install/launchd.test.ts and tests/cli/lifecycle.test.ts prove plist contents, GUI-user launchctl identity, idempotence, collision refusal, restart/disable/uninstall, state preservation, constrained purge, architecture-preflight failure modes, installer-owned log directory creation/modes, and bounded log retention under a repeated crash/output loop.
