...
- Tags, GitHub Releases, hosted/private prereleases, uploads, hosted artifact downloads, publication/cache reconciliation, release-set manifests, release FSMs, and approval-file challenges are excluded and remain item 140 work. Final assembly permits only the fixed Node archive and exact package-lock URL+integrity closure under a reviewed native toolchain tuple and empty caches.
...
- Capture-off fixture data enters only through an idempotent offline fixture-ID lookup anchored by the ownership receipt and verified installed manifest while the service is stopped and both lifecycle/writer locks are held; online capture remains typed 403 before body read.
...
- Plist stdout/stderr are `/dev/null`; every serve path requires log-dir and opens its bounded startup/application sink before other fallible work. Logger-open failure exits 0; doctor reports unavailable only while independently provable and otherwise outcome-unknown.
- Real lifecycle uses only launchctl bootstrap/bootout/read probes and always converges stop/restart through unloaded absence. Candidate lifecycle has a separate process-group FSM.
- Status and doctor use a shared lock plus beginning/end PID/start-time/generation/artifact sandwich with bounded retry; absent lock is not-installed, persistent churn is busy exit 4. Both have named closed schemas, canonical JSON+LF, bounded probes, redacted stderr, and deterministic exit precedence. Doctor never repairs.
...
- Before target-main mutation and real installation, the persistent coordinator commits, pushes, and reads back a fresh exact-operation authorization. A reviewed runner copies each opened non-link asset into an immediately unlinked descriptor snapshot, hashes those descriptors, and runs/consumes only `/dev/fd` streams bound by the authorization. Builders never inherit that authority.
