...
- R13 must verify the R12 fresh-clone-verifier corrections — exact per-mode argv templates (no generic `npm run <script>` form, test:operator rejected, source-only single build/verify pair, release caller-bound verify) and the three fail-closed clean-tree probes — at the patched spec SHA before any convergence call.
