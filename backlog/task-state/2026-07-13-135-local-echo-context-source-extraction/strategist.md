...
- Verification uses process-scoped source-read denial; service smoke allows loopback only and proves cleanup of processes, sockets, SQLite handles, and scratch state.
...
- R3 by independent `codex` and `codex-ops` bindings must verify the exact roster, parity matrix, lifecycle, and service cleanup patch.
