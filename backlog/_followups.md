...
- **052-followup-A (sidecar §3, MED) — strategist task: note pre-commit hook landed.** Add a one-line note to `raw/internal/decisions/2026-05-13-echo-skills-are-the-cross-tool-protocol.md` "Next steps" section recording that `tools/install-pre-commit-hook.sh` shipped with 052. Closes the open follow-up that decision file explicitly named. Estimate ~5 minutes.
- **052-followup-B (BLOCKING on 050 merge) — re-insert `tools/sync-skills.sh --check` line + remediation paragraph into 050's restructured C5.** 050 substantially rewrites skills/merge-and-cleanup.md's C5 block (wraps merge in `$TMPDIR/echo-merger-<uuid>` worktree). The 052-added sync-skills gate must be re-inserted inside 050's new C5 form during 050's merge conflict resolution. AC4's tests/skills/merge-and-cleanup-shape.test.ts will fail the merge-verify step if forgotten. (Resolved when 050's merge commits cleanly with both shape test green AND `tools/sync-skills.sh --check` exiting 0.)
- **050-followup-H (sidecar §"Suggested fixups", founder-only) — remove literal `echo-merge-in-progress` from docs/BACKLOG.md:23-24.** 050 AC7 grep returns 6 hits: 4 are legitimate AC6.5 negative-assertions in `tests/review-queue/worktree-isolation.test.ts:360-363`; 2 are tracking-table row prose in `docs/BACKLOG.md:23` (051 row) and `docs/BACKLOG.md:24` (050 row). AGENT_INSTRUCTIONS forbids builder edits to docs/BACKLOG.md, so the agent correctly left these alone. Founder edit: reword as `the sentinel-file lock convention` (backticking still hits grep). Non-blocking. ~1 min.
- **050-cross-merge-note — Codex consulted mid-merge.** During 050's merge-conflict resolution, founder requested cross-vendor review from Codex (ad-hoc `codex exec --sandbox read-only` against `/tmp/codex-050-conflict-review-prompt.md`). Codex returned `proceed-with-modifications` and identified two non-conflict refinements: (a) remove orphaned 051 `CODEX_BIN` env-hook block at `_run_reviewer.sh:49-53` (outside conflict markers — would otherwise leave a dead test seam), (b) update `push-with-retry.sh` header behavior-comment from `pull --rebase` to `pull --rebase=merges`. Both folded into the merge commit. Pattern: ad-hoc mid-merge cross-vendor review (not the standard review-queue protocol — no `backlog/reviews/**/request.md` involved) is useful for non-trivial conflict resolutions where the resolution plan is judgment-loaded. Consider codifying as `/merge-and-cleanup` optional escalation (C3.5: "if conflicts are deeper than the sidecar's prescriptive playbook, surface to a cross-vendor reviewer before applying").
