...
- [ ] **NEW Bug A — `search_memories` per-match envelope is uncapped.** 15:54 PDT call `search_memories(query='JSON-RPC', source_app='codex')` returned only 3 matches (limit_applied:10) but **318,574 chars** total — match byte sizes 138,903 / 85,321 / 94,147. A single Codex turn match is ~100KB; there is apparently no per-match content cap at all. Distinct from Bug 3 (which is about cluster envelope in `get_recent_work_context`). Spec needed: per-match content cap (e.g., truncate atom content > 4KB to head + tail with explicit `(N chars elided)` marker, project a `match.bytes_elided` field), and a realistic-density envelope test for `search_memories` mirroring 028's pattern. Surfaced in dogfooding journal at 2026-05-08 15:54 PDT.
- [ ] **NEW Bug B — `tail_session(source_app=<app>)` resolves to fs-watcher meta-events.** Both `tail_session(source_app='codex')` and `tail_session(source_app='claude_code')` returned atoms whose `content` is a fs-watcher event payload (`{event_type:"change", path:"...", mtime:"...", size:N}`, `metadata.surface:"fs"`) — i.e., the file-modification meta-stream targeting the app's session file, NOT the app's extracted turn atoms. Tool is wired up and reachable; envelope is well within budget; source resolution arrives at the right *file* but the wrong *atom layer*. Spec needed: source-resolution rule should prefer the extractor's turn atoms over fs-watcher events when both target the same path (e.g., `metadata.surface !== 'fs'` filter, mirroring `recent-work-context.ts:171` `exclude_metadata_surface: ['fs']`). Surfaced in dogfooding journal at 2026-05-08 15:54 PDT.
