...
## ✅ Resolved (V1.6 wave — items 030 + 032; 033 in flight)

### MCP retrieval — long-turn elision + envelope caps (RESOLVED end-to-end by 030 + 032; 033 in flight closes the recovery half)

**Status:** RESOLVED 2026-05-10 via two shipped items and one in-flight item.
...
- **Recovery half — in flight via item 033** (`backlog/claimed/2026-05-10-033-full-atom-recovery.md`, builder claimed 2026-05-10 ~16:30 PDT): new `get_atom(id)` tool returns content verbatim through MCP — no shell, no JSONL fallback, no composer-id context. After 033 ships, the full elision-recovery loop closes in-MCP and the JSONL fallback becomes last-resort instead of routine.
