...
- [ ] **Dogfooding verification for 033 (post-merge action, After Completion §1).** Next time a Codex (or other long-turn) MCP response comes back with `truncations: ["content"]`, the strategist (or any AI client) should call `get_atom(<elided_atom_id>)` instead of `jq` against the JSONL. Log to `raw/internal/dogfooding/mcp-interactions-journal.md` documenting whether the response returned verbatim or hit `atom_too_large_for_wire` — both outcomes are useful data.
...
- **🔴 HIGH — `tests/review-queue/concurrency.test.ts:133` orphan-cleanup test pre-existing failure (surfaced post-039 merge; confirmed reproducing on `main` HEAD `1c2e2a4` 2026-05-12).** The test asserts `combine.py` removes stale `.tmp.*` files older than 30 min from `r{N}/` directories, but it's failing because the cleanup path isn't actually firing. 039 verify step (`npm test` at 039 merge time) returned 782 pass with no failures — either flake-window-luck or test ordering hid this. Now reproduces deterministically. 040's branch did NOT touch `concurrency.test.ts` or `tools/review-queue/combine.py`, so it's not a 040 regression — but the bug is real and the test has been silently red since 039 merged. File its own item (suggested ID: `2026-05-XX-NNN-combine-py-orphan-cleanup-fix`) and either fix the cleanup path in `combine.py` or update the test to match shipped behavior, whichever the strategist conversation decides is correct.
