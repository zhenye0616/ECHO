...
- 049 strategist queue (After-Completion): extend vendor-neutralization to `merge-and-cleanup`, `review-queue-*`, `process-backlog-batch`; generate `agents/openai.yaml` for each codex adapter; ~~pre-commit hook for `sync-skills.sh --check`~~ ✅ **CLOSED 2026-05-15 by 052** (`tools/install-pre-commit-hook.sh` shipped, idempotent, 6-scenario test coverage); verify codex auto-discovery honors symlinks (R2 mitigation).
