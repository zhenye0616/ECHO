...
- [ ] **Strategist wiki promotion for 030 (post-merge action).** Three new pages: `wiki/surfaces/mcp-find-clusters.md`, `wiki/surfaces/mcp-get-atoms.md`, `wiki/surfaces/mcp-wait-for-new-turns.md` (mirror existing per-tool page convention). Also write `wiki/architecture/group-session.md` (synchronized human-driven group pattern + polling fallback + deferred Goal C). Update `wiki/surfaces/mcp-server.md` (list new tools; flag `get_recent_work_context` deprecated → item 031), `wiki/surfaces/mcp-recent-work-context.md` (top-of-page deprecation banner), `wiki/architecture/system-architecture.md` (V1.6 toolkit shift compound→atomic). Move "MCP retrieval — long-turn elision + envelope caps" from "Known V1 degraded surfaces" to a "Resolved" subsection in this file (truncations field closes the trust-bug part; per-cluster sourcing concern unchanged). Source: 030 "After Completion" §§1–7.
- [ ] **Promote `wiki/operating-model/cross-tool-spec-review.md` from candidate to definite-after-030-ships.** The cross-tool review pattern now has TWO independent confirmation cycles: R1 spec review (Cursor + Codex caught 14 complementary findings) and R2 code review (Cursor + Codex caught 3 envelope-ceiling bugs that the Claude code-reviewer subagent missed). The Claude subagent's round-1 verdict was "merge with founder fixups (one-line journal append)" — it missed all three real bugs. Cross-tool review is now load-bearing for items where >25% of the implementation is novel envelope/ceiling logic. Strategist conversation should write the wiki page from the journal evidence at lines 1090–1149 of `mcp-interactions-journal.md`. Source: 030 round-1 reviewer miss + R1 spec review precedent.
