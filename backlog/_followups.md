...
- [x] ✅ **Gap 2 Layer 2 (cursor streaming-continuation orphan-warning spam)** — V1.5.7 patch shipped at `98fbd10` (2026-05-08 20:48 PDT). Live-confirmed at 20:50 PDT: 0 orphan warnings in 30s post-restart vs ~250/minute pre-patch. Root cause: ECHO captured turns mid-stream, set checkpoint to partial-cluster-last assistant; subsequent ticks orphaned every assistant Cursor added to extend the response (902,716 warnings over 1232 minutes). Fix: silently fast-forward past assistants when post-checkpoint.
...
- [x] ✅ **Gap 1 (echo-memory MCP fully broken)** — diagnosed as legacy/superseded sibling-project artifact. The `echo-memory` MCP server is the EchoChat Python backend at `/Users/zhenye/Desktop/Projects/EchoChat/apps/backend/.venv/bin/python -m app.mcp`, registered in `~/.claude.json` user-scope around line 3227. NOT part of Project_echo's V1 published surface. Resolution path: founder un-registers from `~/.claude.json` (edit out the `echo-memory` mcpServers entry around lines 3227-3236). The journal at line 518 (item 025 validation) already noted this dual-registration drift — cleanup pending. Once un-registered, the 5 broken tools disappear from the AI client's tool list and the V1.5 ship-blocker dissolves cleanly.
