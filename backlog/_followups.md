...
- [ ] **NEW Bug A2 — per-match `metadata` envelope is uncapped.** 16:14 PDT live verification: `search_memories(query='JSON-RPC', source_app='codex')` returned **305,632 chars** total (vs 25k budget). Per-match content was correctly capped at 2023 chars by Bug A1 fix, but `metadata.tool_calls` was **130,997 / 120,171 / unprobed** chars per atom — the codex extractor stores raw tool-call payloads in metadata. Spec needed: per-key `metadata` value cap (e.g., 2KB after which the largest values get replaced by `{__elided:true, original_size:N}`), with a `metadata_bytes_elided: number` and optional `metadata_keys_elided: string[]` on the match. Same fix should apply to `tail_session` (which uses the same `searchMatchSchema` and the same metadata-bloat pattern surfaced in the same round). The clean separation: MCP-side cap is unconditional safety; capture-side trim of `tool_calls` is a separate quality question for the codex extractor. Surfaced in dogfooding journal at 2026-05-08 16:14 PDT.
- [ ] **Bug A1 reach gap — `tail-session.ts:68 toMatch` does NOT apply `clipContent`.** The `search-memories.ts` `toMatch` was updated by the Bug A1 fix; tail-session has its own `toMatch` that copies `e.content` verbatim. Today this is masked because tail-session content is small (max 5277 chars observed at 16:14 PDT), but on a tail of long-content extractor atoms (e.g., a Cursor session with very long assistant turns) it would surface. Recommended fix: extract `clipContent` to a shared helper and call from both `toMatch` functions (or have tail-session import the search-memories helper directly). Low-effort follow-up; bundle with Bug A2.
- [ ] **Confirm Bug 3 closure post-028 merge.** After 028 lands, re-run the 15:54 PDT default-args scenario with `format:'skeleton'` and confirm envelope < 25k chars. Log to dogfooding journal as the third real-world regression-closure measurement (after 15:05, 15:14, 15:54). Also confirm the `format:'minimal'` default keeps producing useful resume briefings (i.e., that we have not over-stripped) — strategist + founder pair on the read.
