...
- **`_install_reviewer_launchd.sh --smoke` fails-open when smoke runner is absent** (r2 codex-ops MED #2). Currently `tools/review-queue/_install_reviewer_launchd.sh:97-103`: when `--smoke` is passed but `smoke-test-<reviewer>-runner.sh` doesn't exist, the script prints a warning and exits 0. This let the 044 pre-flight declare codex-ops "smoke verified" without actually running a synthetic-request smoke test. Fix: make the absence fail-closed (exit 1 with clear diagnostic) OR provide a generic `_smoke_test_runner.sh` fallback that's parameterized on `REVIEWER_NAME`. Out of 044 scope (per AC2 "Do NOT change ... `_run_reviewer.sh` itself" — `_install_reviewer_launchd.sh` is in the same scope-defended family). Tiny scope (≤10 lines). Suggested ID: `2026-05-XX-045-smoke-gate-fail-closed`.
