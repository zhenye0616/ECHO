...
- **Strategist post-shipment (owed):** promote 100 per its After-Completion notes — mark `_followups.md` R6.adapter_freshness bullets resolved ("generalize skill-adapter freshness gate to ALL client adapters", "C2 adapter-drift detection for Codex-installer adapter", "Stale Codex producer field" detection half), recording the split: repo-tracked adapters → merge gate (`sync-skills.sh --check`); operator-local Codex adapter → `echoctl doctor` (HOME-relative). Cross-ref 099. Optional one-line operating-model wiki note; no new product wiki page required.
