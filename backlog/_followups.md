...
- [ ] **Strategist wiki promotion for 032 (After Completion §2-§5).** Update `wiki/surfaces/mcp-find-clusters.md` (add "Auto-expand triggers" sub-section: empty + single-source-recent paths + strict-partition demotion rule), `wiki/surfaces/mcp-get-atoms.md` (add "Resume-call usage" sub-section: `prefer='newest_first'` + missing-ID position behavior), `wiki/architecture/group-session.md` (note first-call reliability gate closed for resume-after-gap), and move M2-1/M2-2 from `_followups.md` "biting" to a "Resolved" subsection with this item's merge SHA + dogfooding-verification entry timestamp. **Predecessor dependency:** `wiki/surfaces/mcp-find-clusters.md` and `wiki/surfaces/mcp-get-atoms.md` don't exist yet — they're owed by 030's "After Completion" wiki promotion (also pending). 032's promotion should happen after 030's promotion OR fold the 032 sections into the 030-promoted pages in a single pass.
