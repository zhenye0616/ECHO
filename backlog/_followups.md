...
- [ ] **Strategist wiki promotion for 033 (After Completion §1-§5).** New `wiki/surfaces/mcp-get-atom.md` (per-tool page mirroring `mcp-find-clusters.md` / `mcp-get-atoms.md` convention) covering: content-verbatim + metadata-projected + embedding-excluded contract, three exit shapes (success / `atom_too_large_for_wire` / `atom_not_found`), canonical recovery pattern from `truncations: ["content"]`. Bump tool count 7 → 8 in `wiki/surfaces/mcp-server.md` "MCP toolkit" section + `wiki/architecture/system-architecture.md`. Update `wiki/operating-model/cross-tool-spec-review.md` "Findings classes" + "Cost / value" sections to reference `get_atom` as the in-MCP recovery primitive (replaces "use JSONL/SQLite fallback" language). Move M1-3 from `_followups.md` "biting" → "Resolved" subsection with merge SHA.
