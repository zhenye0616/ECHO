...
- **New MCP read tool `coord_status()`** at `src/mcp/tools/coord-status.ts`. Returns:
  ...
  - **Slot universe is deterministic from `coord-roles.json`** (r4 convergent F1 MED + r5 codex F1 MED — single source of truth, no AC1-registry slip): the set of slots is `{ (role.name, event.expects) : for role in coord-roles.json["roles"], for event in role["events"], where event["expects"] is set }`. The `expects` field lives ONLY in `coord-roles.json`'s `role.events.<event_type>.expects` — the AC1 type registry at `src/coord/types.ts` carries tier classification + subject-role policy but NOT `expects` values. AC3 close/open/reconstruction and AC6 slot-universe construction therefore read from the same source (`coord-roles.json`) — no two-sources-of-truth hazard. The slot universe is recomputed at startup from `loadCoordRoles()` (zero in-memory persistence required) and stays stable across daemon restart.
  - **On-demand rehydration at `coord_status()` call time**, entirely from durable atoms (r4 convergent F1 MED + r4 codex F2 MED + r4 codex-ops F2 MED): the daemon iterates `iterateCoordAtomsByAppendOrder({ sinceSeq: 1 })` once per call, building per-slot `last_miss` (highest-sequence_id `coord:deadline_missed` atom matching the slot) and per-slot `last_close` (highest-sequence_id successful event with `event_type == slot.expected_event_type AND subject_role == slot.subject_role`). For each slot: if `last_miss` exists AND (`last_close` is null OR `last_close.sequence_id < last_miss.sequence_id`), the slot is uncleared and appears in the response. The durable atom log is the sole source of truth — no in-memory `last_miss_clear_watermark` map. Cost: O(coord-atom-count) per call.
  - **V1 performance bound (perf-fixture-only — no runtime warning)** (r5 codex-ops F2 MED + r6 codex F1 MED + r6 codex-ops F1 MED + r6 codex-ops F2 MED): the substrate's append-only storage means coord-atom volume grows without trim (storage has no retention/trim mechanism — the earlier "bounded retention" wording was wrong and is removed). V1 sizing target: at 057b-era volume (~3k coord atoms/day = ~1.1M/year, ~100k after ~34 days), the full-replay reconstruction at boot AND the full-scan in `coord_status()` must complete within operational budgets. **Falsifiable AC8 perf fixture IS the operational contract** (r5 codex-ops F2 acceptable option (a) — "perf/observability contract OR startup/status warning"; V1 picks the perf-fixture path only): synthesize 100k coord atoms in the ledger; assert reconstruction at boot completes in <1500ms on dev hardware AND one `coord_status()` call completes in <300ms. **No runtime warning mechanism in V1** (r6 codex F1 + r6 codex-ops F1+F2 convergent — the r5 warning patch used `getCurrentCoordSequence()` as a coord-row count which it isn't (rowid is shared with non-coord events; sparse-coord/busy-capture ledgers would false-fire on first coord atom), and it emitted a `coord:scheduler_health` atom which AC1 defines as a scheduler-tier self-attestation that opens a `scheduler_health_done` deadline no wrapper would close; both bugs argued against shipping the warning at V1). V1.5+ adds the proper observability path designed around (a) a coord-row-count primitive (not rowid watermark) AND (b) a non-deadline-opening warning atom shape AND (c) visibility in `coord_status()` operator surface — all three are V1.5+ scope, NOT V1.
...
- `tests/coord/coord-volume-perf.test.ts` (r5 codex-ops F2 MED + r6 warning-mechanism-dropped + r7 convergent codex F1 MED + r7 codex-ops F1 MED — merge-blocking enforcement of the V1 perf-only operational contract; the r7 finding was that this entry existed in `files_to_modify` and AC6 prose but was absent from the AC8 merge-blocking inventory) — synthesize 100k coord atoms in the ledger; assert reconstruction at boot completes in <1500ms on dev hardware; assert one `coord_status()` call completes in <300ms. NO startup-warning log assertion AND NO warning-atom-emission assertion (the r5 warning mechanism was dropped in r6; perf-fixture-only is the V1 operational contract).
