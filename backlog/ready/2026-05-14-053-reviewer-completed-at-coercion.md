...
- **AC3.2 — End-to-end pipeline test in an isolated temp git repo (codex-ops R1 F3 + codex/codex-ops R2 hermetic-runtime hardening).** Construct a synthetic `<reviewer>.md` whose frontmatter contains `completed_at: 2026-05-12T23:56:42Z` (UNQUOTED), with all other required fields valid. The test MUST run the actual reviewer-side pipeline (`validate_response_yaml.py <path>` → `commit-reviewer-response.sh` → `combine.py`) inside a **hermetic temp git repo** with a **stubbed local "origin"**.
  ...
  - **Production-repo untouched assertion (load-bearing — codex R2 F1 + codex-ops R2 F4) — required shape:**
    1. **Pre-test snapshot** (captured outside any `try`, BEFORE the pipeline starts): `PROD_HEAD_PRE="$(git -C ~/Desktop/Project_echo rev-parse HEAD)"`; `PROD_STATUS_PRE="$(git -C ~/Desktop/Project_echo status --porcelain)"`; `PROD_REMOTE_MAIN_PRE="$(git -C ~/Desktop/Project_echo ls-remote origin refs/heads/main | awk '{print $1}')"` — capture the SHA of `refs/heads/main` on the REAL github.com remote (not a fixture-branch pattern — `push-with-retry.sh` pushes `origin main`, NOT a fixture-named branch, so the guard MUST be against the actual `main` ref that the helper would touch). **Non-empty 40-hex validation (codex R3 F3):** after capturing `PROD_REMOTE_MAIN_PRE`, immediately assert it is a non-empty 40-character lowercase hex string (`^[a-f0-9]{40}$`). If empty (network failure, ls-remote silent failure, awk pipeline silently dropped the value), ABORT the test setup with a clear "AC3.2 cannot verify production-remote safety — pre-snapshot ls-remote returned empty; refusing to run" message. **Without this validation, a transient ls-remote failure produces `empty == empty` in the post-test guard and silently passes the test even if a real push leaked.** Acceptable alternative implementation (codex R4 F2 + codex R5 F1 — Node API spelled correctly AND API-appropriate success-check). Two viable shapes:
  - **`execFileSync` shape** (preferred for terse positive-path code):
    ```js
    let stdout;
    try {
      stdout = execFileSync('git', ['-C', path.join(os.homedir(), 'Desktop/Project_echo'), 'ls-remote', 'origin', 'refs/heads/main'], { encoding: 'utf8' });
    } catch (err) {
      throw new Error('AC3.2 cannot verify production-remote safety — ls-remote failed; refusing to run: ' + err.message);
    }
    const sha = stdout.split(/\s+/)[0] || '';
    assert(/^[a-f0-9]{40}$/.test(sha), `AC3.2 pre-snapshot SHA invalid: ${JSON.stringify(sha)}`);
    ```
    Rationale: `execFileSync` returns stdout on success and THROWS on non-zero exit / signal — there is NO `status` / `signal` field on the success return. The try/catch IS the success-or-throw check; the only remaining assertion is the non-empty 40-hex match.
  - **`spawnSync` shape** (preferred when status/signal need to be inspected directly):
    ```js
    const result = spawnSync('git', ['-C', path.join(os.homedir(), 'Desktop/Project_echo'), 'ls-remote', 'origin', 'refs/heads/main'], { encoding: 'utf8' });
    assert(result.status === 0 && result.signal === null, `AC3.2 ls-remote failed: status=${result.status} signal=${result.signal} stderr=${result.stderr}`);
    const sha = (result.stdout || '').split(/\s+/)[0] || '';
    assert(/^[a-f0-9]{40}$/.test(sha), `AC3.2 pre-snapshot SHA invalid: ${JSON.stringify(sha)}`);
    ```
    Rationale: `spawnSync` does not throw on non-zero exit; the returned object carries `status` / `signal` / `stdout` / `stderr` for direct inspection.

In both shapes: `~` is NOT shell-expanded (no shell is invoked), so `os.homedir()` / `process.env.HOME` MUST be used to resolve the home directory. The 40-hex assertion is required in both shapes.
  ...
  - **Pipeline assertions:** every stage exits 0; **no quarantine triggered (codex R4 F1 — anchored on the helper's actual mechanism, not a non-existent path):** assert (a) the round directory `$CHECKOUT/backlog/reviews/<fixture-item-id>/r1/` contains NO `<reviewer>.md.invalid.*` sibling files (the helper's `mv <reviewer>.md <reviewer>.md.invalid.<ISO-ts>` quarantine rename, per `commit-reviewer-response.sh` lines 77-84), AND (b) `$CHECKOUT/raw/internal/queue-errors.md` either does not exist OR contains no new `VALIDATION-FAIL` row (compare against a pre-pipeline snapshot if the file pre-exists in the fixture; under the standard hermetic setup the file will not exist pre-pipeline, in which case its post-pipeline absence is the assertion). The old "no file appears under `raw/internal/quarantine/`" formulation is REMOVED — that directory is not written by the helper and asserting its absence is vacuous. Also: the response file lands at the expected path in the temp checkout's `backlog/reviews/<fixture-item-id>/r1/<reviewer>.md`; `combine.py`'s output is well-formed (frontmatter parses, `combined_verdict` field present and is a valid value).
