...
- **AC3.2 — End-to-end pipeline test in an isolated temp git repo (codex-ops R1 F3).** Construct a synthetic `<reviewer>.md` whose frontmatter contains `completed_at: 2026-05-12T23:56:42Z` (UNQUOTED), with all other required fields valid. The test MUST run the actual reviewer-side pipeline (`validate_response_yaml.py <path>` → `commit-reviewer-response.sh` → `combine.py`) inside a **hermetic temp git repo** with a **stubbed local "origin"**:
  - **Test repo setup:** `mktemp -d` a working directory; `git init` a fresh repo; `git -c init.defaultBranch=main init --bare <tmpdir>/origin.git`; `git remote add origin <tmpdir>/origin.git`; copy the minimum subset of `tools/review-queue/` + `tools/review-queue/schemas/` into the test repo so the helpers are runnable in place; copy `backlog/reviews/<fixture-item-id>/r1/request.md` into the expected location with a fixture spec_commit_sha.
  - **Push-stub contract:** the test MUST stub `push-with-retry.sh` (or set an env var that `commit-reviewer-response.sh` reads) so the push targets the temp-local `origin.git`, NEVER the founder's `github.com/zhenye0616/ECHO` remote. If the existing helper does not support an override env var, AC3.2 may either (a) put a stub `push-with-retry.sh` earlier on `PATH` in the test env, OR (b) require a small, additive `--remote=<url>` flag on `push-with-retry.sh` (treated as in-scope for this spec ONLY if the override env-var path is not viable; document the decision in the test file's header comment).
  - **Production-repo untouched assertion (load-bearing):** before the test runs, capture `git rev-parse HEAD` and `git status --porcelain` in the founder's production repo. After the test completes (success or failure), re-capture both values; assert they are byte-identical to the pre-test snapshot. Also assert `git ls-remote origin <branch-pattern>` (against the real github.com remote) shows no new refs matching a test-fixture name pattern. Any deviation FAILS the test loudly with a clear "production repo was modified by AC3.2 test" message.
  - **Pipeline assertions:** every stage exits 0; no quarantine is triggered (no file appears under `raw/internal/quarantine/` in the temp repo); the response file lands at the expected path in the temp repo's `backlog/reviews/<fixture-item-id>/r1/<reviewer>.md`; `combine.py`'s output is well-formed (frontmatter parses, `combined_verdict` field present and is a valid value).
