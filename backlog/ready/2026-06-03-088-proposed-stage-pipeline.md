...
- **AC4 — automated, idempotent promotion.** The watcher, at convergence, stamps `ready_content_sha`
  ...
  - **Pre-promotion content-identity gate (r2 codex-ops HIGH).** `ready_content_sha` stamped on the
    *current* `proposed/` file only protects content *after* promotion; it never proves the current
    ...
    (the spec was edited after the request but before terminalization), REFUSE promotion: leave the
    item in `proposed/`, write an operator-visible `queue-errors.md` row, and dispatch a fresh
    verification round (the changed content must be re-reviewed — promotion never certifies unreviewed
    bytes). AC8 covers the edited-after-request mismatch-refuses case.
