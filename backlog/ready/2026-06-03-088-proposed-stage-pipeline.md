...
files_to_modify:
  ...
  - tools/review-queue/dispatch-next-round.py        # AC4 (r5 codex MED) — ENFORCE the path-(c) cut IN CODE, not just prose (per drift-prevention "enforced in code, not by policy"). Today this helper routes verdict=proceed_after_patches + --patches-applied=false into branch (c) (next_round:null). It must REJECT branch (c) for a PROPOSED-stage artifact (force path b / a verification round) while PRESERVING branch (c) for non-proposed items. Detect proposed-stage via the artifact's folder/stage. Without this, the r4 cut is prose-only and a patched proposed spec can still terminalize, then get stuck at promote.py's content-identity refusal.
...
- **AC4 — automated, idempotent promotion.** The watcher, at convergence, stamps `ready_content_sha`
  ...
  - **No verification-waived promotion after a content patch (r4 codex MED — structural cut).** The
    ...
    **Enforced in code (r5 codex MED):** the cut is NOT prose-only — `dispatch-next-round.py` rejects
    branch (c) for a proposed-stage artifact (forces path b), so a patched proposed spec can never
    terminalize via the waiver branch in the first place. promote.py's content-identity refusal is the
    backstop, not the primary enforcement. Branch (c) is preserved for non-proposed-stage items.
...
- **AC8 — tests green.** `tools/test_blocked.py` (the SHIPPED 086 harness, reworked — r1 codex MED):
  ...
  --check). `dispatch-next-round.py` (r5 codex MED): a PROPOSED-stage artifact with
  verdict=proceed_after_patches + --patches-applied=false must NOT take branch (c) (it rejects or
  forces path b), while a non-proposed artifact still takes branch (c) — pin both. Full `npm test` +
  lint + typecheck + `tools/sync-skills.sh --check` + `python3` test runner green.
