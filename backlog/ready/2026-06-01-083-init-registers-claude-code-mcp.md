...
4. **Idempotent — but verify the URL, don't blind-swallow the duplicate.** Both r1 reviewers verified under a fake HOME that the installed `claude` CLI exits **1** on a duplicate (`MCP server echo already exists in user config`) AND leaves the *existing* URL in place. So "already exists" must NOT be treated as silent success: on the duplicate path, read the current entry (`claude mcp get echo`) and (a) if its URL already equals the resolved `mcpServerUrl`, treat as idempotent success; (b) if it differs (e.g. a prior `--port`/`ECHO_MCP_PORT` install left a stale URL), remove + re-add (`claude mcp remove echo -s user` then re-add) so Claude isn't left pointed at the wrong daemon while `init` reports success. A reconcile failure is the one duplicate case that IS surfaced (remediation line), not swallowed.
...
- **AC3 — best-effort + idempotent + bounded spawn.** (a) `claude` missing on PATH does NOT fail `init` (clear remediation, exit success for the registration step). (b) **Duplicate/stale-URL reconciliation** per Locked decision #4: on the CLI's exit-1 `already exists`, verify the existing user-scope URL via `claude mcp get echo`; if it matches the resolved `mcpServerUrl`, idempotent success; if it differs, remove + re-add so Claude points at the right daemon (a builder must NOT treat only exit 0 as idempotent and swallow the exit-1 duplicate). (c) **Bounded, non-interactive spawn:** the `claude mcp add`/`get`/`remove` calls run under a timeout (reuse the 30s probe precedent), with stdin ignored (non-interactive) and bounded stdout/stderr captured for remediation — a present-but-blocked `claude` (interactive prompt, config lock, slow vendor startup) must NOT hang `echoctl init --answer-file` or the foreign smoke run before it reaches probe/doctor/remediation. Tests (fake `claude` shim): (i) exit-1 with a *stale* URL → reconciled to the resolved URL; (ii) a `claude` that never exits → timeout path fires, init continues.
