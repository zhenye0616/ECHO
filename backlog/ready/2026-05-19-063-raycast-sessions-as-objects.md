...
- **AC6.7** LocalStorage merge-by-id with Partial-patch contract (r2 codex-ops F1; tightened r3 per codex F1 + codex-ops F1/F2):
  - All record* helpers take `Partial<Session>` patches scoped to their specific field set, NOT full Session-shape overrides. This prevents lifecycle regression (r3 codex-ops F2): a late queued `recordSessionUpdate` cannot overwrite the `status="done"` written by an earlier `recordSessionEnd`.
    - `recordSessionStart(initial: Session)` — writes the full initial row (this is the only full-row write).
    - `recordSessionUpdate(id, { answer, auditCalls })` — patches ONLY these two fields. Cannot touch lifecycle fields.
    - `recordSessionEnd(id, { completedAt, status, sourceBreakdown, evidenceClusters })` — patches lifecycle + derived audit fields.
  - **Monotonic status precedence:** if the existing row's `status` is in `{done, errored, cancelled, historical}` (terminal states), any subsequent write that would set `status` to `running` is dropped. The merge helper inspects the EXISTING row's status before applying. This is the second line of defense beyond the Partial-patch field-scoping above.
  - **auditCalls merge by client-side composite key** `(ts, tool, args_shape)` — NOT by call `id`, because the daemon's public `/mcp/recent-calls` payload strips the internal `id` (see `src/mcp/request-log.ts:120-128` `publicClone()`; `tools/raycast-echo/src/lib/audit.ts:3-10` type) and OoS #4 forbids amending the daemon contract. The composite key is sufficient in practice because two distinct calls with identical `ts` (millisecond resolution) + identical `tool` + identical `args_shape` are extraordinarily rare; if they collide, the outcome is one merged audit row showing the later-arriving status — a UI nuisance, not data loss. **Pending → terminal transition is a row UPDATE under the composite key**, not a row ADD: the same call appears first with `status="pending"` then with `status` in `{ok, error}`; the merge selects the row with higher `status` precedence (terminal > pending; tie-break by max `duration_ms`).
  - **Implementation:** every record* helper calls `mergeAndWrite(id, patch)` which re-reads `echo.sessions.v1`, finds the row by `id`, applies the patch (field-scoped, monotonic-status-checked, auditCalls-composite-keyed), then writes the result. Eviction (AC6.5) runs AFTER the merge.
  - **Two overlapping Raycast extension processes** (the original codex-ops r2 F1 scenario) are now safe: each writer only touches its own row, with field-scoped patches that cannot clobber another writer's row even if they share a stale React snapshot.
...
- **AC8.4** Total project test count: ≥57 (current 32 + at least 25 new across AC8.1–AC8.10; AC8.10 expanded to 4 tests in r3).
...
- **AC8.10** `test/sessions.test.ts` (additional, AC6.7 merge contract, r3-tightened): 4 tests minimum:
  - **(a)** Two concurrent `recordSessionUpdate` calls (simulated by holding a stale snapshot then writing) both preserve their target rows after merge.
