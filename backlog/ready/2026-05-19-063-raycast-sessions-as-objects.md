...
- **AC6.4** `recordSessionEnd` is called on subprocess exit (clean, cancelled, or errored). **Final-flush ordering with AWAITED async semantics (r5 codex-ops F1; r6 codex F2 + codex-ops F1):** the runner's exit handler MUST execute these steps IN ORDER, and each `LocalStorage` write MUST be `await`-ed (Raycast `LocalStorage.setItem` is async; non-awaited calls fire-and-forget and can interleave to cause the same final-flush race AC6.4 is trying to close):
  1. Cancel any pending debounced flush timer (so a later debounced write cannot race past the end-write).
  2. `await recordSessionUpdate(id, { answer: finalAnswer, auditCalls: finalAuditCalls })` — guarantees the row contains the FINAL answer body + FINAL auditCalls slice. The mid-run debounced writes may have lagged the actual subprocess output by up to 80ms; this final flush closes that gap. `recordSessionUpdate` returns `Promise<void>`; the await is load-bearing.
  3. `await recordSessionEnd(id, { completedAt, status, sourceBreakdown, evidenceClusters })` — writes lifecycle + derived audit fields via `mergeRowAndWrite`. `recordSessionEnd` returns `Promise<void>`; the await ensures the terminal write lands AFTER step 2's final answer write under Raycast's async LocalStorage.
  After step 3 resolves, the row is terminal: subsequent debounced flushes (none should exist after step 1) would be dropped by the monotonic-status precedence rule (AC6.7) anyway. **Type signatures (r6 codex F2):** `recordSessionUpdate`, `recordSessionEnd`, `mergeRowAndWrite` MUST all return `Promise<void>`; the exit handler MUST be `async` and use `await` for each step; non-awaited fire-and-forget calls FAIL typecheck because the runner's exit handler is declared `async () => Promise<void>` and TypeScript's strict mode flags floating promises. AC8.12 (new) covers the "subprocess exits BEFORE the debounced interval fires" race: the test simulates a subprocess that emits final output then exits within 20ms (well under the 80ms debounce), AND uses a delayed-async LocalStorage mock (each setItem resolves on the next macrotask, NOT synchronously) to PROVE the await semantics. The test asserts: row contains the final answer + final auditCalls AND status="done"; the final-update setItem promise resolved BEFORE the terminal-end setItem promise was issued.
...
- **AC8.4** Total project test count: ≥66 (current 32 + at least 34 new across AC8.1–AC8.13; r6 added AC8.12(c) async-mock test + AC8.13 delete-conditional tests).
