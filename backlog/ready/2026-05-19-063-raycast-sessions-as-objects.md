...
- **AC4.2** Metadata sidebar (Raycast `Detail.Metadata`) shows: **Run** (agent, model where derivable, MCP-call count, duration, status), **Evidence used** (source-breakdown chips — BEST-EFFORT; omitted when not derivable from current `/mcp/recent-calls` result_shape per r1 codex F2), **Sources** (top-5 source paths or commit SHAs — BEST-EFFORT; omitted when not derivable, since the daemon's current request-log projects counts and content-lengths only, NOT source paths / SHAs / atom IDs / source_breakdown — see OoS #9 for the daemon-enhancement follow-up), **Subprocess log** — see fallback rules below. If `subprocessLogPath === null` (log creation failed at startAgent time), render "Log unavailable — agent-runner emitted no path". If `subprocessLogPath !== null`, attempt `fs.statSync(subprocessLogPath)`; on success, render `path + bytes + [Open]` (`Action.Open`) + `[Tail]` (push a Detail view streaming the tail). On `fs.statSync` failure (per r2 codex-ops F3: ENOENT / EACCES / EPERM / EIO from user cleanup, permission changes, stale LocalStorage rows pointing at logs that no longer exist), render "Log unavailable at `<path>` — `<error.code>`" (path preserved for diagnostics) and DISABLE the `[Open]` / `[Tail]` actions (gray out — do NOT remove from the panel, so the user sees the action existed). Calling `fs.statSync` MUST be wrapped in try/catch at the SessionDetail render path; an uncaught throw must not take down the detail view or the parent SessionsList. Test (AC8.3 + AC8.9): SessionDetail renders the fallback string for both `null` path AND a non-null path that doesn't exist; actions are disabled; the row is still navigable.
...
- **AC6.7** LocalStorage merge-by-id (r2 codex-ops F1): every `recordSessionStart` / `recordSessionUpdate` / `recordSessionEnd` write MUST re-read `echo.sessions.v1`, merge the incoming row by `id` (last-write-wins on per-field basis: incoming non-null overrides; `auditCalls` array union by `id` preserves both writers' calls; `answer` takes incoming verbatim — the row author is the source of truth for its own answer), then write the merged array. Eviction (AC6.5) runs AFTER the merge. This prevents two overlapping Raycast command instances (each holding a stale `sessions` snapshot in React state) from overwriting each other's rows on flush. The implementation is a single `mergeAndWrite(updates: Session[])` helper called from all three record* functions.
...
- **AC8.4** Total project test count: ≥55 (current 32 + at least 23 new across AC8.1–AC8.10).
...
- **AC8.9** `test/session-detail.test.tsx` (additional, AC4.2-specific log-stat fallback): 2 tests minimum — SessionDetail with non-null `subprocessLogPath` pointing at a non-existent file renders "Log unavailable at <path> — ENOENT" with `[Open]`/`[Tail]` disabled (not removed); the same row is navigable from SessionsList (no render throw); the path is preserved verbatim in the rendered text.
- **AC8.10** `test/sessions.test.ts` (additional, AC6.7 merge-by-id): 2 tests minimum — two concurrent `recordSessionUpdate` calls (simulated by holding a stale snapshot then writing) both preserve their target rows after merge; the `auditCalls` array from two writers is unioned by call `id` with no duplicates and no dropped entries.
