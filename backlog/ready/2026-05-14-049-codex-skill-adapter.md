...
- Add `tests/sync-skills/install-codex-adapters.test.ts` (vitest, addresses codex F2 + codex-ops F3 from R1):
  ...
  - **`parse-failure preserves evidence outside RUN_DIR cleanup`** (addresses R7 codex-ops F1 MED): Fixture child emits markdown MISSING the `Verdict` heading; orchestrator runs parse → fails → asserts (a) queue-errors row names the missing heading, (b) `raw/internal/queue-errors/<ISO-ts>-review-pending-<item-id>/` exists post-cleanup AND contains the preserved `review.md` + `stdout` + `stderr` triple, (c) `RUN_DIR` itself was cleaned (no leak).
...
- Add `tools/install-codex-adapters.sh` (executable, mode 0755):
  ...
  - **Per-target lock applies to BOTH `--symlink` and `--copy` modes** (addresses R5 codex F3 + codex-ops F1 HIGH): the atomic-mkdir lock at `$HOME/.codex/.echo-locks/<name>` is acquired BEFORE the target-classification probe for BOTH install modes. Lock directory contains `pid` + `timestamp` files written immediately after acquisition (addresses R6 codex-ops F2 MED — stale lock recovery): `mkdir "$LOCK" && echo $$ > "$LOCK/pid" && date -u +%s > "$LOCK/timestamp"`. Trap removes lock on EXIT. **Stale-lock recovery** (addresses R6 codex-ops F2 MED + R7 codex-ops F2 MED — corrupted-timestamp fallback): if `$LOCK` exists, pre-flight tries to read `$LOCK/timestamp`. If readable AND `now - timestamp > 600` seconds (10 min), lock is stale — pre-flight removes it with warning. If `$LOCK/timestamp` is MISSING, NON-INTEGER, or unreadable (crash between `mkdir "$LOCK"` and the timestamp-write would leave this shape), FALL BACK to `$LOCK`'s directory mtime — if mtime > 600s old, treat as stale + remove + warn. Reading non-existent / non-integer timestamps under `set -euo pipefail` MUST NOT terminate the script — wrap the read in `[ -f "$LOCK/timestamp" ] && timestamp=$(cat "$LOCK/timestamp" 2>/dev/null) || timestamp=""`; if `[[ ! "$timestamp" =~ ^[0-9]+$ ]]`, fall through to mtime path. AC3 test covers BOTH stale-lock shapes: (a) lock with old readable timestamp; (b) lock with missing/corrupted timestamp but old mtime — both must recover, install must proceed.
...
- **Stale-copy detection** (addresses R7 codex-ops F3 MED): each `--copy`-installed `~/.codex/skills/<name>/.echo-managed` sentinel file ALSO contains the source-canonical-skill SHA at install time (e.g., `echo "synced_from_commit=$(git -C $REPO_ROOT rev-parse HEAD)" >> "$STAGE/.echo-managed"`). `tools/sync-skills.sh --check` reads the sentinel and emits a warning (not an error — install-step concern, not sync-step) if any installed `--copy` adapter's `synced_from_commit` differs from the canonical's current commit. Lets the operator see "your copy-mode install is stale; re-run install-codex-adapters.sh --copy to refresh."
