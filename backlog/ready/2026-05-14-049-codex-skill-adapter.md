...
- Add `tools/install-codex-adapters.sh` (executable, mode 0755):
  ...
  - **For each `adapters/codex/skills/<name>/` directory** in the in-scope set (matches AC1 — only skills with codex notes get installed), after target-probing clears the path:
    ...
    - `--copy` mode (crash-safe staged-then-rename, addresses R3 codex F2 MED): Stage the copy in `$HOME/.codex/skills/.staging-<name>-$$` (PID-suffixed for parallel-install safety), write the `.echo-managed` sentinel INTO the staging directory FIRST, then `cp -R` content into the staging directory, then atomically `mv` the staging directory to its final name `$HOME/.codex/skills/<name>`. Concretely: `STAGE="$HOME/.codex/skills/.staging-<name>-$$"; mkdir "$STAGE" && touch "$STAGE/.echo-managed" && cp -R "$REPO_ROOT/adapters/codex/skills/<name>/." "$STAGE/" && mv "$STAGE" "$HOME/.codex/skills/<name>"`. If the process dies anywhere mid-sequence, the partial state lives in the `.staging-*` path (clearly orphaned, never seen as a "real" skill by codex) and the target either doesn't exist OR is the pre-existing managed entity that was probed earlier. Trap: `trap 'rm -rf "$STAGE" 2>/dev/null' EXIT` to clean orphaned staging on crash. Idempotent retry: next install run probes the target (sees stale absence or pre-existing managed entity), no partial-overwrite ambiguity.
