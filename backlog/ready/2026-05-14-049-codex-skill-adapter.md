...
- Add `tools/install-codex-adapters.sh` (executable, mode 0755):
  ...
  - **For each `adapters/codex/skills/<name>/` directory** in the in-scope set (matches AC1 — only skills with codex notes get installed), after target-probing clears the path:
    ...
    - `--copy` mode: `cp -R "$REPO_ROOT/adapters/codex/skills/<name>" "$HOME/.codex/skills/<name>" && touch "$HOME/.codex/skills/<name>/.echo-managed"`. The sentinel marker is written immediately after copy so future re-runs identify the directory as managed.
