...
- Add `## Binding-specific notes — codex` section adjacent to the Claude Code one. **Operationally-safe fan-out contract** (addresses codex-ops F2 from R1, and codex F1+F2 + codex-ops F1+F2 from R2):
  ...
  - **Output format (markdown, matching Claude path — addresses R5 codex F1 HIGH)**: each per-item codex exec emits the same MARKDOWN review format that Claude Code's `superpowers:code-reviewer` subagent produces (sections: `Verdict`, `Acceptance status`, `Drift findings`, `Bugs/risks`, `Suggested fixups`, `Test counts observed`). Orchestrator extracts sections by regex match on those headings, not JSON parse. Originally this AC prescribed a JSON output envelope, but the schema was never pinned and standard codex output is markdown — forcing JSON without a `--output-schema` contract was an under-specified requirement that a builder couldn't satisfy. Parse failure (e.g., missing `Verdict` section in child stdout) = hard per-item failure: log to queue-errors with the offending child's stderr+rc; don't silently drop or substitute defaults.
...
- Add `tools/install-codex-adapters.sh` (executable, mode 0755):
  ...
  - **Per-target lock applies to BOTH `--symlink` and `--copy` modes** (addresses R5 codex F3 + codex-ops F1 HIGH): the atomic-mkdir lock at `$HOME/.codex/.echo-locks/<name>` is acquired BEFORE the target-classification probe for BOTH install modes. Trap removes lock on EXIT. This serializes probe → remove → install (symlink OR stage-mv) against concurrent installer processes for either mode, eliminating both the --symlink probe-to-ln race and the --copy probe-to-stage-mv race.
