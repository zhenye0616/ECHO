...
- **J4 — RESOLVED (r1→r3):** the card-open / A1 / close predicates are exact and durable, built ONLY from `combined.md` frontmatter (`escalated_to_founder`, `next_round`) + the review-active backlog dir (`ready/`|`claimed/`|`pending_review/`). **No convergence-call/body-prose parsing and no `r<N+1>/request.md`-presence predicate** (both were stale r2 framing — corrected per r3 codex/codex-ops). A1 resets on `escalated_to_founder: true`, never on `next_round`. See AC2/AC3.
