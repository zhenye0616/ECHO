...
files_to_modify:
  ...
  - src/storage/interface.ts                       # add iterateCoordAtomsByAppendOrder({sinceSeq?, untilSeq?, limit?}) returning atoms in durable append order with sequence_id (r2 codex F1 HIGH — current QueryFilter has no append-order primitive; EventId is random UUID; SQLite orders by (timestamp, id) not durable ingest order); ALSO add getCoordSequenceAtOrAfter(timestamp) and getCurrentCoordSequence() so reconstruction + reconciliation can be expressed entirely in terms of declared methods (r3 codex F1 MED + r3 codex-ops F1 HIGH — the r2 single-method seam left time→seq and next-seq reads as implicit operations)
  - src/storage/sqlite.ts                          # implement all three coord-seam methods; iterate via rowid (SELECT rowid AS sequence_id ... FROM events WHERE source LIKE 'coord:%' AND rowid > ? AND rowid <= ? ORDER BY rowid LIMIT ?; r3 codex F3 LOW — table is `events` not `atoms`); getCoordSequenceAtOrAfter binary-searches on (emitted_at, rowid); getCurrentCoordSequence reads max(rowid)+1
  - src/storage/memory.ts                          # implement all three coord-seam methods using monotonic insertion counter; getCoordSequenceAtOrAfter binary-searches the insertion list; getCurrentCoordSequence reads the counter
...
- **Durable append-order storage seam** (r2 codex F1 HIGH + r2 codex-ops F6 MED convergent): both reconstruction and periodic reconciliation order events by **durable append sequence**, NOT by caller-supplied `emitted_at`. `emitted_at` is canonicalized but originates client-side and is vulnerable to clock skew or delayed delivery; using it as the replay ordering primitive lets out-of-order arrivals (e.g. `tick_start` with an earlier `emitted_at` than `reviewer_invoked`) corrupt the rebuilt open-record set. AC3 therefore extends the storage interface:
  - **Three explicit storage methods** at `src/storage/interface.ts` (r3 codex F1 MED + r3 codex-ops F1 HIGH convergent — the prior r2 single-method seam left `time→seq` conversion and `next-seq` reads as implicit operations the algorithm relied on but the interface did not declare; r3 codex F3 LOW — SQL example used the wrong table name):
    1. **`iterateCoordAtomsByAppendOrder(opts: { sinceSeq?: number; untilSeq?: number; limit?: number })`** — returns coord atoms in monotonic durable-append order, paginated. Each yielded record carries its `sequence_id` so callers can record watermarks.
    2. **`getCoordSequenceAtOrAfter(timestamp: ISOZ): number | null`** (NEW r3 — r3 codex F1 MED + r3 codex-ops F1 HIGH) — returns the smallest coord-atom `sequence_id` whose canonicalized `emitted_at >= timestamp`, or `null` if no such atom exists. This is how reconstruction converts the 24h time-horizon bound into a `sinceSeq` value without scanning the full ledger. SqliteStorage binary-searches by `(emitted_at, rowid)` ordering on the `events` table (existing index covers this); MemoryStorage binary-searches its insertion list (the entries are already sorted by insertion order, and `emitted_at` is canonicalized at append time, so the search is well-defined).
    3. **`getCurrentCoordSequence(): number`** (NEW r3 — r3 codex F1 MED + r3 codex-ops F1 HIGH) — returns the next `sequence_id` that live ingest WILL assign (`max(rowid) + 1` over all currently-durable atoms, equivalently a snapshot of the rowid counter). This is how periodic reconciliation captures `highSeq` without inventing private SQLite APIs.
  - **SqliteStorage** uses the SQLite `rowid` (monotonic per insertion, durable across restart) as `sequence_id`. Iteration query: `SELECT rowid AS sequence_id, * FROM events WHERE source LIKE 'coord:%' AND rowid > ? AND rowid <= ? ORDER BY rowid LIMIT ?` (r3 codex F3 LOW — table is `events`, not `atoms`; the r2 sketch used the wrong table name). The single-writer constraint (`wiki/architecture/storage.md:119-127`) guarantees `rowid` reflects ingest order.
  - **MemoryStorage** (test/dev) uses a monotonic insertion counter — append assigns `_seq = ++counter`; iteration returns entries with `_seq` in `(sinceSeq, untilSeq]`. `getCoordSequenceAtOrAfter` binary-searches the insertion list; `getCurrentCoordSequence` reads the counter.
  ...
  - Parity test in `tests/storage/iterate-coord-by-append-order.test.ts` (extended r3): same-timestamp coord atoms ingested in `[A, B, C]` order replay as `[A, B, C]` from both backends; range `(2, 4]` returns only `[B, C]` from a 5-atom backend; out-of-order `emitted_at` does NOT affect append-order iteration; **`getCoordSequenceAtOrAfter` parity — given an injected timestamp boundary, both backends return the same `sequence_id`; null when no atom exists past the boundary**; **`getCurrentCoordSequence` parity — both backends agree on the next-seq value after N appends**.
- **Reconstruction on daemon boot** runs as a single queued task on the lane BEFORE the MCP server accepts new `coord_emit` calls (a hard startup gate; r1 codex-ops F6 HIGH eliminates the concurrent-emit-during-boot race). Algorithm in terms of the three declared storage methods (r3 codex F1 MED + r3 codex-ops F1 HIGH): (1) compute `horizonStart = now - max-deadline-horizon` (24h V1; extend if any role's `max_deadline_sec > 86400`); (2) call `sinceSeq = getCoordSequenceAtOrAfter(horizonStart)` — if `null`, no coord atoms in the horizon and the reconstruction terminates with an empty rebuilt set; (3) iterate `iterateCoordAtomsByAppendOrder({ sinceSeq })` in pages; (4) populate the idempotency cache from any `coord:deadline_missed` atoms encountered (so post-restart heartbeats do not double-fire across restart); (5) replay the close-then-open transition rule over the remaining atoms **in durable append order** (not `emitted_at` order — r2 codex-ops F6 MED); (6) record `last_full_replay_watermark = max(sequence_id)` of atoms consumed; (7) immediately call `fireMissedDeadline` for any record still open AND past `expected_by` — fired records' open-map entries are removed by the cache-hit-also-terminal branch (r2 codex-ops F1 HIGH). Only after the task returns does `startMcpServer()` start accepting `coord_emit` requests.
- **Periodic reconciliation** (every 10 min) (r1 codex-ops F6 HIGH + r2 codex F1 HIGH + r3 codex F1 MED + r3 codex-ops F1 HIGH): enqueues onto the same lane as a single task. (1) Call `highSeq = getCurrentCoordSequence()` at the moment the task acquires the lane — this is the next sequence ingest will assign, so any atom whose append is in progress sits BEHIND this task in the queue. (2) Iterate `iterateCoordAtomsByAppendOrder({ sinceSeq: last_full_replay_watermark, untilSeq: highSeq })` in pages. (3) Replay the close-then-open rule on those atoms in durable append order (not `emitted_at` order — r2 codex-ops F6 MED). (4) Set `last_full_replay_watermark = highSeq` on completion. Because the task runs on the same serial lane as live ingest and heartbeat, it cannot race; because the scan stops at `highSeq`, atoms appended after the watermark (including `tick_end` events that closed records during the task wait) are processed in their normal ingest position behind the reconciliation task and cannot be "resurrected" by the replay.
...
- **New MCP read tool `coord_status()`** at `src/mcp/tools/coord-status.ts`. Returns:
  ...
  - **Per-role-per-event-type last miss** (r1 codex-ops F7 MED + r2 codex F3 MED + r3 codex F2 MED + r3 codex-ops F2 MED — persistence-of-failure signal, key disambiguated, **durable across daemon restart**): `[{subject_role, expected_event_type, last_missed_at, opened_event_type, key, age_sec}...]`. **The list is keyed by `(subject_role, expected_event_type)`** — i.e. the event the operator was waiting for. This list IGNORES the recent-window horizon: a `deadline_missed` atom older than 24h remains visible until a subsequent successful event with `event_type == expected_event_type` arrives for that `subject_role`. The list is computed by **on-demand rehydration at `coord_status()` call time** (r3 codex F2 MED + r3 codex-ops F2 MED — the in-memory map from the r2 patch is not durable across restart; the boot reconstruction 24h horizon would drop a 48h-old uncleared miss). The rehydration algorithm runs on the serial lane: (1) start from the in-memory `last_miss_clear_watermark` map (one `sequence_id` per `(subject_role, expected_event_type)` recording the latest successful close that has been observed in live ingest; empty at startup); (2) for each `(subject_role, expected_event_type)` slot the daemon has ever observed (kept as a small in-memory set), iterate `coord:deadline_missed` atoms newer than the slot's clear-watermark, picking the most-recent one matching the slot; (3) include the slot in the result if any such atom exists. Cost: O(open-slots × matching-atoms-per-slot) — coord atoms are rare, so this is small (V1 ceiling: 8 roles × 6 event_types × <100 atoms each = trivial). The on-demand path means a 48h-old uncleared miss survives daemon restart because the durable atom log is the source of truth, not in-memory state.
  - **Clearing rule** (unchanged): on every incoming coord event `E` (live ingest), the daemon advances `last_miss_clear_watermark[(E.subject_role, E.event_type)] = E.sequence_id` if `E` is a successful close (i.e. NOT a `coord:deadline_missed` atom and `E.event_type` is in the registry as an expected close). Subsequent `coord_status()` rehydrations using that watermark skip any `deadline_missed` atom with `sequence_id <= watermark`. A successful `tick_start` from `codex` clears the `(codex, tick_start)` slot; a fresh `reviewer_invoked` does NOT clear it. Lets the founder check status in the morning and see "codex hasn't successfully sent `tick_start` since 3 days ago" without running a forensic search — even after a daemon restart.
