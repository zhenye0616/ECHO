...
Removed in 038: `tail_session`. Removed in 2026-05-17 follow-up: `recent_work_context`. Added in 038: `echo_resolve_mru`. Other 6 tools unchanged in identity (`wait_for_new_turns` modified in shape only — bodyless).
...
2. **Output shape — search-ready descriptor per resolved source (R2 correction Codex HIGH #2).** The original R1 spec returned bare source paths; that loses repo/composer filter context, leading to cross-repo leak when `search_memories(source=<resolved>)` is called for Cursor (whose resolved source is the global `state.vscdb`). The R2 fix returns a **search-ready descriptor** per source:
   ```
   {
...
3. **R2 correction — `cursor_phase2 → phase` field semantic (Cursor R2 #5).** The `phase: 'cursor_legacy'` field is set ONLY when Phase 2 returned a **non-null `composer_resolved`** value. Phase 2 attempted-but-empty (resolver returned null) leaves `phase` absent on the descriptor — same shape as 037's AC6 `composer_resolved` invariant after its R1 patch.
...
6. **Tools/list description:** explicitly state this is an IDs-only primitive — bodies are NOT fetched here. Document the canonical composition patterns using the descriptor shape:
   - Tail: `r = echo_resolve_mru({sources: ['cursor'], repo_path: X})` → `desc = r.sources['cursor']` → `search_memories({source: desc.source, ...desc.filter, limit: N})`. The spread `...desc.filter` carries through repo_path or metadata_match cleanly without the caller needing to know which phase fired.
   - Live watch: `wait_for_new_turns({sources: [desc.source], ...desc.filter, since: now})`.
7. Tests: at least 10 unit tests — (each of 4 source_app names as `sources` entries) × {with, without} repo_path = 8 cases; one fresh-Cursor-composer Phase 1 test (asserts `desc.filter.repo_path` is set, `desc.phase` is absent); one legacy-Cursor-atom Phase 2 test (asserts `desc.filter.metadata_match.composer_id` is set, `desc.phase === 'cursor_legacy'`, and `desc.filter.repo_path` is absent — per Cursor R2 #5); one Phase-2-attempted-but-empty test (resolver returned null → descriptor.phase absent — per Cursor R2 #5); one mixed-input-type `sources=['cursor', 'claude_code', 'fs:/Users/.../session.jsonl']` test confirming app-name vs literal-path mechanism split (R2 Codex #4); one validation test (empty `sources` array → isError); one absolute-path validation for `repo_path`; **one end-to-end composition test asserting `echo_resolve_mru → search_memories(source=desc.source, ...desc.filter)` recovers only atoms from the specified repo** (Codex R2 HIGH #2 closure — verifies cross-repo leak is structurally impossible).
...
**Surface:** new internal file `src/mcp/internal/cluster-engine.ts` (canonical home for the cluster discovery engine); rewrite `src/mcp/tools/find-clusters.ts:190` to call the new internal helper directly; turn `src/mcp/tools/recent-work-context.ts` into a thin re-export shim that wraps the new internal engine (≤10 lines) — its MCP-tool registration via `registerTool` STAYS unchanged.
...
5. **R2 correction — re-export shim shape (Cursor R2 #3):** the file `src/mcp/tools/recent-work-context.ts` is rewritten as a **≤10-line re-export shim**. It imports the cluster engine + the helpers it needs from `src/mcp/internal/cluster-engine.ts` and re-exports the same `getRecentWorkContext` function (preserving the MCP tool's registration handler signature). All callers (`find-clusters.ts` and the MCP registration in `server.ts`) continue to work without import-path changes. The 2026-05-17 follow-up then deletes ≤10 lines + the MCP registration in `server.ts`, NOT 340 lines.
...
8. Tests: existing `recent-work-context.test.ts` tests are renamed and re-rooted on the new internal engine — they test the same behavior (auto-expand, demotion, fs-exclusion, **and 037's repo_path forwarding**), just at a different seam. **The shim itself gets ≥1 integration test asserting `getRecentWorkContext` called through the shim produces identical output to calling the internal engine directly** (this is the regression guard that lets the 2026-05-17 follow-up safely delete the shim without re-running the full cluster-engine test suite). `find_clusters.test.ts` gets one additional test asserting the internal engine integration works end-to-end with the same shape, including a regression test for `find_clusters({repo_path: X})` returning identical results to post-037 behavior.
