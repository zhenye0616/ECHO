...
=======
- **`src/coord/deadlines.ts`** maintains an in-memory map of `(correlation_id, subject_role, event_type, expected_by)` open records. **Generic transition rule (r2 codex F2 MED — was implicit, now explicit):** on EVERY incoming coord event `E`:
