...
acceptance:
  ...
  - "**Bug 4 — Pagination cursor on `search_memories`.** Add a `cursor?: string` input (ISO 8601 timestamp). Internally treated as `until`, with a precedence rule: if both `cursor` and `until` are passed, the *earlier* one wins (more restrictive — never widen the window across a paginated cursor). Add `next_cursor: string | null` to `SearchResult`. When `total_returned === limit_applied` AND there are more rows in the storage layer beyond the slice, populate `next_cursor` with the oldest match's timestamp minus 1 ms (so the next call doesn't re-return the boundary atom). When `total_returned < limit_applied`, emit `next_cursor: null`."
  ...
  - "  - Test: seed 60 events; first call returns 50 + a non-null `next_cursor`; second call with that cursor returns the remaining 10 + `next_cursor: null`."
  - "  - Test: `cursor` and `until` together — the earlier wins; `query_echo` records both raw inputs for debuggability."
...
**Schema scoping decision.** Full mirroring of `RecentWorkContextResponse` (clusters, atoms, edges, hints — see `src/trace/types.ts`) would lock the agent into validating fields whose internal contract is still moving (we just shipped 022 last hour and have follow-ups for 026+). Use `z.record(z.unknown())` / `z.array(z.unknown())` for deeply nested cluster/atom/edge bodies; mirror only top-level keys (`clusters`, `atoms`, `query`, `truncation`, `warnings`). Document this scoping inline in `recent-work-context.ts` so the next item that revisits it knows the schema is intentionally permissive. `search_memories` and `echo_ping` are small enough to mirror fully.
...
Add `cursor?: string` (ISO 8601 timestamp) to the input schema. Internally treated as `until`, with the more-restrictive of the two winning when both are passed.

Add `next_cursor: string | null` to `SearchResult`. Populated with `oldest_match.timestamp` minus 1 ms when `total_returned === limit_applied` AND more rows exist beyond the slice. `null` otherwise.
...
- **New: `wiki/surfaces/mcp-server.md`** — promote the V1 MCP surface from `planned` to `shipped`. Cover: three tools (`echo_ping`, `search_memories`, `get_recent_work_context`), `source_app` enum, default cost model (`limit=25`, `format='minimal'`), `next_cursor` pagination, `readOnlyHint` annotations, `outputSchema`+`structuredContent` wire shape. Cross-reference [[capture-gate]], [[storage]], [[interface-layers]] (L3).
