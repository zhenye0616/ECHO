...
- **Change:** `git -c rebase.autoStash=true pull --rebase origin main && git push origin main` becomes `git -c rebase.autoStash=true pull --rebase --rebase-merges origin main && git push origin main`. Single-flag addition. No other change to the script.
...
- **Test:** new shell-driven test `tests/review-queue/push-with-retry-rebase-merges.test.ts` (or `.sh` if the existing test harness supports shell tests — verify by reading `package.json` and existing `tests/review-queue/concurrency.test.ts` BEFORE writing). The test sets up:
  ...
  5. **Asserts:** the merge commit's two-parent shape is preserved on `origin/main` post-push (`git rev-parse origin/main^2` returns non-error and matches the expected feature-branch tip SHA). Without `--rebase-merges`, this assertion fails because the rebase replays the merge as a single linear commit and `^2` returns "unknown revision."
...
- **Lifecycle insertion point:** after the existing `cd "$REPO_ROOT"` + `git rev-parse --git-dir` checks (lines 17–26) and AFTER the `_reviewer_gate.py` validation (line 45), but BEFORE the codex child invocation (line 69). The lock check is wrapper-side, not prompt-side, so the codex child never starts when the lock is held — saving the ~10-30s of codex initialization cost AND eliminating any window where the codex child could race the lock between read and `git add`.
- **Lock file path:** `.git/echo-merge-in-progress` (relative to `$REPO_ROOT`). Resolved via `git rev-parse --git-path echo-merge-in-progress` so it works correctly inside any future worktree (defensive against the case where a builder runs the wrapper interactively from a worktree CWD — not the launchd path, but cheap robustness).
...
- **Risk R2 — `git rev-parse --git-path` resolution inside future worktrees.** If `_run_reviewer.sh` is invoked from inside a worktree (not the launchd codepath, but possible during interactive testing), the lock path resolves to the COMMON `.git/` not the worktree-specific `.git/worktrees/<name>/`. This is the correct behavior — the lock is a repo-level coordination primitive — but operators should know that running `_run_reviewer.sh` interactively from a worktree CWD STILL honors the lock written from the main checkout. Documented here for clarity; no AC.
