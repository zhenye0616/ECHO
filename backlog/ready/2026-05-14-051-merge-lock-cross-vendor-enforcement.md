...
- **Test:** new shell-driven test `tests/review-queue/run-reviewer-honors-merge-lock.test.ts` (or `.sh`):
  ...
  2. **Copy the reviewer prompt fixture** to `<test-repo>/.claude/commands/review-queue-codex.md` (any content — the wrapper's `[ -f "$PROMPT" ]` check at line 64 is the only consumer in the lock-absent path; `printf 'stub prompt\n' > <test-repo>/.claude/commands/review-queue-codex.md` is sufficient). Without this, the lock-absent re-invocation in step 6 hits the existing `tick abort: prompt missing` early-exit BEFORE reaching the stub, defeating the assertion. (R3 codex F1 fix.)
