...
- **Test:** new shell-driven test `tests/review-queue/push-with-retry-rebase-merges.test.ts` (or `.sh` if the existing test harness supports shell tests — verify by reading `package.json` and existing `tests/review-queue/concurrency.test.ts` BEFORE writing). The test sets up:
  ...
  5. **Asserts:** the merge commit's two-parent shape is preserved on `origin/main` post-push — `git rev-parse origin/main^2` returns non-error (asserting the merge is preserved as a merge). Under the buggy `--rebase` default, the rebase replays the merge as a single linear commit and `^2` returns "unknown revision" — that is the load-bearing differential the test detects. **Do NOT assert SHA equality OR tree-content equality with the pre-rebase feature tip** — `--rebase=merges` rewrites the rebased second parent onto the new base, and if the sibling clone's commit lands on `origin/main` first (the test setup's non-fast-forward state), the rebased `^2`'s tree includes that sibling commit's content and will not match the pre-rebase feature tree. The `^2` existence check alone is the falsifying assertion. (R1 codex F1 + F2 + R4 codex F1 lineage: F1 fixed the `--rebase=merges` syntax, F2 introduced the tree-equality assertion as a "less brittle than SHA" alternative, F4-codex showed even tree-equality fails empirically; R4 simplifies to the minimal differential — existence-only.)
...
- **Wrapper change for test determinism (in addition to the lock check above):** add a `CODEX_BIN` environment hook to `_run_reviewer.sh`, mirroring the existing `tools/backlog/run-codex-builder.sh:94` pattern. Change the existing `codex exec -C "$REPO_ROOT" ...` invocation to `"$CODEX_BIN" exec -C "$REPO_ROOT" ...` with `CODEX_BIN="${CODEX_BIN:-codex}"` set near the top of the script (alongside the existing `PATH` export). This is the deterministic injection point for tests; production behavior is unchanged (defaults to the unqualified `codex` resolved through the wrapper's prepended PATH).
- **Test:** new shell-driven test `tests/review-queue/run-reviewer-honors-merge-lock.test.ts` (or `.sh`):
  ...
  2. **Copy reviewer prompt fixtures** to BOTH `<test-repo>/.claude/commands/review-queue-codex.md` AND `<test-repo>/.claude/commands/review-queue-codex-ops.md` (any content — the wrapper's `[ -f "$PROMPT" ]` check at line 64 is the only consumer in the lock-absent path; `printf 'stub prompt\n' > <fixture>` is sufficient). Both fixtures because AC3 parameterizes this test across `REVIEWER_NAME=codex` AND `REVIEWER_NAME=codex-ops`; the wrapper's `_reviewer_gate.py` resolves the slash_command per `reviewers.json`, so `codex` → `review-queue-codex` and `codex-ops` → `review-queue-codex-ops`. Without both fixtures, the parameterized AC3 test variant hits `tick abort: prompt missing` before reaching the stub, defeating the assertion. (R3 codex F1 + R4 codex F2 fix lineage.)
  3. Set `REVIEWER_NAME=codex`, `ECHO_REVIEW_QUEUE_REPO_ROOT=<test-repo>`, `HOME=<test-home>` (for log isolation), and `CODEX_BIN=<test-stub-path>` where the stub is an executable script `printf 'CODEX_INVOKED=true' >> "$STUB_FIXTURE_FILE"` (resolved via the `CODEX_BIN` env hook added by this AC's wrapper change above, NOT via `$PATH` manipulation — `_run_reviewer.sh` prepends `/opt/homebrew/bin:/usr/local/bin:...` to PATH and would shadow a PATH-resolved stub on machines with a real Codex install). (R3 codex F1 fix.)
  4. Invoke `_run_reviewer.sh`.
  5. **Assert:** exit code 0; `$STUB_FIXTURE_FILE` does NOT contain `CODEX_INVOKED=true` (codex was never spawned); `<test-home>/Library/Logs/echo-review-queue-codex.log` contains a `tick skipped: merge in progress` line with the lock holder string `test-id @ <ts> by 12345`.
  6. Remove the lock file. Re-invoke. **Assert:** `$STUB_FIXTURE_FILE` now contains `CODEX_INVOKED=true` (the wrapper proceeds past the gate when lock absent and invokes the stub via `$CODEX_BIN`).
