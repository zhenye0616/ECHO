...
spec_refs:
  ...
  - tools/raycast-echo/src/lib/sessions.ts  # Session shape (lines 8-24, 85-98) — READ-ONLY reference. `since-resolver.ts` reads existing Ask ECHO sessions via the `LocalStorage.allItems()` + key-prefix iteration pattern to find the last `done` Session for the `last_session` window resolver path. The resolver does NOT write any Session row. The Session interface itself is NOT modified per r8 option-F (no `recapWindow` field).
