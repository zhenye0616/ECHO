...
- Create `skills/role-typed-task-state.md` documenting:
  ...
  - **`round-state.md` ownership and write protocol (compare-and-swap against upstream).** Single owner: the **watcher** writes the round-boundary rewrite as part of its post-combine disposition step (skills/review-queue-watch.md AC3.5 (a)/(b)/(c) flow). Strategist updates between rounds (e.g., dispositioning) edit the file. Both writers use the same compare-and-swap protocol, executed exactly in this sequence (codex R2 F1 + codex-ops R2 F6 — the R1 protocol was self-failing because it compared post-commit HEAD blob against the read blob, which by definition differs):
    1. **Read base:** `base_blob = git rev-parse HEAD:backlog/task-state/<task-id>/round-state.md` (capture the blob SHA the writer is editing).
    2. **Compute new content** in a tmp file. Free-form work; no git ops in this step.
    3. **Refresh upstream:** `git fetch origin main` (no rebase yet; just refresh `origin/main` ref).
    4. **Compare-and-swap:** `now_remote_blob = git rev-parse origin/main:backlog/task-state/<task-id>/round-state.md`. If `now_remote_blob != base_blob`, abort: another writer landed between step 1 and now. Append a single `queue-errors.md` entry (`ROUND_STATE_WRITE_CAS_ABORT: <writer> base=<base> remote=<now_remote_blob> ts=<ISO>`), do NOT replace the file, exit non-zero, and let the next tick re-read from the new base.
    5. **Atomic FS swap:** `os.replace(tmp_path, final_path)` (matches the established 040 pattern).
    6. **Commit + push** via `tools/review-queue/push-with-retry.sh`. The helper internally does pull-rebase-on-reject; if rebase introduces a conflict on this file specifically, push-with-retry exits non-zero so the writer can re-run the CAS protocol from step 1 (do NOT auto-resolve the conflict).
