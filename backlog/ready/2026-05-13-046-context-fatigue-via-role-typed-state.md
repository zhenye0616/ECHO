...
- Create `skills/role-typed-task-state.md` documenting:
  ...
  - **`canonical_anchors` body syntax is pinned** as a key:value bulleted list, one anchor per line, in the form `- <key>: <value>`. Allowed keys for V1: `spec` (required), `reviews` (optional). Example:
    ...
    ```
    Parser implementation is **TypeScript-only** in V1 (`src/mcp/parse-anchors.ts`), called from the MCP server's `list_task_states` handler. The lint script (AC2 `tools/task-state/lint.py`) only checks BLOCK PRESENCE and order, not anchor structure — so there is no Python parser to drift from the TS one (codex R2 F4 — R1's `_lib.parse_anchors` was a phantom cross-language dependency).
    - **Shared fixture file:** `tests/task-state/anchors-fixtures.json` carries `[ { input: <markdown body>, expected: { spec, reviews? } | { _parse_error: <reason> } } ]` pairs. Tests under `tests/task-state/anchors.test.ts` assert each fixture parses identically. If a future Python consumer needs anchor parsing, it MUST be a port that passes the same fixtures.
  ...
  - **`round-state.md` ownership and write protocol (compare-and-swap against upstream — blob-lease form).** Single owner: the **watcher** writes the round-boundary rewrite as part of its post-combine disposition step (skills/review-queue-watch.md AC3.5 (a)/(b)/(c) flow). Strategist updates between rounds (e.g., dispositioning) edit the file. Both writers use the same compare-and-swap protocol, executed exactly in this sequence (codex R3 F1 + codex-ops R3 F1 — the R2 protocol broke because step 6 delegated to `push-with-retry.sh`'s generic pull-rebase, which can cleanly replay a stale full-file rewrite onto a newer round-state blob; line-level clean rebase is NOT a semantic CAS success):
    1. **Read base:** `base_blob = git rev-parse HEAD:backlog/task-state/<task-id>/round-state.md`. If the path does not resolve (first-write path; file does not yet exist at HEAD), set `base_blob = ABSENT` — a literal sentinel string that compares only to itself or to other resolution failures (codex-ops R3 F2). The first-write path runs the same protocol as the rewrite path; the only difference is the sentinel.
    2. **Compute new content** in a tmp file (`mkdir -p backlog/task-state/<task-id>` first; the directory may not yet exist on first write). Free-form work; no git ops in this step.
    3. **Refresh upstream:** `git fetch origin main` (no rebase yet; just refresh `origin/main` ref).
    4. **Compare-and-swap (read-to-write race):** `now_remote_blob = git rev-parse origin/main:backlog/task-state/<task-id>/round-state.md` (or `ABSENT` if the path does not resolve at `origin/main`). If `now_remote_blob != base_blob`, abort: another writer landed between step 1 and now. Append `ROUND_STATE_WRITE_CAS_ABORT: <writer> base=<base_blob> remote=<now_remote_blob> ts=<ISO>` to `queue-errors.md`, do NOT replace the file, exit non-zero, let the next tick re-read from the new base. **First-write CAS:** if both are `ABSENT`, the CAS succeeds (no concurrent creator); if only one is `ABSENT`, the CAS aborts (another writer either appeared or removed the file — both anomalous).
    5. **Atomic FS swap:** `os.replace(tmp_path, final_path)` (matches the 040 pattern).
    6. **Commit + push via the round-state-specific helper** `tools/review-queue/push-round-state.sh <task-id> <base_blob>`. The helper does NOT delegate to generic `push-with-retry.sh`; instead it implements a **blob-lease around the push** (codex R3 F1 + codex-ops R3 F1):
       - `git add backlog/task-state/<task-id>/round-state.md` + commit.
       - `git push origin main`. On success, exit 0.
       - On rejection: `git fetch origin main`, then re-read `now_remote_blob = git rev-parse origin/main:<path>` (or `ABSENT`). If `now_remote_blob != base_blob`, the lease is broken. **Durable-log abort sequence** (codex R4 F1 + codex-ops R4 F1 — the prior wording said "append to queue-errors.md, then `git reset --hard`" which would have wiped the append since `raw/internal/queue-errors.md` is tracked):
         ...
         3. `git add raw/internal/queue-errors.md` + commit (`queue-errors: round-state CAS push abort`) + push via the generic `tools/review-queue/push-with-retry.sh` (a log-only commit; uncontested; the generic helper's pull-rebase-on-reject is safe here because this commit only touches queue-errors.md).
         4. Exit non-zero so the writer's caller sees the abort.
         **Do NOT rebase the original stale round-state commit.** A clean line-level rebase would silently land the stale rewrite on top of the newer round-state blob — exactly the race R3 surfaced. The writer's job is to abort and let the next tick re-synthesize from the new base.
       - If the blob is unchanged (origin/main advanced but didn't touch this file), the lease holds — `git pull --rebase origin main` is safe in that narrow case; retry push once. If pull-rebase introduces ANY conflict on this file specifically, run the durable-log abort sequence above (do NOT auto-resolve).
       - **Test fixture (matches codex R4 F1's "add a helper test that simulates a push rejection where the remote round-state blob changed"):** under `tests/task-state/push-round-state.test.ts`, set up a tmpdir + bare origin + two clones (writer A, writer B); both read same base blob; A pushes a different round-state rewrite first; B's `push-round-state.sh` invocation MUST exit non-zero AND `raw/internal/queue-errors.md` MUST contain the `ROUND_STATE_WRITE_CAS_ABORT_PUSH` row at the end of the run AND that row's commit MUST be on the local main (and origin/main).
    Reviewer ticks NEVER write `round-state.md` (and don't read it).
...
- For `backlog/reviews/.../request.md`: add to the reviewer-request schema validated by `tools/review-queue/validate.py`. **Reviewers MUST NOT consume `task_state_ref` from the request.** Enforcement (codex R2 F2 + codex-ops R2 F7 — the R1 substring scan would have quarantined this very R2 response, which legitimately critiques the marker as a problem):
  - **Reviewer schema gains an optional field `consumed_task_state: bool`** (default `false` when omitted). validate.py rejects on explicit `consumed_task_state: true`.
  - **Field-aware content detection (not substring scan).** validate.py rejects ONLY when the reviewer response body contains evidence of having READ a task-state file — specifically: three or more of the required-block heading patterns (`## current_thesis`, `## locked_decisions`, `## open_questions`, `## dont_touch`, `## canonical_anchors`, `current_round:`) appearing within the body. Three-of-six is the threshold because one or two could appear in a legitimate critique that names the contract; three-or-more is statistical evidence of verbatim quotation. Tests:
    - **Negative (must pass validation):** reviewer finding text like *"AC3 says `task_state_ref` shouldn't leak into reviewer responses; the substring `backlog/task-state/` will currently false-positive"* — names markers as critique targets, does NOT contain required-block headings.
    - **Negative (must pass):** reviewer body lists a SINGLE required-block name as a quoted example (e.g. *"the `## current_thesis` block must come first"*) — single mention, not three.
    - **Positive (must reject with `REVIEWER_FRESH_EYES_VIOLATION`):** reviewer body that quotes three or more required-block headings from an actual pointer file.
    - **Positive (must reject):** frontmatter has `consumed_task_state: true`.
  - On REVIEWER_FRESH_EYES_VIOLATION, `commit-reviewer-response.sh` quarantines per 041 AC4 `.invalid.<ISO-ts>` rename. `queue-errors.md` gets the trace.
...
- `get_role_state(task_id: string, role: 'strategist' | 'builder' | 'round-state', ref?: string) -> { content: string, last_updated: ISO8601, source_path: string, line_count: int, ref: string }`.
  - **Always-pin-to-commit-SHA contract** (codex R2 F3 + codex-ops R2 F5 — branch/tag echo + HEAD-race fix):
    1. **Resolve input ref first.** `resolved_ref = git rev-parse <input_ref_or_HEAD>^{commit}` runs ONCE at the start of the call. If input was `null`/omitted, this resolves `HEAD` → a concrete SHA at call entry. If input was a branch (`main`, `feature/x`) or tag, this resolves to the commit it currently points at. If input was already a SHA, this is a no-op verification.
    2. **All subsequent git reads use `resolved_ref`** — `git show <resolved_ref>:<path>` for content; never raw `HEAD` after step 1.
    3. **Response `ref` field is `resolved_ref` (always the commit SHA).** Consumers pinning a follow-up call to this `ref` get byte-identical content regardless of what `main`/`HEAD` moves to in the interim.
  - `last_updated` is the commit time of `resolved_ref`.
  - **No working-tree reads in V1.** This eliminates partial / uncommitted / dirty-tree-mid-pull-rebase reads. Acceptable trade for V1 (codex-ops R1 #6).
  - Returns isError if `task_id` resolves to no directory at `resolved_ref`, `role` file missing at `resolved_ref`, or input ref does not resolve via `rev-parse`.
- `list_task_states({ role?: string, binding?: string, stage?: 'ready' | 'claimed' | 'pending_review' | 'complete', ref?: string }) -> { task_states: Array<{ task_id, stage, roles_present: string[], task_state_ref_path, last_updated, canonical_anchors: { spec: string, reviews?: string } }>, ref: string }`.
  - `ref` parameter mirrors `get_role_state` semantics with the **same always-pin-to-commit-SHA contract**: `resolved_ref = git rev-parse <input_or_HEAD>^{commit}` runs ONCE at call entry, and the entire listing — directory traversal of `backlog/task-state/`, blob reads for pointer content, AND stage cross-references against `backlog/<stage>/<id>.md` — uses that single pinned commit. No re-evaluation of HEAD between operations within the call.
  - Response's top-level `ref` is `resolved_ref` (commit SHA, never a branch/tag string).
  - `stage` filter cross-references `backlog/<stage>/<id>.md` existence at `resolved_ref` (no cross-snapshot reads, no working-tree fallback).
...
- Tests under `tests/echo-mcp/role-state.test.ts` covering: (a) get returns content for existing file at HEAD, (b) get returns content at explicit SHA different from HEAD, (c) get returns isError for missing role file, (d) get returns isError for unresolvable ref, (e) list filters by stage at HEAD, (f) list returns empty array gracefully, (g) `binding` parameter accepted-but-unused without error, (h) `canonical_anchors` malformed → response succeeds with `_parse_error`, (i) repo-root resolution via constructor option overrides env + cwd, **(j) HEAD-race test: omit `ref`, then between the `git rev-parse HEAD` resolution and the `git show` content read, advance `main` to a new commit (fixture-controlled); response MUST contain content from the SHA pinned at entry and `ref` MUST equal that pinned SHA, NOT the new HEAD** (codex-ops R2 F5), **(k) branch-ref test: pass `ref="some-branch"` where the branch points at SHA X; response `ref` MUST equal SHA X (not the literal string "some-branch"); move the branch to SHA Y and call again; response MUST reflect Y** (codex R2 F3). Fixtures use a tmpdir + git-init helper; production checkout is never touched by tests.
