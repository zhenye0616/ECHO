...
Add an MCP tool `get_atom(id)` that returns ONE atom with **content verbatim** — no `WIRE_SHAPE_CAPS.match_content` clip — while still applying the existing **metadata projection** (`WIRE_SHAPE_CAPS.metadata_value` per-key clip + `tool_calls` → `trajectory` reshape via the `projectMatch` pipeline) and **excluding `embedding`**. The goal is to recover the full upstream content payload for consumers hitting `truncations: ["content"]` on `search_memories` / `tail_session` / `get_atoms` results, through MCP alone — no shell, no JSONL fallback, no composer-id context.

Demo bar: a Codex review turn elides mid-content; the strategist (or any AI client) calls `get_atom(<elided_atom_id>)` and receives the full verbatim turn body in a single MCP response — no JSONL fallback required. The Codex-realistic case (≈10KB content + ≈130KB raw `tool_calls` metadata) MUST succeed because metadata is projected before envelope-size check; only when `content` alone exceeds ~24KB does the explicit `atom_too_large_for_wire` error path fire.
...
**Pre-flight size check:** before serializing the success response, compute `JSON.stringify({...success_envelope_with_atom...}).length`. If > 25_000, return the atom-too-large error shape (above) instead. There is NO middle path — `get_atom` either returns the projected atom (content-verbatim + metadata-projected + embedding-excluded) or returns an explicit error. **No silent clipping of content** is the load-bearing contract; metadata projection still applies via `projectMatch` and is signaled in the returned `truncations` array.
...
- **Unit test (content verbatim + metadata projected + truncations correctness):** insert an atom with `content` = 5KB lorem ipsum (long enough that `projectMatch` would mark `truncations: ["content"]` if its content-clip path fired) + `metadata = {tool_calls: <8KB structured payload>, session_id: "abc", git_state: {...}}`. Call `get_atom(id)`. Assert: response `atom.content` byte-for-byte equals input content (no clipping); `atom.metadata.tool_calls` is the PROJECTED shape (trajectory reshape per `projectMatch`) NOT the verbatim 8KB; `atom.metadata.session_id` and `atom.metadata.git_state` byte-for-byte equal input (small-key path doesn't fire `WIRE_SHAPE_CAPS.metadata_value` clipping); `atom.embedding` is NOT present (excluded); **`"content"` is NOT in `atom.truncations`** (the truncations-cleanup step removed it after the verbatim override — load-bearing for R2 Finding 1 correctness); `atom.truncations` contains `"metadata.tool_calls:projected"`; `atom_size_bytes` matches `JSON.stringify(response.atom).length`.
- **Unit test (Codex-realistic recovery — the primary use case):** insert an atom shaped like the typical Codex long-turn extractor output — `content` = 10KB chat turn (a multi-finding review body that would normally elide under `WIRE_SHAPE_CAPS.match_content = 2_000`) + `metadata.tool_calls` = 130KB raw payload (the documented Bug A2 scale from `mcp-interactions-journal.md:737`). Call `get_atom(id)`. Assert: response success path (NOT atom_too_large), `atom.content` byte-for-byte = input content (no clipping), `atom.metadata.tool_calls` is projected (so the 130KB doesn't blow the envelope), **`"content"` is NOT in `atom.truncations`** (verbatim content recovered cleanly), `atom.truncations` contains `"metadata.tool_calls:projected"`, response envelope `JSON.stringify(envelope).length < 25_000`. This is the load-bearing test that validates the contract revision from R1 Finding 1 + the truncations-correctness fix from R2 Finding 1.
...
- **Integration test (round-trip from `truncations`):** insert an atom with content 5KB; call `search_memories` → assert returned match has `truncations: ["content"]` (because `WIRE_SHAPE_CAPS.match_content = 2_000` clips it); call `get_atom(match.id)` → assert `response.atom.content` byte-for-byte matches input AND `"content"` is NOT in `response.atom.truncations` (the trust signal correctly reflects the recovery — the consumer who saw `["content"]` on the first call should see no content-truncation flag on the recovery call).
...
- **Metadata projection + truncations cleanup (per R1 Finding 1 contract revision + R2 Finding 1 truncations-correctness fix):** import `projectMatch` from `src/mcp/wire-shape/match.ts` and use it for the metadata path, BUT bypass its content clipping AND fix up `truncations` to match the new reality. The simplest implementation:
  1. Call `projectMatch(captureEvent)` to get the projected wire-shape match (content clipped + metadata projected + embedding stripped).
  2. OVERWRITE `match.content` with `captureEvent.content` (the verbatim source).
  3. FILTER `"content"` out of `match.truncations` — because content is no longer truncated after step 2, leaving `"content"` in the array would be a lie to the consumer. Remaining entries (e.g. `"metadata.tool_calls:projected"`, `"metadata.<key>"` per-key caps) carry over correctly — they describe metadata projections that ARE still in effect after step 2.
  4. The resulting `match.truncations` is exactly the set of projections the consumer should be aware of: empty `[]` means content verbatim AND no metadata projection fired; non-empty means content verbatim AND some metadata key(s) were projected.

This pattern avoids duplicating the metadata projection logic; only the content-clipping decision differs, and the truncations-array fix-up keeps the trust signal honest. **AC4 unit tests MUST assert `"content"` is NOT in returned `truncations` even when the original `projectMatch(captureEvent)` would have included it** (i.e., even when `content.length > WIRE_SHAPE_CAPS.match_content`).
...
4. **Cross-reference gate:** AC ↔ Implementation Notes ↔ Out-of-Scope ↔ After Completion all cross-referenced consistently. ⚠️ **Initial pre-R1 claim of ✅ was honest at the time** (the v1 contract had no internal contradictions); however, **the post-R1-patch ✅ claim was wrong** — R1's contract revision (verbatim everything → content-verbatim + metadata-projected + embedding-excluded) introduced 5 stale cross-references the gate-4 check failed to catch (Goal §49, AC2 §129, Out-of-Scope §172-173, spec_refs comment §19). The gate as written did not explicitly mandate a "re-grep the whole spec for the OLD contract's vocabulary after every contract revision" sub-step. **R2 patch (2026-05-10 16:15 PDT) strengthens this gate** — see `## Review Round 2` below for the gate-4 update commitment and Codex R2 findings 1-4.

This spec was the first to apply the checklist before the first cross-tool review. The gate-4 failure surfaced by Codex R2 is the strongest argument yet for **mandatory re-grep after every contract-revision patch**, not just before initial commit.
...
# Review Round 2 — Codex post-R1-patch review (2026-05-10 16:05 PDT)

Codex (only — Cursor R2 did not occur in this cycle; Cursor's R1 "Proceed with two small nits" was treated as final from that reviewer) re-reviewed the R1-patched spec and returned **"Pushback, but narrow"** with 4 findings — all dependent-section drift from R1's contract revision. **This is exactly the failure mode the 032 R3-hypothesis described** ("strategist patches that touch one section but not cross-referencing sections of the same spec are a recurring failure mode") — eighth confirmation cycle today, first time on a 033 R1-patch.

**Findings:**

- **R2-1 (Codex HIGH — Class B truncations-correctness, code semantics):** Implementation Notes (pre-R2 §182) said: call `projectMatch(captureEvent)`, overwrite `match.content` with verbatim, then *"`truncations` array carries over from `projectMatch`."* But if the original `projectMatch(captureEvent)` clipped content (likely on the 5KB+ recovery case), it would set `truncations: ["content"]`. After overwriting content with verbatim, leaving `"content"` in the array is **a lie to the consumer** — the trust signal that survived 030 would be wrong on the very tool meant to recover from it. **Fixed:** Implementation Notes now have an explicit step 3 — filter `"content"` out of `match.truncations` after the verbatim override. AC4 unit tests #1 + #2 + integration test all add an assertion: `"content"` is NOT in returned `truncations`, even when the underlying content would have been clipped by `projectMatch`. This is now load-bearing for R2 correctness.
- **R2-2 (Codex Medium — Class B Goal-section staleness):** Goal §49 still described the OLD contract: *"returns ONE atom verbatim — no `WIRE_SHAPE_CAPS.match_content` clip, no `WIRE_SHAPE_CAPS.metadata_value` clip, no projector reshape."* A builder reading top-down would internalize the wrong contract before reaching AC1's revised "Scope of verbatim." **Fixed:** Goal rewritten to "content verbatim + metadata projection (per `projectMatch`) + embedding excluded" with explicit Codex-realistic demo bar reference.
- **R2-3 (Codex Medium — Class B AC2 wording drift):** AC2 §129 said *"`get_atom` either returns verbatim or returns an explicit error."* The word "verbatim" without qualifier now contradicts the revised contract where metadata is projected. **Fixed:** "returns the projected atom (content-verbatim + metadata-projected + embedding-excluded) or returns an explicit error. **No silent clipping of content** is the load-bearing contract; metadata projection still applies via `projectMatch`."
- **R2-4 (Codex Medium — Class B Out-of-Scope contradicts new contract):** Out-of-Scope §172-173 said `get_atom` "skips `projectMatch` entirely" AND "the escape hatch returns everything." Both directly contradict R1's contract revision. **Fixed:** §172 rewritten to "REUSES `projectMatch` for the metadata-projection + embedding-strip pipeline and then overrides only its content-clip with verbatim content; do not change `projectMatch`'s behavior, signature, or output shape." §173 rewritten to "no `fields[]`, no extra projection, no opt-out from `tool_calls` reshape — returns `projectMatch`'s metadata output unchanged."
- **R2-5 (Codex Low — Class B spec_refs comment):** spec_refs §19 said `projectMatch — the projector get_atom deliberately skips`. **Fixed:** "projectMatch — reused for metadata projection (content-clip overridden with verbatim)."

**Pattern note (load-bearing for the operating-model wiki):**

This is the **eighth cross-tool review cycle today** and the **first time the 4-gate checklist's Gate 4 (Cross-reference gate) failed on a post-patch state** that was claimed ✅ in the spec. The honest read: my `✅` on §207 (pre-R2 line numbering) was claimed *before* the R1 patch landed, when the v1 contract was internally consistent. After the R1 patch introduced the new vocabulary ("content verbatim, metadata projected"), I did not re-run gate 4. The checklist as written says "AC ↔ Implementation Notes ↔ Out-of-Scope ↔ After Completion all cross-referenced consistently" — it doesn't explicitly say "**re-run after every patch that changes the contract.**" That's the gap.

**Process change (proposed; pending founder approval):**

Add to gate 4 in `wiki/operating-model/cross-tool-spec-review.md` "Strategist self-review checklist":
> **Gate 4 (cross-reference) — mandatory re-grep after every contract-revision patch.** When a spec patch changes the contract vocabulary (e.g., adds "projected", removes "verbatim metadata", introduces an error class), grep the whole spec for the OLD contract's load-bearing terms before committing. Each match is a candidate stale cross-reference. The grep command must be recorded in the patch commit message so the founder can verify it ran (e.g., `git commit -m "...verified via: grep -nE '(no projector|deliberately skip|returns everything)' <spec>"`).

The pre-R2-commit grep for this patch caught all 5 stale spots in one pass (`grep -nE "(no projector|projector.*skip|deliberately skip|skips it|skips projectMatch|returns everything|verbatim metadata|no.*metadata_value clip|metadata_value.*clip|verbatim.*metadata)"`). The discipline is cheap; the failure to apply it was discipline-on-patches, not gap-in-the-checklist. **The proposed change makes it explicit and verifiable in commit history.**

**Disposition:** All 5 R2 findings addressed in this revision. The contract is now internally consistent (Goal ↔ AC1 ↔ AC2 ↔ AC3 ↔ AC4 ↔ Implementation Notes ↔ Out-of-Scope ↔ R1 history ↔ R2 history). The R2-1 truncations-correctness fix is load-bearing — if the builder implements steps 1-2 without step 3, the trust signal added in 030 lies on the recovery tool. **Expecting R3 to verify the truncations-correctness assertion is testable and present in the AC4 tests** (it is — added to two unit tests + the integration test).
