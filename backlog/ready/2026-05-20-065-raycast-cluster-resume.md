...
files_to_modify:
  - tools/raycast-echo/src/lib/sessions.ts  # AC1, AC6, AC8 — add optional `clusterId?: string` to Session shape + `recordSessionStart` payload; persist on every session that originated from a cluster click (cluster row → runAsk path) so the cluster_id ↔ session edge is durable. New ASYNC lookup: `findLatestSessionForCluster(clusterId: string, statuses?: ReadonlyArray<Session["status"]>): Promise<Session | null>` returning the most-recent session matching the cluster + status filter — Promise-returning because the existing session store is async LocalStorage (`listSessions()` awaits migration; r1 codex F1). Default statuses = `["running", "done"]` (excludes `errored` and `cancelled`; AC6 punts opening past failures as a separate UX). Storage shape is additive — existing sessions without `clusterId` continue to load and behave as today. **clusterId round-trip preservation (r1 codex-ops F4)**: `normalizeSession` MUST preserve `clusterId` through `mergeRowAndWrite`, `recordSessionUpdate`, and `recordSessionEnd`; without this, the first flush after `recordSessionStart` would silently erase the cluster edge. AC1's test contract awaits the helper AND drives the full lifecycle (start → update → end) before asserting the cluster_id is still discoverable. **NEW per-cluster singleflight primitive (r2 codex F1 + codex-ops F5 — ATOMIC reservation; r3 mixed-intent refinement; r4 codex-ops F1 ownership signal)**: export `acquireOrAwaitClusterSession(clusterId: string, intent: "default" | "fresh", factory: () => Promise<Session>): Promise<{ session: Session; createdByThisCall: boolean }>` backed by a module-level `Map<string, Promise<Session>>` whose key is `${clusterId}#${intent}` (two parallel singleflight pools per cluster, one per intent — r3 codex F1 + codex-ops F1). Contract: if no in-flight entry exists for the composite `clusterId#intent` key, register `factory()`'s promise, await, delete on settle, return `{ session, createdByThisCall: true }`. If an in-flight entry exists for the SAME `clusterId#intent` key, await that existing promise, return `{ session, createdByThisCall: false }` — DO NOT call factory. "Open Prior Answer" (intent=`default`) and "Ask Again from This Cluster" (intent=`fresh`) for the same cluster use DIFFERENT keys and proceed in parallel — neither short-circuits the other (preserves AC3 — codex r3 F1, codex-ops r3 F1). **The `createdByThisCall` flag is load-bearing for the r4 ownership signal (codex-ops r4 F1)**: without it, multiple same-intent waiters all receive the same fresh-created Session, all observe "freshly-created" state, and all spawn duplicate startAgent processes. With it, exactly one caller per key (the owner — `createdByThisCall: true`) progresses past the AnswerView's startAgent gate; all waiters (`createdByThisCall: false`) replay-attach via the AC4 short-circuit instead.
...
**The contract (r3 mixed-intent + r4 ownership signal):** `tools/raycast-echo/src/lib/sessions.ts` exports `acquireOrAwaitClusterSession(clusterId: string, intent: "default" | "fresh", factory: () => Promise<Session>): Promise<{ session: Session; createdByThisCall: boolean }>`, backed by a module-level `Map<string, Promise<Session>>` whose key is `${clusterId}#${intent}`. Semantics per composite key:

- If no in-flight entry exists for `${clusterId}#${intent}`: register `factory()`'s promise, await, delete on settle, return `{ session, createdByThisCall: true }`. **The factory is called exactly once per key; the caller that ran it is the owner.**
- If an in-flight entry exists for the same key: await that existing promise, return `{ session, createdByThisCall: false }`. **Do NOT call `factory`.** Waiters receive the same session as the owner with the flag set false.
...
- The `createdByThisCall` flag (r4 codex-ops F1) is load-bearing for AnswerView's `startAgent` and `onSessionChanged` gating — without it, same-intent waiters all observe the freshly-created Session and all spawn duplicate agents.
...
Outside the factory, AnswerView branches on BOTH the session shape AND the `createdByThisCall` flag:
- Pre-existing done/running session returned (default-intent replay path; the factory's `findLatestSessionForCluster` returned a hit): NO `startAgent`, NO `onSessionChanged`. Render the matching replay banner per AC4. Behavior is identical regardless of `createdByThisCall` — replay is read-only.
- Freshly-created running session returned AND `createdByThisCall: true` (the OWNER): invoke `onSessionChanged?.()` (AC7), then `startAgent(invocation, { sessionLogPath: subprocessLogPath })` using the SAME pre-allocated path. The persisted row's `subprocessLogPath` matches the file `startAgent` actually opens.
- Freshly-created running session returned AND `createdByThisCall: false` (a WAITER that observed the owner's just-created running row): treat as a running-replay — NO `startAgent`, NO `onSessionChanged`, render the running banner per AC4. The owner's startAgent is already running; the waiter attaches by reading the persisted row's growing `answer` field via the existing flush poll. This is the load-bearing r4 contract: without the owner check, both callers would observe the freshly-created session and both would spawn startAgent (codex-ops r4 F1).
...
- **(4a) Same-intent collapse, default (with ownership check):** two near-simultaneous `acquireOrAwaitClusterSession(C, "default", factory)` calls; factory's `findLatestSessionForCluster` resolves NULL for both BEFORE the first's `recordSessionStart` resolves. Assert: factory called exactly once; `findLatestSessionForCluster`, `recordSessionStart` each called exactly once; **`startAgent` called exactly once and only by the owner** (`createdByThisCall === true`); the waiter (`createdByThisCall === false`) returns the same session WITHOUT calling startAgent or onSessionChanged (r4 codex-ops F1).
- **(4b) Same-intent collapse, fresh (with ownership check):** two near-simultaneous `acquireOrAwaitClusterSession(C, "fresh", factory)` calls. Assert: factory called exactly once; `recordSessionStart` called exactly once; **`startAgent` called exactly once and only by the owner**; `onSessionChanged` called exactly once (by the owner; waiters do not duplicate-fire it). Proves Ask Again de-dupes both the factory AND the agent spawn.
- **(4c) Mixed-intent parallel:** one `acquireOrAwaitClusterSession(C, "default", factory_d)` AND one `acquireOrAwaitClusterSession(C, "fresh", factory_f)` for the SAME cluster `C`, both started before the first settles. Assert: BOTH factories called exactly once (different keys, no collapse); BOTH return `createdByThisCall: true` (each is owner of its own intent's key); two `recordSessionStart` calls (one per intent); two `startAgent` calls (each owner spawns its own). This proves the mixed-intent contract — Ask Again is never swallowed by a default in-flight (codex r3 F1, codex-ops r3 F1).
