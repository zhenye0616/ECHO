...
- **One new daemon-side integration test for the wrapper end-to-end (round-2 arch reviewer 2026-05-18).** `tests/mcp/recent-calls-endpoint.test.ts` starts `startMcpServer`, fires one POST `/mcp` invocation of `echo_ping`, then `GET /mcp/recent-calls?since=0`, asserts (1) the entry is present with the correct projection shape and `status: "ok"` and a finite `duration_ms`, (2) the original POST `/mcp` response is unchanged from pre-wrapper behavior (regression guard for the registration sweep / instrumentation). This is the test that catches "wrapper missed a registration" and "wrapper accidentally broke MCP behavior" — neither of which the pure ring-buffer unit tests would catch.
