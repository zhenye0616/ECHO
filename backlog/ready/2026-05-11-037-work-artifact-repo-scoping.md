...
4. **Cache discipline (R2 correction — Codex + Cursor R2 finding #1).** Cache POSITIVE results only in a module-level `Map<composer_id, string>`, with **Stage 1 (registry) ALWAYS checked first when the workspace registry has a binding for the composer**. Lookup order is:
   - **a.** Check `composerToWorkspace.get(composer_id)` — if a `workspace_id` is present, ALWAYS run Stage 1 (`resolveRepoRootForWorkspaceId`). Stage 1 outcome (whether success or fail) is the authoritative `repo_root` for this turn; cache is populated/overwritten with the result. Registry priority is preserved on every tick; the cache never shadows it.
   - **b.** If no workspace binding (`composerToWorkspace.get(composer_id) === undefined`), fall through to cache: cache hit → return cached path. Cache miss → run Stage 2 (file-walk). On Stage 2 success, populate cache. Never cache null.
   - This preserves both invariants from R1 + R2: (i) negative outcomes are always re-attemptable (R1 finding 3); (ii) registry path takes priority over a positive file-walk cache hit (R2 finding 1). The "registry-priority-after-positive-fallback" test is now satisfied by step (a) running unconditionally on every tick where a registry binding exists.
...
6. **Git scope-restriction (R2 correction — Cursor R2 finding #3):** for `source_app='git'`, `repo_path` matches `metadata.repo_root` only; legacy git atoms without that metadata are not recovered here (use `source_prefix='git:<path>'` instead). Document in tool description.
...
4. **Git scope-restriction (R2 correction — Cursor R2 finding #3):** for sources matching the git prefix, `repo_path` matches `metadata.repo_root` only; legacy git atoms without that metadata are not recovered. Document in tool description.
...
6. **Note 3 (git scope across other tools) — R2 correction (Cursor R2 finding #3).** The git two-path OR fallback in Note 2 is **`tail_session`-specific** because the MRU resolver depends on it to surface legacy git atoms (which may pre-date AC1 and lack `metadata.repo_root`). For `search_memories`, `find_clusters`, `wait_for_new_turns` with `source_app='git'` + `repo_path`: the `repo_path` filter matches `metadata.repo_root` only. Callers needing legacy git atoms by path can use the existing `source_prefix='git:<path>'` parameter on those tools today. This explicit scope-restriction is added to AC3, AC4, AC5 descriptions.
...
**Total review cycles:** R1 closed the load-bearing architectural flaws (AC6 priority + AC1 resolver direction); R2 closed the implementation-disambiguation + spec-clarity issues that survive a structural fix. No further pre-claim review is warranted — the spec has two passes of independent cross-tool validation against multiple readers from different starting points (Codex via MCP probes + JSONL inspection; Cursor via in-IDE file reads + git probes).
