...
6. Renders the plist with all substitutions (LABEL + NODE_EXEC_PATH + INSTALLED_DAEMON_PATH + LOG_DIR + ECHO_HOME + ECHO_MCP_PORT + ECHO_DATA_DIR + ECHO_DB_PATH).
...
10. **Post-bootstrap health-probe wait (r2 codex-ops F1 MED).** `launchctl bootstrap` returning success means the LaunchAgent was loaded, NOT that the daemon is actually serving. A crash-looping daemon, a bad import, a DB-open failure, an occupied port, or a bad persisted env all let `bootstrap` return 0 while leaving the operator with a broken replacement (especially dangerous during an upgrade where step 8 already booted out the working previous daemon). The installer MUST wait for the resolved daemon to answer the same MCP/doctor health probe used by `status` (default deadline: 10s, with exponential backoff starting at 200ms). On probe-timeout, exit non-zero with a structured error naming the resolved label + port + a one-line `daemon logs --tail 50` suggestion + the recovery path (re-install / inspect / rollback). Do NOT treat a merely-loaded LaunchAgent as healthy.
...
4. Assert (proves AC3.2's plist envs actually reached the launchd-started daemon, NOT defaults — closes r1 codex F2 / codex-ops F1 HIGH AND r2 codex F1 HIGH):
   ...
   - A `coord_invoke` MCP call for any role returns `isError: true` with a CoordPathError-shaped message (proves AC1.5's de-scope mechanism is intact in the packaged install — the daemon stays up and surfaces a clean rejection without crashing)
5. `echoctl daemon stop $OVERRIDES`
6. `echoctl daemon uninstall $OVERRIDES`
7. Verify the test launchd job is gone (`launchctl print gui/$(id -u)/com.echo.daemon.test-<uuid>` exits non-zero) AND — critically — the founder's production `com.echo.daemon` job (if present) is untouched. The test snapshots `launchctl print gui/$(id -u)/com.echo.daemon` before + after the test block; if pre-existing, the after-state PID must equal the before-state PID; if not pre-existing, both states must be "not loaded." The snapshot ALSO covers the production data-dir + db-path: the test asserts the mtime + size of `~/Library/Application Support/ECHO/` + `~/Library/Application Support/ECHO/echo.db` (if present) are unchanged across the test block.
...
- **Data dir + db path (r2 codex F1 HIGH)** — unique tmp dir per test run; the production `~/Library/Application Support/ECHO/` + its `echo.db` are NEVER read, written, locked, or mtime-touched. The test asserts mtime + size unchanged across the test block.
- **Cleanup** — runs even on test failure (`try/finally` or vitest `afterEach`). Cleanup MUST first verify production-job untouched AND production-data-dir untouched (snapshot diff per AC5.1 step 7); a cleanup that finds production was mutated MUST surface a TEST FAILURE (not a cleanup error swallowed in logs).
- **Pre-flight skip** — if the production label `com.echo.daemon` exists AND the test environment cannot snapshot+verify it (e.g., permission denied on `launchctl print`), the test SKIPS rather than runs with a poisoned safety net. Same skip-rule applies if `~/Library/Application Support/ECHO/` exists AND its mtime cannot be snapshotted.
