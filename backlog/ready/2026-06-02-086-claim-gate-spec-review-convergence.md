...
Terminal convergence covers BOTH `proceed` and the case-(c) `proceed_after_patches --patches-applied=false` verdict; the watcher writes `converged` for either. The gate consumes one boolean the watcher already decided; the selector reads item frontmatter it already parses (plus one `git show` for the staleness normalization), and the convergence *judgment* stays in the watcher where it is computed.
