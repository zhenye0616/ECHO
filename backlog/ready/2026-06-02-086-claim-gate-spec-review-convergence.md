...
files_to_modify:
  ...
  - tests/backlog/blocked-spec-review-gate.test.*     # AC6 — NEW test (path/runner per repo convention; blocked.py currently has NO dedicated test file). Cover: (i) requested_reviewers empty/absent ⇒ claimable (regression: today's behavior); (ii) requested_reviewers non-empty + no spec_review field ⇒ BLOCKED; (iii) + spec_review:converged + fresh sha ⇒ claimable; (iv) + spec_review:converged but stale sha ⇒ BLOCKED; (v) spec_review:waived ⇒ claimable (founder bypass); (vi) --validate rejects malformed spec_review value / malformed spec_review_sha; (vii) --list-blocked prints the new review-gate reason.

spec_refs:
  - tools/blocked.py  # candidates() at L226 is the SOLE claim selector; its only gate today is "stage==ready AND every blocked_by id ∈ complete/". L167-169 explicitly: "the status: field is informational only … we deliberately do NOT validate status." There is NO review-state check anywhere. parse_frontmatter already captures requested_reviewers (list) + arbitrary scalars, so the new fields are readable without a parser change.
...
The watcher is the actor that authoritatively knows convergence (it calls `dispatch-next-round.py --terminal` and commits `review-r<N>: terminal on <item_id>`). At that exact terminal step it ALSO writes into the reviewed item's frontmatter, in the same commit:

```
spec_review: converged
spec_review_sha: <terminal spec_commit_sha>   # the sha the converged round was pinned to
```
...
2. **not stale**: the item's current spec content matches `spec_review_sha` (else BLOCKED — reason `spec-edited-after-review`; an edit after convergence must force re-review), **and**
3. `spec_review != <escalated>` — convergence that escalated to the founder never sets `converged` (the watcher only writes the field on a non-escalated terminal verdict), so escalation is naturally excluded.

Terminal convergence covers BOTH `proceed` and the case-(c) `proceed_after_patches --patches-applied=false` verdict; the watcher writes `converged` for either. The gate does NOT re-interpret review verdicts — it consumes one boolean the watcher already decided. This keeps the safety-critical deterministic selector minimal: it reads only item frontmatter it already parses, and the convergence *judgment* stays in the watcher where it is computed.

### Founder bypass (explicit, auditable)
The founder (principal) may fast-track a spec past review by setting `spec_review: waived` in the item frontmatter. `blocked.py` treats `waived` as claimable. This satisfies the prior note's "founder-approved exceptions must have an explicit field or command path" — the bypass is greppable and deliberate, never silent.
...
- **AC1 — Watcher writes the convergence field.** At the watcher's terminal/convergence step (both the zero-patch §(a) and the proceed_after_patches §(c) terminal paths), `skills/review-queue-watch.md` instructs writing `spec_review: converged` + `spec_review_sha: <terminal spec_commit_sha>` into the reviewed item's frontmatter, folded into the existing terminal commit (no new commit). The adapter `.claude/commands/review-queue-watch.md` is regenerated via `tools/sync-skills.sh` and `tools/sync-skills.sh --check` passes. *Test:* skill text + a fixture-level assertion that a terminal disposition produces the two fields; OR, if the watcher step is not unit-testable, an explicit worked-example block in the skill plus a checklist line. (Builder: pick the strongest available verification and state it in agent_notes.)
- **AC2 — Gate in the selector.** `tools/blocked.py candidates()` marks a `ready/` item BLOCKED when it has non-empty `requested_reviewers` and `spec_review != converged` and `spec_review != waived`. The predicate lives in an isolated, unit-testable helper (e.g. `spec_review_satisfied(item) -> (bool, reason|None)`), NOT inlined into the sort. The module docstring "Selection rule" is updated to state the review gate.
- **AC3 — Staleness re-blocks.** When `spec_review: converged` but the item's current spec content no longer matches `spec_review_sha`, the item is BLOCKED (reason `spec-edited-after-review`). The mechanism for "current spec content" (git blob hash of the item file, or git-log "is the latest commit touching this file the terminal commit", whichever the builder verifies is deterministic and CI-safe) is the builder's call; it MUST be pinned by a test. `waived` is exempt from the staleness check.
- **AC4 — Docs reflect the gate.** `docs/AGENT_INSTRUCTIONS.md` (claim contract) and `backlog/README.md` (pipeline) describe: the review gate, the conditional-on-`requested_reviewers` scope, the `waived` founder bypass, and that `spec_review`/`spec_review_sha` are watcher/founder-owned (NOT builder-managed).
- **AC5 — Validation.** `blocked.py --validate` rejects (exit 2) an item whose `spec_review` is present but not in `{converged, waived, pending}` and whose `spec_review_sha` is present but malformed. Absent `spec_review` is valid (means "not yet reviewed").
- **AC6 — Tests.** The new test file covers cases (i)-(vii) enumerated in `files_to_modify`. Empty/absent `requested_reviewers` ⇒ claimable is a REGRESSION guard for today's behavior. All existing `npm test` / repo test suites still pass; `blocked.py --validate` passes on the real backlog after this spec's own fields are present.
