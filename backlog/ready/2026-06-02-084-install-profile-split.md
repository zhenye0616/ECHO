...
- **AC4 — onboarding.json schema + durable inference discriminator (codex r1 F2 + codex-ops r1 F5; hardened per codex r3 F2 + codex-ops r3 F1).** `onboarding.json` gains a `profile` field (+ Ajv validator); the resolved profile is persisted there. When no `profile` is recorded, inference must NOT key on file-presence alone — `ensureEchoHome()` writes a valid profile-less file before init can persist, so an interrupted fresh scaffold and a completed legacy install are byte-identical on "valid file, no profile." Use a **durable discriminator over the EXISTING onboarding fields**: **(a) no onboarding file, OR a bare/incomplete scaffold (`completed: false` AND empty `agents`) ⇒ fresh ⇒ `customer`; (b) a completed prior install (`completed: true` OR non-empty `agents`) lacking `profile` ⇒ legacy ⇒ `dogfood`** (back-compat: the founder's existing machine — `completed: true`, agents populated — keeps its full surface; emit a one-line migration note). This closes the crash-window collapse: an interrupted fresh customer install (`completed:false`, no agents) re-resolves `customer` on rerun regardless of timing, and is independent of whether `profile` was persisted before the crash. doctor consumes the field (AC5).
...
- **AC7 — tests green.** New/updated tests for AC1–AC5 pass, including: `adapter-sync.test.ts` proving customer-skip is a successful no-op (not failure) while dogfood preserves today's role/workflow behavior; **the AC1 precedence chain** incl. a no-TTY answer-file `profile: dogfood` install coming up dogfood (not customer); **the AC4 discriminator branches** — (i) no onboarding file ⇒ customer, (ii) completed legacy file (`completed:true`/agents) without `profile` ⇒ dogfood, (iii) **bare/incomplete scaffold** (`completed:false` + empty `agents`, no `profile`) ⇒ customer (the partial-scaffold/no-flip regression, asserting the actual on-disk shape); and the **doctor text-output** profile assertion (AC5). Full `npm test` + typecheck green.
