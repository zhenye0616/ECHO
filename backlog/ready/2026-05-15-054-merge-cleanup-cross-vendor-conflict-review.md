...
2. A fenced code block whose contents include ALL FIVE of these literal substrings on the same `codex exec` line (continuation backslashes `\` allowed): **"codex exec"** AND **"$MERGER_WT"** (or equivalent — accepts `"$MERGER_WT"` with quotes or unquoted `$MERGER_WT`) AND **"--sandbox read-only"** AND a stdout redirect to a named file under `$MERGER_WT` (mechanical: substring **".c3.5-stdout"** anywhere on the line, preceded by `>` or `>>`) AND a stderr redirect to a named file under `$MERGER_WT` (mechanical: substring **".c3.5-stderr"** anywhere on the line, preceded by `2>` or `2>>`). Mechanical detection in AC2 asserts all five substrings co-occur on a single logical line (joining `\`-continuation lines) inside a fenced block. The named-file capture is load-bearing for AC1b.7 consult-failure recovery: terminal scrollback is not durable after a `/clear` or session restart; named files under `$MERGER_WT` survive at least until the merger worktree is cleaned up (post-`continue` or post-`abort`), giving the strategist a fixed handle for both happy-path stdout parsing and unhappy-path stderr surfacing.
...
4. A prompt-template section listing the SIX load-bearing prompt elements: (i) working-tree state captured *inside `$MERGER_WT`* via `git status --porcelain` + `git diff` (NOT relative to the live checkout), (ii) batch context (other recent merges + their sidecar prescriptions), (iii) conflict markers (the prompt MAY direct the reviewer to read files inside `$MERGER_WT` via `git diff <file>` since the invocation cwd is `$MERGER_WT`; OR the prompt embeds conflict markers verbatim — both are acceptable), (iv) specs/sidecars the reviewer should consult, (v) the proposed resolution (verbatim), (vi) output format with the explicit instruction that the reviewer MUST emit `consult_cwd: $(pwd -P)` as a header field (per AC1b.5) — the value MUST be the canonical/physical path (not `$PWD`, not a logical path) so the strategist's canonical-vs-canonical compare survives the macOS `/var/folders/...` vs `/private/var/folders/...` ambiguity. Mechanical detection in AC2b: an ordered/unordered list of ≥6 items under a subheading containing "prompt" (case-insensitive); the list MUST mention `$MERGER_WT` (or `MERGER_WT`) at least once; the list item describing (vi) MUST contain the substring `consult_cwd` AND the substring `pwd -P` (asserting the canonical-path discipline is named at the prompt-instruction site, not only in the output-format section).
5. An output-format specification: the reviewer's response MUST start with a YAML-like header containing THREE required fields — `verdict: proceed-as-proposed | proceed-with-modifications | pushback`, `reviewer: codex` (or other vendor name), AND `consult_cwd: <canonical-path>` where the value is the **physical/canonical path** returned by `pwd -P` (POSIX) inside the cwd the reviewer ran from. The prompt template (per AC1b.4 below) instructs the reviewer to capture `$(pwd -P)` at the start of its response generation (NOT `$PWD`, which is the logical path and on macOS resolves to `/var/folders/...` vs the canonical `/private/var/folders/...` — the discrepancy makes exact string compare brittle). The strategist canonicalizes its side via `(cd "$MERGER_WT" && pwd -P)` (or equivalent) before the wrong-tree compare. This is load-bearing for AC1b.7 failure mode (iv) wrong-tree detection: the strategist verifies `consult_cwd == $(cd "$MERGER_WT" && pwd -P)` post-hoc; a mismatch means the reviewer ran in the wrong tree even though `-C "$MERGER_WT"` was specified in the recipe (e.g., a misconfigured codex CLI ignored `-C`, or a future binding's adapter shape doesn't honor the equivalent flag). Mechanical detection in AC2b: in the C3.5 block's output-format section, substring **"proceed-as-proposed"** AND substring **"proceed-with-modifications"** AND substring **"pushback"** AND substring **"verdict:"** AND substring **"reviewer:"** AND substring **"consult_cwd:"** AND substring **"pwd -P"** (the spec MUST literally name the canonicalization mechanism so a future builder reading only the output-format section can implement correctly without consulting AC1b.4).
...
7. A **"Consult-failure recovery"** subsection (the heading text MUST contain the literal "Consult-failure recovery" case-insensitive) covering the four failure modes: (i) `codex` (or other vendor binary) not found / exit code 127 (detected via `$MERGER_WT/.c3.5-stderr` containing `command not found` OR the shell's `$?` being 127); (ii) `codex exec` exits non-zero with no parseable response (detected via `$?` non-zero AND `$MERGER_WT/.c3.5-stdout` empty or lacking the required YAML header); (iii) response file (`.c3.5-stdout`) present but YAML header malformed / `verdict:` field missing or not one of the three allowed values (detected by strategist's YAML parse failing or by the parsed `verdict` not being in the allowed set); (iv) response present and YAML-parseable but `consult_cwd:` does not match the canonicalized `$MERGER_WT` — the reviewer ran in the wrong tree even though the recipe specified `-C "$MERGER_WT"` (detected by string compare of the echoed `consult_cwd` value against the strategist's `$(cd "$MERGER_WT" && pwd -P)` — both sides physical paths; raw-`$MERGER_WT` compare is NOT used because `$TMPDIR` on macOS resolves logically as `/var/folders/...` and physically as `/private/var/folders/...`, and the reviewer's emitted `pwd -P` is always the physical form). In ALL four cases, the strategist MUST (a) surface the failure to the founder with the captured stderr (or response excerpt for iii/iv) — concretely: read `$MERGER_WT/.c3.5-stderr` for (i)-(ii), read `$MERGER_WT/.c3.5-stdout` for (iii)-(iv), (b) record `C3.5 cross-vendor consult: <reviewer> @ failed — <one-sentence failure reason>` in the eventual C6 review_notes line per AC4a, and (c) return to the C3 pause prompt with the existing three branches (`c3.5` retry-with-different-vendor / `continue` apply-without-consult / `abort`). The strategist does NOT auto-retry. Mechanical detection in AC2b: substring **"Consult-failure recovery"** (case-insensitive) AND each of the four failure-mode signatures **"not found"** OR **"127"**, **"exit"** + **"non-zero"** (within 50 chars of each other), **"malformed"** OR **"unparsable"**, **"consult_cwd"** AND (**"does not match"** OR **"mismatch"** OR **"!="**).
...
- Assert that the C3.5 block contains, in order:
  ...
  - A fenced code block (`` ``` `` … `` ``` ``) containing a single logical line (joining `\`-continuation lines first) that has ALL FIVE substrings: `codex exec`, `$MERGER_WT` (or `MERGER_WT` allowing for unquoted/quoted forms), `--sandbox read-only`, `.c3.5-stdout` (preceded by `>` or `>>`), AND `.c3.5-stderr` (preceded by `2>` or `2>>`).
  ...
  - The SIX verdict-header substrings (`verdict:`, `reviewer:`, `consult_cwd:`, `proceed-as-proposed`, `proceed-with-modifications`, `pushback`) anywhere in the block (AC1b.5 — header existence only; the YAML header co-occurrence does not constrain action-sentence scoping below).
...
- **Scoped consult-failure-recovery block:** locate the "Consult-failure recovery" subsection inside the C3.5 block via anchored regex `^#+\s+Consult-failure\s+recovery(?:[^A-Za-z0-9]|$)` (case-insensitive). Within that subsection, assert the four failure-mode signatures listed in AC1b.7: (i) `not found` OR `127`, (ii) `exit` AND `non-zero` co-occurring within 50 chars, (iii) `malformed` OR `unparsable`, (iv) `consult_cwd` AND one of (`does not match`, `mismatch`, `!=`).
- Add synthetic-content test cases (parallel to the existing C5/C6 synthetic tests at lines 87-124): one happy-path test that passes; one missing-C3.5 test (`C3.5 heading not found`); one missing-C4 test (`C4 heading not found after C3.5`); one missing-MERGER_WT-in-codex-exec-line test (distinct error message); one missing-stdout-redirect-in-codex-exec-line test (distinct error message naming `.c3.5-stdout`); one missing-stderr-redirect-in-codex-exec-line test (distinct error message naming `.c3.5-stderr`); one missing-verdict-header-string test (distinct error per missing string from the six in AC1b.5); one missing-`pwd -P`-in-output-format-section test (distinct error); one missing-`consult_cwd`-in-prompt-template-(vi)-item test (distinct error — asserts `consult_cwd` AND `pwd -P` both appear in the prompt-template list under the (vi) item, not only in the output-format section); one missing-Post-review-handling-subsection test (distinct error); one Post-review-handling-with-only-2-entries test (distinct error: expected 3 entries got 2); one Post-review-handling-verdict-without-action-prose test (distinct error per missing action sentence); one missing-Consult-failure-recovery-subsection test (distinct error); one Consult-failure-recovery-missing-failure-mode test (distinct error per missing signature, including failure mode iv's `consult_cwd` form); one §C3 missing-`c3.5`-branch test.
...
**AC4a — §C6 review_notes template field.** The existing §C6 review_notes template MUST add a new line under "Conflicts resolved" (or as a sibling top-level entry): `C3.5 cross-vendor consult: <reviewer> @ <verdict> — <one-sentence summary>`. The `<verdict>` slot holds one of the three success verdicts (`proceed-as-proposed`, `proceed-with-modifications`, `pushback`) OR the literal `failed` when AC1b.7 consult-failure recovery fired. Summary text varies: for `proceed-as-proposed` → `"no modifications"`; for `proceed-with-modifications` → `"applied N modifications: <one-line list>"`; for `pushback` accepted → `"founder accepted pushback; redesigning resolution"`; for `pushback` overridden → `"pushback rejected by founder — applied original anyway"`; for `failed` → `"<failure mode signature, one of: not-found / non-zero exit / malformed response / consult_cwd mismatch>"` (the four signatures match AC1b.7's four detected failure modes exactly; the `consult_cwd mismatch` label is canonical for failure mode (iv) and replaces the earlier `wrong SHA` vocabulary that R3 removed). If no C3.5 fired during the merge, the line reads `C3.5 cross-vendor consult: none invoked`. Mechanical detection in AC2c: §C6's review_notes template (lines 197-225 in the post-AC1 canonical skill, adjust if line numbers shift) MUST contain the substring `C3.5 cross-vendor consult:`.
