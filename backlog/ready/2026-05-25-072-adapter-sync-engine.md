...
**AC4.1 — `populateEchoSkills(opts: { sourceDir: string; targetDir: string }): SkillSyncResult | { ok: false; sourceDir: string; targetDir: string; error: string }`.** First-hop copy.
...
- `AdapterSyncProfile`, `SyncAllOpts`, `SyncResult`, `SyncConflict` are all defined inline in `src/echo-home/adapter-sync.ts` (no shared `src/echo-home/types.ts`). `AdapterSyncProfile` is deliberately named to NOT collide with 070's `OnboardedAgentProfile` (the persistent onboarding-state record); they describe different things.
  ...
  interface AdapterSyncProfile {
    ...
    // If REQUIRED but omitted, the markers adapter is SKIPPED for this profile — the agent entry
    // is `ok: true` with `skipped: ['mergeWithMarkers']` and no AdapterError. Callers (073) can
    // pass empty profiles for a no-op probe; production wizard flows always populate.
    echoSection?: string;
    ...
    // REQUIRED for kind = 'codex' | 'cursor'; ignored for kind = 'claude-code' (Claude Code uses
    // CLAUDE.md, not a per-server config). If REQUIRED but omitted: same skipped semantics as above
    // — `skipped: ['syncCodexMcpBlock'] | ['syncCursorMcpEntry']`, agent still `ok: true`, no AdapterError.
  ...
  interface SyncAllOpts {
    ...
    // Lock-acquisition timing knobs (codex r4 M2). All have production defaults; tests override.
    lockRetryBudgetMs?: number;        // default 30_000
    lockRetryIntervalMs?: number;      // default 250
    corruptLockFreshnessMs?: number;   // default 5 * 60_000 (5 min)
    foreignHostStaleMs?: number;       // default 60 * 60_000 (1 hour)
  }
  ```
...
- **Concurrency: per-user advisory lock.** Per codex-ops r2 M2 (lost-update race between overlapping `syncAll` invocations): `syncAll` acquires an advisory lock at `path.join(ECHO_HOME_PATHS.state, 'adapter-sync.lock')` at entry and releases it at exit. Mechanism uses the `tmp-write-then-link` atomic-metadata pattern (per codex-ops r3 M1 — protects against crash between `openSync` and metadata write):
  ...
  2. **Generate per-invocation owner token.** `acquisitionToken = crypto.randomUUID()`. This token is written into the lockfile metadata; release / exit-handler unlinks ONLY when the on-disk token still matches `acquisitionToken` (closes codex-ops r4 M1 — prevents a leaked exit handler from a completed sync from unlinking a different process's live lock).
  3. Write `{pid: process.pid, hostname: os.hostname(), started_at: new Date().toISOString(), acquisitionToken}` JSON to `<lockPath>.<pid>.<randomUuid8>.tmp` (a fresh temp path each acquisition attempt; distinct from the owner token).
  4. Call `fs.linkSync(tempPath, lockPath)` — atomic on the same filesystem; succeeds iff `lockPath` does not exist; fails with `EEXIST` if it does.
  5. On success, unlink the temp; on `EEXIST` apply the stale-lock recovery rules below; on other errors (EACCES, ENOSPC, ENOTDIR, etc.), unlink the temp and convert to the lock-timeout `SyncResult` shape (NOT a throw).
- **Stale-lock recovery (three independent rules; any of them justifies removal — but removal itself is inode-guarded; see "stale removal protocol" below):**
  1. **Dead PID.** Parse the lockfile JSON; if `process.kill(parsedPid, 0)` returns ESRCH (PID not alive on this host) AND `hostname` matches `os.hostname()`, the lock is stale.
  2. **Corrupt or empty lockfile (codex-ops r3 M1).** If `JSON.parse(lockfileBytes)` throws OR the parsed object is missing `pid`/`hostname`/`started_at` OR `lockfileBytes.length === 0`, AND `fs.statSync(lockPath).mtimeMs` is older than `corruptLockFreshnessMs` (default 5 minutes — comfortably longer than any legitimate sync should take), the lock is stale. (A crash between `link` and metadata-readability is the corruption window; the 5-min freshness gate avoids removing a lock that another process just acquired but has not yet had time to be read.)
  3. **Foreign-host lock older than 1 hour.** If `hostname` does NOT match `os.hostname()` (the lock came from a different machine — possible if `~/.echo/` is on shared storage) AND mtime is older than 1 hour, treat as stale. (1 hour is conservative; production-shared-home users probably do not exist in V1.)
- **Stale removal protocol — inode-guarded (codex r5 M3 fix for TOCTOU).** When a caller decides a lock is stale (via any rule above), naive `unlinkSync(lockPath)` is a race: between observation and unlink, another caller could have removed the same stale lock and acquired a fresh one; the second caller's unlink then deletes the new live lock. Fix:
  1. Capture `staleStat = fs.lstatSync(lockPath)` at the moment of staleness judgment (record `staleStat.ino` and `staleStat.mtimeMs`).
  2. Just before `unlinkSync`, `fs.lstatSync(lockPath)` again. If the new inode equals `staleStat.ino` AND the new mtime equals `staleStat.mtimeMs`, the lock is the same file we judged stale — unlink. If either differs, ABORT removal and retry the acquisition loop from the top (another caller already cleaned up; we just need to take their place).
  3. If the second lstat throws ENOENT, the lock was already removed by another caller — proceed directly to retry.
  This is the standard "double-checked compare-by-inode" pattern; it has no race window because between the two lstat calls our process holds no lock, and the moment `unlinkSync` runs the kernel guarantees inode-level atomicity. Pinned by AC9 case 18 (overlapping stale-recovery).
- **Retry budget.** On `EEXIST` after stale-lock check, sleep `opts.lockRetryIntervalMs` (default 250) and retry. Wall-clock budget: `opts.lockRetryBudgetMs` (default 30_000). On timeout, return `SyncResult` with `syncLock` populated (see lock-timeout result shape below).
...
- `tests/echo-home/adapter-sync.test.ts` — seven cases:
  ...
  14. **Lock-timeout `SyncResult` shape** (pins AC6 lock-timeout result shape / codex r3 M1). Pre-create the lock file with a live PID (use `process.pid` of the test runner itself). Call `syncAll` with `lockRetryBudgetMs: 500`. Assert: `result.syncLock` is defined with `code === 'RETRY_CONFLICT'`; `result.agents === []`; `result.roles.results === []`; `result.skillsPopulated.ok === false` with error `'sync_skipped:lock_unavailable'`; `result.overallOk === false`.
  15. **Lock-setup filesystem failure → syncLock, not throw** (pins codex r4 M3 / codex-ops r4 H1). Set `ECHO_HOME` env to a path that points at a *regular file* (forcing ENOTDIR when `mkdirSync(state, { recursive: true })` runs). Call `syncAll`. Assert: it resolves; `result.syncLock` is populated with `code: 'ENOTDIR'` (or `'UNKNOWN'` if the mapping doesn't recognize the errno); `result.overallOk: false`; no exception propagates.
  16. **Exit handler is unregistered on completion** (pins codex-ops r4 M1 — process exit-handler leak). Capture `process.listenerCount('exit')` before and after a successful `syncAll`; assert they are equal (handler registered + unregistered in the finally block). Companion sub-case: simulate a thrown exception inside a mocked adapter; assert the exit handler is still unregistered (finally semantics).
  17. **Owner-token verification on release** (pins codex-ops r4 M1 — stale handler doesn't unlink wrong process's lock). Run `syncAll` A (with mocked delay); while it holds the lock, manually overwrite the lockfile metadata so `acquisitionToken` differs. When A finishes, assert it does NOT unlink the lockfile (the token mismatch protected another process's claim). Cleanup: manually remove the lockfile in afterEach.
  18. **Stale-removal inode-guard prevents TOCTOU** (pins codex r5 M3). Pre-create a stale lockfile (dead PID, correct hostname). Race two `syncAll` callers concurrently against the same `ECHO_HOME_PATHS.state`. Assert: at the end, EXACTLY ONE caller successfully acquired (and released) the lock with its real work executed; the other received `RETRY_CONFLICT` syncLock (lost the race after both observed the stale lock, only one inode-guarded unlink succeeded). Neither caller has its real-work writes silently lost. (Implementation hint: deterministically order the race by stubbing `process.kill` to call `setImmediate` so one caller's stale-check finishes before the other's.)
  19. **previous*-absent rule: existing differs from proposed → conflict** (pins codex r5 M1, codex-ops r5 H1 — verifies the AC1 / AC2 / AC3 "previous absent" rule end-to-end through `syncAll`). Pre-seed CLAUDE.md with `<!-- BEGIN ECHO -->\nold-content\n<!-- END ECHO -->`. Call `syncAll` with `previousEchoSection` OMITTED and `echoSection: 'new-content'`. Assert: claude-code agent is `ok: false` with one conflict entry, no write occurred (file bytes unchanged); files_written is empty. Companion case: pre-seed with markers and content EQUAL to `echoSection`; call with `previousEchoSection` omitted; assert `action: 'noop'`, agent `ok: true`.
  20. **First-run parent-dir creation** (pins codex-ops r5 M1). Use a fresh tmpdir as HOME; verify `<tmpdir>/.codex/` does NOT exist before the call. Call `syncAll` with a codex profile (default paths). Assert: `<tmpdir>/.codex/` now exists; both AGENTS.md and config.toml are written; agent is `ok: true`. Same companion case for `~/.cursor/`.
  21. **Symlink in `~/.echo/skills/` is NOT propagated** (pins codex-ops r5 M2). Pre-create `<echoHome>/skills/regular.md` (regular file) and `<echoHome>/skills/symlinked.md` as a symlink to a fixture file outside `~/.echo/` (e.g. tmpdir `secret.txt`). Call `syncAll` with claude-code profile. Assert: `<tmpdir>/.claude/commands/regular.md` exists with the right content; `<tmpdir>/.claude/commands/symlinked.md` does NOT exist (symlink skipped); skill-sync's `skipped` array includes `'symlinked.md'`. Same negative-pin for `populateEchoSkills` against a symlink in the in-repo `skills/` source (sourceDir hardening).
  22. **Roles error channel** (pins codex r5 M2 + codex-ops r5 H2). Set `targetDir = ECHO_HOME_PATHS.roles` but `chmod 0555` on the parent (so writes to `~/.echo/roles/` fail with EACCES). Call `syncAll`. Assert: codex + cursor + claude-code agents that don't touch roles are `ok: true` and files were written. `result.roles.results[]` contains entries with `action: 'error'` and `error: { code: 'EACCES', ... }` for each role. `result.roles.rolesErrors[]` is a flat list of those errors. `result.overallOk: false`. No exception propagates.
...
- `adapter-sync.ts` — 22 cases pinning all-ok / partial-failure-conflict / user-modified-role-and-overallOk / populate-runs-first / default-roles-from-071 / populate-failure-blocks-fanout / conflict-not-logged / malformed-cursor-json-error-variant / EACCES-error-variant-via-parent-or-mock / per-user-lock-acquire-release / overlapping-sync-RETRY_CONFLICT / stale-lock-dead-pid / stale-lock-corrupt-empty / lock-timeout-result-shape / lock-setup-fs-failure-to-syncLock / exit-handler-unregistered / owner-token-verification / stale-removal-inode-guard / previous-absent-conflict / first-run-parent-dir / symlink-skipped / roles-error-channel.
...
- AC9: All test files pass (markers ×6, codex-config ×7, cursor-config ×7, skill-sync ×8, role-sync ×4, atomic-write ×7, adapter-sync ×22 — 61 cases total). Existing tests in `tests/` (root Vitest) continue to pass.
