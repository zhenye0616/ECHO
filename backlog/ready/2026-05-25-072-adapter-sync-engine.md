...
- Aggregate into `SyncResult`:
  ...
  interface SyncResult {
    ...
    overallOk: boolean;  // true iff:
                        ...
                        //   (i.e. role 'error' and 'source-missing' BOTH flip overallOk to false — codex-ops r6 H2)
  }
  ```
...
- **Lock-failure result shape (pinned in the public contract):** when lock acquisition fails (lockfile EEXIST, EACCES on link, mkdirSync failure on state dir, etc.), `syncAll` returns:
  ```ts
  {
    ...
    syncLock: { code: 'RETRY_CONFLICT', operation: 'stat', file: lockPath, message: <human-readable; tells user how to manually remove if stale; never includes lockfile bytes> },
    overallOk: false,
  }
  ```
  The caller (074 / 073) can distinguish "lock conflict" (`result.syncLock !== undefined`) from "populate failure" (`result.syncLock === undefined && result.skillsPopulated.ok === false`) and render different UX (e.g. "another sync is in progress, or stale lockfile from a crash — `rm <path>` if sure no sync is running" vs "skill source directory unreadable").
...
**AC7.2a — Symlinked target handling (codex-ops r6 M2 — dotfiles workflow preservation).** Before the temp-write, `fs.lstatSync(filePath)` to check if the target is a symlink. If it is, resolve via `fs.realpathSync(filePath)` and write through the resolved target — `atomicWrite` performs the temp-then-rename at the resolved path, NOT at the symlink location. This preserves the common dev-founder workflow of managing `~/.codex/config.toml` or `~/.cursor/mcp.json` as a symlink into a dotfiles repo: the symlink stays a symlink, and the dotfiles-repo file receives the update. If `realpathSync` throws (broken symlink, dangling target), the symlink is treated as the target of the write directly (the rename will replace the broken symlink with a regular file — surfacing the breakage rather than silently propagating it). Pin: AC9 case 23.
...
- `tests/echo-home/adapter-sync.test.ts` — seven cases:
  ...
  21. **Symlinked-dotfile target is written through** (pins codex-ops r6 M2 + AC7.2a). Use a tmpdir as HOME; pre-create `<dotfiles>/codex-config.toml` (regular file); symlink `<tmpdir>/.codex/config.toml -> <dotfiles>/codex-config.toml`. Call `syncAll` with codex profile. Assert: `<tmpdir>/.codex/config.toml` is STILL a symlink (`lstat.isSymbolicLink() === true`); the dotfiles-repo target file received the update (contents reflect `mcpServerConfig`); no temp file remains adjacent to either path.
...
- `adapter-sync.ts` — 21 cases pinning all-ok / partial-failure-conflict / user-modified-role-and-overallOk / populate-runs-first / default-roles-from-071 / populate-failure-blocks-fanout / conflict-not-logged / malformed-cursor-json-error-variant / EACCES-error-variant-via-parent-or-mock / per-user-lock-acquire-release / lockfile-present-then-removable / lock-setup-fs-failure-to-syncLock / exit-handler-unregistered / owner-token-verification / no-temp-leak-on-EEXIST / previous-absent-conflict / first-run-parent-dir / symlink-skipped / roles-error-channel / overallOk-falsifies-on-role-source-missing / symlinked-dotfile-write-through.
...
- AC9: All test files pass (markers ×6, codex-config ×7, cursor-config ×7, skill-sync ×8, role-sync ×4, atomic-write ×7, adapter-sync ×21 — 60 cases total). Existing tests in `tests/` (root Vitest) continue to pass.
