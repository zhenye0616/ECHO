...
11. **(r2 codex F3) Recovery's non-zero return blocks publish.** Setup: corrupt the recovery preconditions (e.g., pass an unsafe surface that fails the prefix-guard, OR make `git rm --cached` fail on a real-error path — NOT just an `--ignore-unmatch` no-op which returns 0 by design). Assert: recovery returns non-zero (3/4/5 depending on the failure); the caller's `|| exit $?` triggers process exit BEFORE `git pull --rebase` runs. The test inspects the process exit code and asserts no pull occurred.
