...
4. **Compare-and-swap (read-to-write race).** `now_remote_blob = git rev-parse origin/main:backlog/task-state/<task-id>/round-state.md` (or `ABSENT` if the path does not resolve at `origin/main`). If `now_remote_blob != base_blob`, abort: another writer landed between step 1 and now. Write a per-event log file at `raw/internal/queue-errors/<ISO-ts>-<writer>-<task-id>.md` containing `ROUND_STATE_WRITE_CAS_ABORT: <writer> base=<base_blob> remote=<now_remote_blob> ts=<ISO>`, do NOT replace the round-state file, exit non-zero. The next tick re-reads from the new base. **First-write CAS:** both `ABSENT` succeeds (no concurrent creator); only one `ABSENT` aborts (another writer either appeared or removed the file — both anomalous).
...
6. **Commit + push via the round-state-specific helper** `tools/task-state/push-round-state.sh <task-id> <base_blob>`. The helper implements a **blob-lease around the push** and does NOT delegate to the generic `push-with-retry.sh`:
   ...
   - On rejection: `git fetch origin main`, then re-read `now_remote_blob = git rev-parse origin/main:<path>` (or `ABSENT`). If `now_remote_blob != base_blob`, the lease is broken. **Durable-log abort sequence** (per-event file shape closes the concurrent-abort-log writer race — 046 R5 F2):
     1. `git reset --hard origin/main` FIRST — discards the stale local round-state commit. Worktree now matches `origin/main`. (Safe because the precondition gate guarantees no unrelated tracked files are dirty — see "Clean-other-than-target invariant" below.)
     2. Write a per-event log file at `raw/internal/queue-errors/<ISO-ts-compact>-<writer>-<task-id>.md` containing `<ISO-ts> ROUND_STATE_WRITE_CAS_ABORT_PUSH: <task-id> base=<base_blob> remote=<now_remote_blob> reason=<reason> writer=<writer>`. Each writer's path is unique by `(ts, writer, task)`, so concurrent abort-loggers never collide.
     3. `git add raw/internal/queue-errors/<filename>` + commit (`queue-errors: round-state CAS push abort (<task-id>)`) + push via the generic `tools/review-queue/push-with-retry.sh` — a log-only commit on a new unique path, uncontested by other writers.
   ...
   **Clean-other-than-target invariant (046 R5 F1).** The helper REFUSES to start if any tracked file OTHER than `backlog/task-state/<task-id>/round-state.md` is dirty (modified, staged, or unmerged). Untracked files (`??`) are ignored. Violation exits 2 with `ROUND_STATE_HELPER_DIRTY_TREE`. This prevents the `git reset --hard` in the abort path from wiping unrelated in-flight strategist/watcher edits.
