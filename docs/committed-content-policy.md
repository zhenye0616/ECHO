...
1. **Secrets.** API keys (`grn_*` Granola keys, `ANTHROPIC_API_KEY`, brain-vendor tokens), `.env*` values, `~/.echo/state` credential files, Slack bot/app tokens, OAuth client secrets, session cookies, private keys. This class is machine-detectable. The repo-owned `tools/secret-scan.sh` pins Gitleaks 8.30.1 and scans all reachable textual history plus separately enumerated binary/archive history with full output redaction; `.github/workflows/secret-scan.yml` runs it on every push/PR, including docs/raw-only changes. Operators may deliberately install the matching local pre-push guard with `tools/install-pre-push-hook.sh`; the installer is never automatic, preserves a different existing hook by default, and requires explicit `--force` to replace it.
