...
| HIGH | [2026-05-08-025](../backlog/ready/2026-05-08-025-mcp-best-practices.md) | V1.5.3 MCP best-practices — structured outputs, source-app enum, cost-safer defaults, pagination, readOnly annotations, doc parity | 1-2d | Six bundled fixes to the MCP wire shape, surfaced by a Claude→Codex peer-review chain on 2026-05-08 (Codex's review at rollout `019e0935-…:338`, 13:25 PDT; Claude's 8/8 reverification at journal 13:35 PDT). **1:** add `outputSchema` + return `structuredContent` on all 3 tools (SDK 1.29.0 supports both). **2:** add `source_app: 'cursor'\|'claude_code'\|'codex'\|'git'` enum on `search_memories`; keep `source_prefix` as escape hatch — closes the foot-gun where two AI clients in one day guessed `'claude_code'` literal and got 0 results. **3:** flip `get_recent_work_context` defaults from `limit=100, format='full'` to `limit=25, format='minimal'` — current default blew the consumer 25k-char tool-result budget on every retrieval today. **4:** add `cursor` + `next_cursor` pagination on `search_memories`. **6:** `readOnlyHint: true` annotations on all 3 tools. **8:** `docs/mcp-integration.md:53,100` doc/test parity. Codex's claims 5 + 7 are no-ops (already correct). |
