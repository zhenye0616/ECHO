...
| HIGH | [2026-05-11-039-cross-tool-review-dispatch-queue](../backlog/ready/2026-05-11-039-cross-tool-review-dispatch-queue.md) | Cross-tool review dispatch queue — file-backed protocol; founder out of dispatch loop (RC2 / R1-patched) | 1.5-2d | Operating-model item, not substrate. Removes founder-as-dispatch-layer between Claude (strategist) + Codex + Cursor on R1→R_final spec-review cycles. **R1 dispatched 2026-05-11 23:35 PDT** (last manual cycle before queue exists per bootstrap moment); both reviewers landed by 23:50 PDT — Codex `pushback` (atom `3711eeae`), Cursor `proceed_after_patches` (bubble `aae455fc`). 18 findings dispositioned (2 HIGH-convergent on `/loop` parity + atomicity; 1 HIGH-near-convergent on push-race; 3 HIGH-singleton on AC6 dispatch coherence + watcher behavior + git-pull-before-poll; 7 MED + 5 LOW). RC2 patches added AC0 (polling-primitive parity), rewrote atomicity (`os.link` no-overwrite + push-race retry), folded watcher behavior into combine.py, mandated strategist `/loop`, split AC6 into AC6a (synthetic e2e) + AC6b (post-merge real-use follow-up). **R2 dispatch needed** to verify atomicity fix + concrete polling-primitive commands per client + strategist /loop implementability. Builder: any agent (pure protocol + helper scripts + slash-command prompts). |
