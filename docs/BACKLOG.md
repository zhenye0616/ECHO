...
| HIGH | [2026-05-08-024](../backlog/ready/2026-05-08-024-fs-watcher-test-quarantine-successor.md) | fs-watcher.test.ts Path C successor — extend 023's quarantine to the third flaky file | 15-30min | 023's verification surfaced `tests/capture/surfaces/fs-watcher.test.ts` flaking on ~33% of runs from the same chokidar `watcher.close()` race. 023's Out-of-Scope correctly refused to expand scope mid-flight; founder shipped 023 as-is and filed this tight successor. Mechanical: `describe.skip` on the `startFsWatcher` block (line 41 only — `classifyKind` and `_isAllowedPathIn` blocks remain enabled), plus the same 023-shape tracking comment ending with `2026-05-08-024-...`. Closes the open portion of the 014 followup's `fs-watcher.test.ts remains open` annotation. |
| HIGH | [2026-05-08-026](../backlog/ready/2026-05-08-026-tail-session-tool.md) | V1.5.4 `tail_session` MCP tool — cheap exact-fetch for the cross-AI handoff demo | 0.5-1d | Fourth MCP tool. Returns N most-recent atoms from a single named `source` (or auto-resolves the most-recently-active session via `source_app`). No clustering, no substring filter — the missing cheap primitive that today is served by direct rollout-JSONL `grep`. Surfaced by 2026-05-08 dogfooding: strategist bypassed ECHO twice in one afternoon (13:27 PDT, 14:00 PDT) for cross-AI Codex lookups because `search_memories` returns paraphrase and `get_recent_work_context` returns clusters — neither shape fits "fetch Codex's last 3 turns from session X". Reuses 025's composite cursor + outputSchema + readOnlyHint scaffolding. **`blocked_by`: 025 AND 027** (per Codex's 14:40 PDT pair-review — 027's stateless-transport refactor must land before 026 registers a 4th tool, otherwise the lex-tie-break in `tools/blocked.py:242` would unblock 026 first). Spec amended same review for: count clamp-vs-reject contradiction (now `min 1` schema + handler-clamps `>20`), exact-source filter using existing `QueryFilter.source` (was incorrectly suggesting `source_prefix` as exact), cursor-helper extraction to `src/mcp/tools/_cursor.ts` if 025's builder kept them file-local. Default `count=5`, clamps to 20, typical response < 10k chars. |
| HIGH | [2026-05-08-025](../backlog/ready/2026-05-08-025-mcp-best-practices.md) | V1.5.3 MCP best-practices — structured outputs, source-app enum, cost-safer defaults, pagination, readOnly annotations, doc parity | 1-2d | Six bundled fixes to the MCP wire shape, surfaced by a Claude→Codex peer-review chain on 2026-05-08 (Codex's MCP-best-practices review at rollout `019e0935-…:338`, 13:25 PDT; Claude's 8/8 reverification at 13:35 PDT; Codex's review of THIS spec at `…:565`, 13:51 PDT, drove the first 4-amendment update; Codex's second review at `…:610`, 14:00 PDT, drove a further 4 amendments — path-aware `limit+1` overfetch (recency-only path uses storage-side, query-bearing path slices post-filter to preserve item 022's invariant), test bullet correction (`.codex/sessions/`), `next_cursor: z.string().nullable()` always-present + `isError: true` on malformed cursor, and storage `ORDER BY ..., id ${orderSql}` parallel-direction with `before`-asc rejection). **1:** add `outputSchema` (incl. `schema_version` + `tool`) + return `structuredContent` on all 3 tools (SDK 1.29.0 supports both). **2:** add `source_app: 'cursor'\|'claude_code'\|'codex'\|'git'` enum on `search_memories`; codex maps to `.codex/sessions/` (narrower per allowlist); keep `source_prefix` as escape hatch. **3:** flip `get_recent_work_context` defaults from `limit=100, format='full'` to `limit=25, format='minimal'` + envelope-byte-size acceptance test (<25k chars on 200-atom fixture). **4:** composite `{timestamp, id}` opaque base64 cursor + `limit+1` overfetch detection + `ORDER BY timestamp DESC, id DESC` deterministic secondary sort in storage layer (closes same-ms-tie skip bug Codex flagged in `sqlite.ts:129`). **6:** `readOnlyHint: true` annotations. **8:** `docs/mcp-integration.md:53,100` doc/test parity. Codex's claims 5 + 7 are no-ops. |
| HIGH | [2026-05-08-027](../backlog/ready/2026-05-08-027-mcp-stateless-transport.md) | V1.5.4 MCP stateless transport — eliminate stale-session failures after daemon restart | 0.5-1d | Root cause of Codex's `JsonRpcMessage` ECHO failures: this Codex session successfully called ECHO at 20:12 UTC, then ECHO daemon restarted at 20:22:11 UTC; from 20:22:20 onward Codex reused a stale `Mcp-Session-Id`, while ECHO's process-local `sessions` map was empty and the custom router returned `400 Bad Request: no active session`. Fresh curl/Node SDK clients still pass, proving the server is live. Fix: make ECHO's read-only MCP tools stateless StreamableHTTP (`sessionIdGenerator: undefined`, JSON response mode), update stale-session regression tests and smoke script, preserve loopback/DNS-rebinding protection. **Blocked by 025** to avoid overlapping smoke-script and docs edits. |
| HIGH | [2026-05-08-028](../backlog/ready/2026-05-08-028-rwc-envelope-skeleton-format.md) | V1.5.5 `get_recent_work_context` envelope fix — `format:'skeleton'` mode + realistic-density acceptance test | 0.5-1d | **Closes the gated followup** ("Second verification round after 026 + 027 merge"). Regression baseline: 72,283 chars (15:05 PDT, post-025) → 76,593 chars (15:14 PDT, "use echo to resume") → **84,188 chars (15:54 PDT, post-026+027, +16.5% worse)** on default args (`limit=20`, `format='minimal'`). Root cause: `truncateForMinimal` only caps `action.input/output`; `artifacts[]`, `actors`, `provenance`, cluster `edges[]`, and `open_loop_hints[].text` pass through unbounded. Fix: add `format:'skeleton'` enum value that strips every uncapped sub-collection, keeping ids + label + source_breakdown + counts only. Realistic-density acceptance test sourced from a real spilled response (the 025 synthetic 200-atom test passed at merge precisely because synthetic atoms were envelope-cheap). Default `format='minimal'` does NOT change — skeleton is caller opt-in. Two related bug classes (search_memories per-match cap; tail_session source resolution) explicitly out of scope, queued separately in `backlog/_followups.md`. |

*(Items 022, 023 shipped 2026-05-08 and now live in `backlog/complete/`; their rows above will be reconciled into the Complete section on the next wiki-promotion pass — the 022 row is already stale.)*
