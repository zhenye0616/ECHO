...
| HIGH | [2026-05-13-043-per-round-reviewer-roster](../backlog/ready/2026-05-13-043-per-round-reviewer-roster.md) | Per-round reviewer roster — make `request.requested_reviewers` the per-round active set; add `reviewers.json` as runtime metadata; defer non-Codex headless runner abstraction | 1-1.5d | **Filed 2026-05-13 post-042 merge.** Generalizes the cross-tool review queue's hardcoded `(codex, cursor)` assumption to N reviewers. Load-bearing fix (AC1): reviewer prompts + `combine.py` honor `request.requested_reviewers` per-round (the schema property already exists; downstream ignores it). AC2-AC3: `reviewers.json` as runtime metadata source; shared `_run_reviewer.sh` + `_install_reviewer_launchd.sh` helpers parameterize on slug. AC4: late-response race fix in `commit-reviewer-response.sh` (re-check `combined.md` existence before `os.link`). AC5: widen `offending_response` regex for hyphenated slugs. AC6: N-way `compute_combined_verdict` with explicit semantics (unanimous; optional missing OK; required missing → `partial_responses` with present-verdict enumeration; proceed*-vs-pushback → divergent). AC7: byte-identical default-deploy regression test. Out of scope: non-Codex headless runner abstraction (YAGNI until real use case), finding-enumeration audit (existing `_followups.md:577` followup), Cursor timeout default change. Spec carries a brainstorm-phase friction log for the next task (per founder direction). **Claim-ready: any agent.** |
