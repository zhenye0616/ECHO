...
| HIGH | [2026-05-08-023](../backlog/ready/2026-05-08-023-chokidar-flake-quarantine.md) | Quarantine the recurring chokidar / capture / daemon-lifecycle flake cluster | 0.5-1d | Re-flagged at every merge verify since 014 (5 references in `_followups.md`). 3-14 fluctuating failures in `tests/capture/extractors/cursor.test.ts` + `tests/daemon/lifecycle.test.ts`. Spec offers three paths (real FSEvents race fix / per-file timeout bump / `.skip()` quarantine); agent chooses based on 3-run baseline measurement. Acceptance: 3 consecutive `npm test` runs with zero failures + close all 5 historical followup references. |
| HIGH | [2026-05-07-021](../backlog/ready/2026-05-07-021-trace-cross-gap-where-left-off.md) | V1.5 trace patch — make "where did I leave off" work across >4h gaps | 1-2d | Two independent bugs jointly break the overlay's primary use case (the dogfooding 22:50 PDT investigation overturned the original C3 conjecture about 019's filter). **Bug A:** `Storage.query()` runs `ORDER BY timestamp ASC LIMIT N` and silently drops the *newest* atoms when limit is hit (verified live: today's window had 3,107 events; 2000-row cap landed exactly on the founder's 22:37 design conversation). Default flips to DESC. **Bug B:** `buildGraph`'s 4h `windowMs` edge cap isn't exposed via MCP, so cross-gap atoms can't cluster even when they share artifacts. Patch exposes `window_hours` and infers from `(since, until)` span (capped at 24h). **Independent guardrail C:** naive ISO timestamps without `Z` are silently TZ-shifted by `Date.parse`; tool emits a one-line `response.warnings` when detected. Prerequisite for the V1 hotkey overlay's "morning orientation after sleep" use case. |
| HIGH | [2026-05-07-020](../backlog/ready/2026-05-07-020-open-loop-resolution-heuristics.md) | V1 trace patch — open-loop resolution heuristics (R1) | 3-5d | Adds `resolved: boolean` + `resolved_by_atom_id?: string` to each `OpenLoopHintEnriched`. Four kind-specific rules: user-Q → later non-Q in same convo; assistant-Q → any user reply in same convo; TODO → later edit on same file; explicit-followup never auto-resolves (V1 conservative). Heuristic-only (no LLM on read path). Prerequisite for the V1 hotkey overlay surface — founder's compositor rule is "unresolved primary, pad to 5 with most-recent if fewer than 5 unresolved (5 is a floor, not a cap)," which decouples cleanly into this trace-layer item plus a separate UI item. Includes a founder hand-scored validation pass against last 7 days of live data; calibration is a follow-up. |

*(Item 019 shipped 2026-05-07; BACKLOG row history will be reconciled into the Complete section on next housekeeping pass.)*
