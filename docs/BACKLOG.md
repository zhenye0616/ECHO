...
| HIGH | [2026-05-14-049-codex-skill-adapter](../backlog/ready/2026-05-14-049-codex-skill-adapter.md) | Codex skill adapter — third sync target for `tools/sync-skills.sh` + vendor-neutralization of `skills/review-pending.md` body | 0.5d | Wires `~/.codex/skills/<name>/SKILL.md` as the third adapter target the cross-tool-protocol decision doc anticipated. Tracked at `adapters/codex/skills/` + `tools/install-codex-adapters.sh` symlinks into user-home. Vendor-neutralizes `skills/review-pending.md` body (replaces Claude-Agent-tool-specific dispatch language with binding-agnostic + per-binding notes sections, mirroring the 047 pattern in `skills/process-backlog.md`). Reviewer roster: two codex (`["codex", "codex-ops"]`) per founder direction "full auto mode with two codex reviewers till converge." Builder: any agent. |
