...
| HIGH | [2026-05-13-044-reviewer-cycle-infrastructure-debt](../backlog/ready/2026-05-13-044-reviewer-cycle-infrastructure-debt.md) | Reviewer-cycle infrastructure debt — fix the 4 compounding frictions from 043's dogfooding (autostash + direct-invoke wrapper + per-reviewer timeout + single-reviewer auto-disposition) | 1-1.5d | **Filed 2026-05-13 post-043 merge.** Inverts 043's "architecture over infrastructure" priority. AC1: watcher Step 1 `git -c rebase.autoStash=true` (fixes #1 + cascading #5). AC2: direct-invoke pattern for manual reviewer force-fires; docs-grep enforcement (fixes #2). AC3: per-reviewer `timeout_hours` from reviewers.json; `DEFAULT_TIMEOUT_HOURS=2.0` deprecated (fixes #3). AC4: single-reviewer-missing-AND-present-proceeds auto-disposition; new `proceed_after_patches_partial` combined verdict; multi-missing + pushback escalations preserved (fixes #4). **First production exercise of 043's "Adding a Reviewer Changelist"** — deploys `codex-ops` (operational/runtime perspective) as the 2nd reviewer; this spec's review uses `requested_reviewers: ["codex", "codex-ops"]` (Cursor omitted for this spec only). §Pre-flight enumerates 6 edits + smoke before r1 dispatches. Out-of-scope: friction items #5-#13 (process-not-infra), generalizing autostash, removing launchd, per-reviewer CLI timeout. Builder: any agent. |
