...
|---|---|---|---|---|
| HIGH | [2026-05-17-060-hotkey-overlay-v0-raycast-dogfood](../backlog/ready/2026-05-17-060-hotkey-overlay-v0-raycast-dogfood.md) | V0 hotkey overlay — Raycast extension for founder dogfooding (V1 spec deferred until v0 surfaces friction) | 0.5-1d | **First V1-product-surface item** (vs. the substrate/coord-layer work that dominated 040-059). Founder confirmed scope choice (a) "v0 first, throwaway" + host shell (a) "Raycast extension" in the 2026-05-17 brainstorm. Ships `tools/raycast-echo/` — Raycast extension calling existing MCP tools (`find_clusters`, `search_memories`, `get_atom`, `get_atoms`) at the daemon's `http://127.0.0.1:38478/mcp`. Two-state list view (empty input → recent clusters, typed → search), 5-action menu (↩ copy / ⌘↩ paste / ⌘O open source / ⌘B trace viewer / ⌘C raw JSON). NO V1 acceptance criteria, NO native Swift app, NO wiki update. AC6 (dogfooding gate) and AC7 (V1 deferred) are the load-bearing scope-discipline ACs. Reviewer roster: `["codex", "claude"]` (one codex + one claude per founder direction 2026-05-17 14:0X PDT). Builder: any agent. |
