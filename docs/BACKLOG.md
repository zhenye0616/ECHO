...
| HIGH | [2026-05-12-040-watcher-state-executable-test](../backlog/ready/2026-05-12-040-watcher-state-executable-test.md) | Watcher post-combine state machine — executable test of AC3.5 (b) (closes Codex R4 LOW #1) | 0.5d | **Live-test target for 039 AC6b loop-close gate** — the first qualifying spec to enter the new file-backed review queue. Extract the watcher's (a)/(b)/(c) post-combine prose from `.claude/commands/review-queue-watch.md` into `tools/review-queue/dispatch-next-round.py`; add integration test in `tests/review-queue/watcher-state.test.ts` with (b)-positive + (a)-negative + (b) race-loser fixtures asserting the load-bearing transition (`r{N+1}/request.md` exists + `next_round: N+1` in prior combined.md). Closes Codex R4 LOW #1 with full falsifiability. Builder: any agent. |
