...
| HIGH | [2026-05-12-042-reviewer-emission-yaml-validation](../backlog/ready/2026-05-12-042-reviewer-emission-yaml-validation.md) | Reviewer emission YAML validation — yaml.YAMLError → typed ValueError in `_lib.parse_frontmatter` (closes reviewer-side gate) + combine.py defensive parse → `escalated_to_founder: true` combined.md (closes watcher crash path) | 0.5d | **Filed 2026-05-12 ~16:30 PDT post-041 merge.** Closes the "🔴 AC3 reviewer-emission validation gap" entry in `_followups.md` (line 522). Canonical failure: 040 R1 cursor.md `finding: ""embedded quote..."` raised `yaml.parser.ParserError` from `_lib.parse_frontmatter:41`; watcher tick crashed; strategist hand-patched. Two-layer fix: AC1 wraps the `yaml.safe_load` call in `_lib` to re-raise as ValueError (which `validate.py` already catches → clean reviewer-visible error, no commit lands); AC2 wraps combine.py's `parse_frontmatter` call sites to emit `escalated_to_founder: true` + `reason: malformed_reviewer_response` combined.md instead of crashing. AC3 adds `escalated_to_founder` / `reason` / `offending_response` / `parse_error` to `combined.schema.json`. AC4 appends one row to `raw/internal/queue-errors.md` per escalation. AC5 observational: **this spec's own review cycle is the AC8 empirical test vehicle** — strategist records founder Codex/Cursor activations in `review_notes` at merge; target = 0. **Claim-ready: any agent.** |
