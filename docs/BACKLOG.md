...
|---|---|---|---|---|
| HIGH | [2026-05-10-034-cursor-capture-coverage](../backlog/ready/2026-05-10-034-cursor-capture-coverage.md) | Cursor capture coverage — mid-stream bubble cadence + tool-call bubble parsing (M1-1) | 1.5-2d | Closes the two capture-layer gaps that drive Cursor review-content invisibility: (A) ~81% miss rate during sustained streaming — chokidar misses mid-stream WAL appends, fix is periodic re-poll with mtime-guard; (B) tool-call frames in `toolFormerData` etc. silently dropped at parse. Builder: **Cursor's Claude** (deliberate dogfooding — Cursor-domain work goes to Cursor's Claude per project convention). Sub-gap C (`tail_session` repo-scoping) deferred to item 035 candidate post-034. |

*V1.6 successor items map (post-030 friction revisit, 2026-05-10): (1) 029 follow-ups [`_followups.md:127-138`], (2) ✅ **032 M2 reliability** — shipped, (3) ✅ **033 full-atom recovery** — shipped, (4) **034 Cursor capture coverage** — this item, M1-1, (5) item 035 candidate `tail_session` repo-scoping (deferred — file when 034 ships + dogfooding shows resolver still load-bearing), (6) hotkey overlay vertical slice, (7) search-ranking / verdict-turn finding (M1-2, **the hardest — saved till the end** per founder direction 2026-05-10 20:08 PDT), (8) subagent dispatch compact/pagination, (9) token-count enrichment per-extractor, (10) 031 deprecation [gated on ≥1 week post-033 dogfooding].*
