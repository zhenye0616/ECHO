...
| HIGH | [2026-05-13-044-reviewer-cycle-infrastructure-debt](../backlog/ready/2026-05-13-044-reviewer-cycle-infrastructure-debt.md) | Reviewer-cycle infrastructure debt — fix the 4 compounding frictions from 043's dogfooding (autostash + direct-invoke wrapper + per-reviewer timeout + single-reviewer auto-disposition) | 1-1.5d | **CONVERGED 2026-05-13 ~13:50 PDT after 3 rounds** at spec_commit_sha `17f44d7` (path-c terminal, verification waived). R1: pushback / 7 findings → R2: proceed_after_patches / 2 → R3: proceed_after_patches / 1 (cross_ref to R2, mechanical). Matches structural-reform baseline (042=3, 040=3); 043's 8-round growth does NOT repeat for class:narrow. AC1: ALL THREE watcher-transaction git pulls (watcher Step 1 + push-with-retry.sh:25 + combine.py:690) use `-c rebase.autoStash=true` (fixes #1 + cascading #5). AC2: direct-invoke via existing `run-<reviewer>-reviewer.sh` 5-line drivers (fixes #2). AC3: per-reviewer `timeout_hours` from reviewers.json; `FALLBACK_TIMEOUT_HOURS=0.5` for null entries; new `not_yet_due` round-eligibility gate (fixes #3). AC4: existing `partial_responses` verdict reused; flip `escalated_to_founder` only for single-missing-AND-present-proceeds sub-case (fixes #4). **First production exercise of 043's "Adding a Reviewer Changelist"** — codex-ops (operational/runtime perspective) deployed; 5 of 10 r1 findings were codex-ops-only (uncorrelated with codex), empirically validating that adding a second reviewer with a different lens adds signal. AC8=0 founder activations during the 3-round review cycle. **Claim-ready: any agent.** |
