...
| HIGH | [2026-05-12-041-reviewer-background-execution](../backlog/ready/2026-05-12-041-reviewer-background-execution.md) | Reviewer background execution — Codex headless via launchd; Cursor accept-degradation policy; mechanically-enforced emission validation | 0.5-1d | **Closes the 040 "founder activation friction" gap** — 040 proved AC6b loop-close on the strict reading (zero founder→reviewer dispatch messages) but founder still physically activated reviewers ~5×/cycle. 041 ships: (1) Codex launchd plist + wrapper script + log file → unattended 10-min ticks after one-time install; (2) Cursor accept-degradation policy documented (manual paste-per-round remains explicit fallback per 039 §AC0); (3) `tools/review-queue/commit-reviewer-response.sh` — mechanically-enforced YAML/schema validation gate (Codex pushback amendment: validate before commit must be mechanically unbypassable, not prose-asked of each reviewer prompt); (4) AC0 sandbox + flag recipe fixes folded in; (5) scoped `atom_id`→`id` audit. AC8 observational: target ≤1 founder activation per cycle (pre-041 baseline ~5). Builder: any (macOS required for launchd verification). |
