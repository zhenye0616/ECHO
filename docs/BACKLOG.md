...
| HIGH | [2026-05-13-045-queue-reliability-friction-cluster](../backlog/ready/2026-05-13-045-queue-reliability-friction-cluster.md) | Queue-reliability friction cluster — recurring frictions surfaced during the 044 cycle | 1-1.5d | **Filed 2026-05-13 post-044 merge** per the friction-first prioritization directive. Bundles 6 recurring frictions from 044's own dogfooding cycle (per the founder request "list all recurring frictions, add to 045"). AC1: reviewer-side YAML emission validation gate (HIGHEST compound-interest signal — 5 quarantines across 042+043+044). AC2: smoke gate fail-closed (original 045 scope absorbed). AC3: orphan-cleanup test fix (`concurrency.test.ts:133`, 3-cycle deferral). AC4: cosmetic prose alignment (`combine.py:684` ↔ `review-queue-watch.md:38`). AC5: worktree cleanup robustness in `/merge-and-cleanup` (handle regenerable node_modules before retry, without `--force`). AC6: `/review-pending` → `/merge-and-cleanup` sidecar commit gap (skill prose edit). Explicit Out-of-Scope defense against F-G through F-P (process discipline + cron-during-cycle + path-c waiver template + round-to-round drift + etc.) — those are 046+ candidates. Cursor omitted per 044 precedent; reviewers = [codex, codex-ops]. **Claim-ready: any agent.** |
