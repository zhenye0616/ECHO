...
|---|---|---|---|---|
| HIGH | [2026-05-07-019](../backlog/ready/2026-05-07-019-trace-edge-filter-and-format.md) | V1.5 trace patch — drop redundant edges + add `format` parameter | 0.75d | Targeted patch on item 018 surfaced by first-day dogfooding (response was 454K — too big for AI client context). Filters scope/session-only edges (97% bloat in tightly-coupled clusters); adds opt-in `format: 'minimal'` query param that caps `action.input/output` to 500 chars + suffix. Default stays `'full'` — the default-flip is a separate later patch. Items B (`shared_artifacts[]` schema) and C (work-role-only clustering) are parked until A's dogfooding produces evidence. Design reasoning in `raw/internal/decisions/2026-05-07-trace-edge-filter-design.md`. |
