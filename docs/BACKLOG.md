...
| HIGH | [2026-05-06-018](../backlog/ready/2026-05-06-018-recent-work-context-tool.md) | V1.5 trace layer — `get_recent_work_context` MCP tool (cross-tool clustering) | 2.5d | New MCP tool returning clusters of related atoms (atoms + join graph, not flat results). Connected components over shared artifact identity within a 4h time window. Read-time pure; no persisted traces table; heuristic labels only. Blocked by item 016. Design reasoning in `raw/internal/decisions/2026-05-06-v15-trace-layer-design.md`. This is the V1.5 magic primitive — substrate for cross-tool answer demo. Wave 4 (GitHub/Slack adapters) deferred to a parallel thread. Item 017 reserved for the parallel `search_memories`-returns-normalized-atoms wiring (per item 016's after-completion notes). |
