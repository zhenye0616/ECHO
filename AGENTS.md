...
`raw/internal/decisions/2026-07-16-echo-context-sequential-program-delegated-authority.md`, as narrowly reconciled by `raw/internal/decisions/2026-07-17-echo-context-137-two-pass-scope-reset.md`, delegates the human founder's decision, merge, push, release, install, and live-execute checkpoints only for items 136, 137a, 137b, 138 and exactly two successor items replacing 139. Items 137a and 137b jointly occupy the original 137 step and do not consume those successor slots. The persistent Codex coordinator must keep one covered item active, use a fresh implementation builder and a different reviewer for each item, commit and read back exact-operation authorizations, and repair/rerun every failed gate. Builders do not inherit this authority: they stop at handoff or uncertainty, and the coordinator resolves and resumes. Any later founder-only or pause-for-founder wording in this file is superseded only within that named scope; ordinary gates remain unchanged elsewhere.
