...
- **Event types.** A versioned registry in `src/coord/types.ts` declares each `event_type` with its tier (round vs scheduler), `schema_version`, and (where applicable) the next event it `expects`. V1 events: `reviewer_invoked`, `tick_start`, `tick_end`, `tick_failed_to_bind`, `scheduler_health`, `scheduler_health_done`, `deadline_missed`. Builder/merger/watcher lifecycle event types are deferred to a follow-on spec — their registry entries don't yet exist and `coord_emit` rejects them at runtime. Unknown event types and unknown schema versions are rejected on write and ignored on read (forward-compat).
