...
**Recovery primitive for elided reviewer responses (V1.6.1, item 033):** Cross-tool review on dense multi-finding turns (Codex `xhigh` reasoning regularly produces these) routinely fires the `truncations: ["content"]` trust signal on `search_memories` / `get_atoms` / `wait_for_new_turns` responses — the strategist would historically read the source JSONL via `jq` / SQLite to recover the full pushback list. [[mcp-get-atom|`get_atom(id)`]] returns the verbatim atom content through MCP in one call, no shell or composer-id required. Use it whenever a reviewer's response comes back elided. This closes Magic Moment M1-3 end-to-end — the cross-tool review pattern is no longer structurally dependent on JSONL/SQLite shell access for elision recovery.

**Post-038 canonical resolver pattern (item 037 + 038 wave):** When the strategist needs to retrieve a specific reviewer's latest turn, the canonical recipe is now [[mcp-echo-resolve-mru|`echo_resolve_mru({sources:[<app>], repo_path:X})`]] → spread descriptor into [[mcp-search-memories|`search_memories`]] (`source: desc.source, ...desc.filter`). For Cursor composers opened before item 037's AC1 shipped (pre-2026-05-11), the resolver returns `phase: 'cursor_legacy'` and `filter.metadata_match: {composer_id}` — no SQLite probe needed. Post-AC1 composers resolve via Phase 1 (`metadata.repo_root`) directly. The SQLite-probe-with-workspace-hash recovery chain documented in the 2026-05-10 22:11 / 22:25 / 22:45 PDT journal entries is now a last-resort fallback for the rare ECHO-substring-index-freshness lag case (M1-1 sub-gap A; 039 R1 + R2 cycle fired this live on the very spec aimed at making the dispatch self-service). Cross-project bleed is structurally impossible post-037+038.
