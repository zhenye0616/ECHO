...
A typical Codex session contains roughly **3–4× more assistant messages than user messages** (assistants stream multi-paragraph replies plus reasoning + tool turns). The extractor pairs each user message with **every consecutive assistant message until the next user (or end-of-file)**, joining their texts with `\n\n`. This matches the cluster pattern adopted by [[cursor-extractor]] for the same reason.
...
assistant a2        ─── still pending
(EOF)               ─── pending; not emitted until the next user appears
```
...
The "only advance through emitted clusters" rule is critical: pending clusters live AFTER the recorded offset, so they get re-parsed (not re-emitted) on the next pass. When the closing next-user finally arrives, the cluster emits exactly once.
...
`session_id` doubles as the chat thread identifier. `cwd` lets you filter or group by which project the session was running in (Codex captures cwd in the `session_meta` line; the extractor caches it and stamps every emitted turn from that pass).

## What It Does NOT Do

- **Does not extract tool-call payloads.** When the assistant invoked a tool between the user and the next user, the turn is marked `had_tool_use: true` but the tool's name, arguments, and output are not captured. A future Tier-A-style extension (analogous to Cursor's) could surface tool names and stdout snippets into `metadata.context`.
- **Does not extract reasoning blocks.** Codex's `response_item.payload.type == 'reasoning'` lines (the assistant's hidden thinking) are dropped. Only user-visible `message` content lands in storage.
...
- **Does not emit pending clusters on EOF.** A cluster only emits when the next user arrives, proving the cluster is closed. Sessions that end on an assistant and never receive another user message lose their final cluster. Acceptable for V1; a future improvement could emit on file-mtime-stable-for-N-minutes.
