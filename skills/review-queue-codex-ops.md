...
## Step 5 — Write codex-ops.md atomically and commit via the validation helper

Write to a unique temp file, then `os.link` it into place (no overwrite). If `FileExistsError`, someone else (a retry of yours, or a parallel codex-ops agent) wrote first — drop your temp and skip this candidate.
