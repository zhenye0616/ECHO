...
export class TomlParseError extends Error {
  constructor(
    ...
    super(`TOML parse failed (offset ${byteOffset}): ${firstLine}`);
...
function parseSliceUnwrap(slice: string, filePath: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new TomlParseError(filePath, firstLine, 0);
  }
  if (!isPlainObject(parsed)) {
    throw new TomlParseError(filePath, 'parsed slice was not a table', 0);
  }
  const mcpServers = parsed['mcp_servers'];
  if (!isPlainObject(mcpServers)) {
    throw new TomlParseError(filePath, 'missing mcp_servers section in parsed slice', 0);
  }
  const echo = mcpServers['echo'];
  if (!isPlainObject(echo)) {
    throw new TomlParseError(filePath, 'missing mcp_servers.echo section in parsed slice', 0);
  }
  return echo;
}
